...
| Vertical click frame workload | Upstream animation frames write the comparatively light root-document scrollTop; the ordinary `上下滑动` handler derives page number and defers persistence. | Fourth user verification exposed the remaining structural cost: `06ac89b` promoted the full, potentially tens-of-thousands-pixel `.reader-body` into one temporary WAAPI transform layer. The current fix removes all full-body `animate()`/`will-change` work and uses the already-deferred lightweight scrollTop frame path with the accepted faster-start symmetric curve. | **fourth P0 fix and Docker publication completed in `32dc616` on 2026-07-19; user phone verification pending.** A 360-paragraph fixture at 390×844/360×800 proves no full-body animation/layer hint, continuous time-based scrollTop movement, exact duration/endpoint, bounded repeated taps, no selection polling, and preserved native touch/wheel. See [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md). |
