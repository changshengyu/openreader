...
**browser-validated / awaiting device verification**：移动竖向点击恢复 power-cubic ease-in-out，
动画中拒绝同向/反向重叠点击，结算按配置时长延后；可见段落从全章几何测量改为命中即停，
同一稳定位置只捕获一次快照。前端 `532/532`、生产构建、Go 全量以及 1440×900、390×844、
360×800 的文字模式、移动工具层/iPad 与连续跨章真实 Chromium 合同通过。发布状态和镜像摘要
将在本批本地 Docker 门禁完成后补记。
