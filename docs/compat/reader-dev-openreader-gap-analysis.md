...
| Online source parsing | Upstream reader3-compatible source semantics live across `AnalyzeRule` plus `BookList/BookInfo/BookChapterList/BookContent`. | Current Go parser executes the extracted CSS/JSONPath/XPath/regex/composite/replace/pagination subsets and bounded request-scoped `@put`/`@get`; JS/templates remain explicit errors. | 1G audit confirms upstream persists JSON `variable` on search/book/chapter and uses chapter→book→temporary lookup. P2-Parser-2A now provides redacted additive parser `code`/`stage` errors without changing legacy statuses or `error`; the additive, user-scoped SQLite/backup variable contract remains. `{{...}}` JavaScript remains an explicit security-gated difference, never a silent empty result. | `partial` for P2 parser | Variable migration/backup fixtures, request/user isolation tests, source-debug/error-redaction tests, browser source flow. |
