...
| Vertical click frame workload | Upstream animation frames write scrollTop; the ordinary `上下滑动` scroll handler only derives page number and defers persistence. | Every inner-scroller animation frame currently runs chapter synchronization, window-extension checks, layout reads, visible-block queries, local progress assembly, and persistence scheduling. | **2026-07-18 must-fix after mobile user regression.** Defer heavy synchronization while the programmatic animator is active, then settle exactly once without changing the upstream cubic curve, duration, or page stride. See [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md). |
| Page-mode chapter-end entry | Upstream non-slide/non-scroll content ends with a flow-positioned `加载下一章`; the last chapter keeps the entry and reports `本章是最后一章` when clicked. | No chapter-end entry is rendered. | **2026-07-18 must-fix.** Restore the flow entry with explicit last-chapter handling and touch/click propagation guards; do not place it in the fixed chrome. |
