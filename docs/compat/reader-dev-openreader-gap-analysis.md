...
| Vertical click frame workload | Upstream animation frames write the comparatively light root-document scrollTop; the ordinary `上下滑动` handler derives page number and defers persistence. | After third user verification, ordinary taps now use one non-retrying selection check, mobile text/page prewarms its temporary compositor layer at touchstart and releases every non-page path, and only that compositor path uses the user-requested faster-start symmetric curve. | **third P0 code fix and Docker publication completed on 2026-07-18; user touch verification pending.** Dual-mobile input-window evidence requires at least 1% page movement within 40ms, no selection polling, no ≥50ms Long Task, exact configured duration/endpoint, and cleared layer state. Full mobile selection, native continuous scroll and historical-volume gates pass in `06ac89b`. See [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md). |
