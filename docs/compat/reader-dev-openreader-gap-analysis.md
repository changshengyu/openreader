...
| RSS | Upstream `RssSourceList.vue`, `RssArticleList.vue`, `RssArticle.vue`. | Current root source dialog, independent article-list/content dialogs, `RSSManager.vue`, overlays and Go RSS parser. | The three-dialog transition and compact fullscreen behavior have been rebuilt; the 2026-07-27 lifecycle re-audit found blank-name fallback, same-URL duplicate creation, stale article-list response ownership and non-transactional source deletion. Persistent per-user cache/filtering and sanitization remain allowed adaptations. | `partial`; see focused lifecycle contract | [`rss-source-lifecycle-p2-contract.md`](rss-source-lifecycle-p2-contract.md), RSS fixture/parser tests and source/article browser smoke. |
