...
| Vertical click frame workload | Upstream animation frames write the comparatively light root-document scrollTop; the ordinary `上下滑动` handler derives page number and defers persistence. | The first fix defers heavy business synchronization, but mobile page animation still writes the inner `.reader-content.scrollTop` on every main-thread frame. Its same-thread sampling smoke passes even when both animation and sampler pause together; the user still observes a stop-start feel. | **reopened 2026-07-18 after second mobile regression.** Move only mobile page click motion to a composited body transform, commit exact scrollTop once at settlement/cancellation, and add one bounded queued page action. Keep configured duration, stride, chapter boundaries and native touch/wheel. See [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md). |
