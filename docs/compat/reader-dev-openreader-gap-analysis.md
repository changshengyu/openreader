...
| Auth/user management | Upstream `AddUser.vue`/`UserManage.vue` impose account rules, a shared desktop/mobile table, exact last-login data, separate WebDAV/LocalStore rights, protected default namespace, private/default source actions and complete namespace deletion. | JWT/admin protection, account rules, independent permissions, transactional deletion and user/default source ownership are implemented. The remaining UI substitutes mobile cards and a merged permission column, reverses footer operations, omits Cancel, and exposes a `lastActiveAt` value that login never updates. | Data ownership/account safety is aligned; the 2026-07-28 re-audit reopens the visible manager shell and last-login semantics only. | `P2 UI/last-login must-fix; source ownership aligned` | [`user-management-p2-contract.md`](user-management-p2-contract.md), [`book-source-ownership-p2-contract.md`](book-source-ownership-p2-contract.md); login/API contracts plus one-table three-viewport browser gate. |
