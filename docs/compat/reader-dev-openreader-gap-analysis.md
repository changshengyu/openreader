...
| iPad / forced-mini state consistency | Upstream adaptive mini mode is width-only (`window.innerWidth <= 750`); only a non-adaptive page mode forces mini. | `b8b70f9` correctly made the selected mini state drive one coherent DOM/CSS scene, but the shared predicate still treats any iPad/mobile UA as mini at 1024/1366px. That makes Reader primary panels and unrelated dialogs use phone/full-width presentation on a wide tablet. | **reopened 2026-07-19 / must-fix.** Adaptive wide iPad must use desktop Reader/dialog presentation; explicit persisted phone mode may still force the coherent mobile scene. See [`reader-ipad-responsive-p0-contract.md`](reader-ipad-responsive-p0-contract.md). |
