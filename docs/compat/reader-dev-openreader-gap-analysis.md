...
- **Remaining P1-D4-B.** `refreshBook`, `changeBookSource`, and `refreshLocalBook` still need post-commit superseded-cache pruning with fixture coverage. The upstream SSE cache progress/disconnect-cancel interaction has not been restored; the current bounded REST behavior remains under review and must not be treated as a completed parity decision.
