...
| Reader content formats | Upstream `Content.vue` handles text, images/comic-like content, EPUB iframe documents, audio-related branches, read-aloud, and cross-chapter behavior. `EpubFile.kt` additionally treats `BookChapter.startFragmentId`/`endFragmentId` and the next chapter URL as EPUB content boundaries. | Current `ReaderChapterContent.vue` handles text/images/volume blocks, CBZ image resources, EPUB iframe resources, a dedicated audio branch for `type === 1` chapters, and the extracted TTS/read-bar state machine. | E4 fixed the image-only first EPUB spine cover. The new upstream extraction confirms the current EPUB parser strips TOC/NCX fragments and collapses same-resource entries, while the iframe serves full documents. E4-EPUB-2 must restore fragment slices and exact cross-resource chapter mapping without weakening capability/CSP protections. | `must-fix` for E4-EPUB-2 | [`epub-fragment-p1e4-contract.md`](epub-fragment-p1e4-contract.md): failing engine/API/browser/security fixtures, then migration-aware implementation and old-volume smoke. |
