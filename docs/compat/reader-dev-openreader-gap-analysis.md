...
- **Current evidence.** Static/controller contracts in `rssWorkspaceContract.test.mjs` and the full frontend suite (**350** tests) pass; `npm run build` passes. `scripts/smoke/rss-workspace-contract.mjs` is ready to validate source → article list → content → image → close/reopen at `1440×900`, `390×844` and `360×800`, but its Chrome run is currently blocked by the workspace execution-credit policy. No backend API/data code changed in this visual slice.
- **Release status.** This RSS batch is **not a Docker release candidate** until the three-viewport browser contract, global workspace-operation smoke, local Docker build and mounted-volume/backup smoke all run successfully from a Git-traceable commit.
