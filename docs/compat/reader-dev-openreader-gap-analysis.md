...
| Online source parsing | Upstream reader3-compatible source semantics live across `AnalyzeRule` plus `BookList/BookInfo/BookChapterList/BookContent`. | Current Go parser preserves/imports many rule fields but executes only a simplified CSS selector form; JSONPath/XPath/regex rule chains, variables and JS fields cannot be treated as aligned. | 2026-07-13 contract extracted in `docs/compat/online-booksource-parser.md`. Rebuild the shared non-script evaluator with golden fixtures before changing any search/info/toc/content flow; JS remains an explicit security-gated follow-up rather than a silent empty result. | `must-fix` for P2 parser | CSS/JSONPath/XPath/regex fixtures across search/info/toc/content; explicit unsupported-JS contract; browser source flow. |
