...
| Bookshelf/BookManage/BookGroup | Upstream: `BookShelf.vue`, `BookManage.vue`, `BookGroup.vue` under Index workspace. | Current: `Home.vue`, `OverlayBookManagement.vue`, `OverlayBookGroups.vue`, unified scoped BookGroup projection and Category/BookCategory assignment model. | The 2026-07-19 P2 implementation now persists and manages all four built-ins, audio/local/ungrouped/custom filters, full mixed ordering, set-mode add/edit, stable selected tab, sync and `bookGroup.json` round-trip. Many-to-many custom categories remain an allowed data adaptation. | `aligned` for the completed BookGroup P2 contract; `acceptable technical adaptation` for many-to-many categories and hardened validation | Evidence and allowed differences are fixed in [`book-group-p2-contract.md`](book-group-p2-contract.md). |
...
| iPad / forced-mini state consistency | Upstream mini is width-only. OpenReader intentionally adds mobile-browser detection and a persisted “手机模式”, so a viewport wider than 750px may still be mini. | The JS state selected mobile panels on iPad Pro while several CSS blocks still required `max-width:750px`; the top tool strip was hidden behind a content-sized settings/catalog/shelf/source panel, removing the visible close path. `isMobileReader` now controls the root semantic class plus mounted desktop/mobile tools; the mobile chrome no longer adds a conflicting width gate. | **resolved 2026-07-19.** Portrait 1024×1366 and landscape 1366×1024 iPad Safari/touch browser contracts prove all four primary panels retain an interactive same-button close path, desktop controls are absent, and 16px text symmetry is preserved. |
...
| Vertical click frame workload | Upstream animation frames write the comparatively light root-document scrollTop; the ordinary `上下滑动` handler derives page number and defers persistence. | Fourth user verification removed the full-body WAAPI layer; fifth phone feedback exposed a remaining zero-displacement first rAF, per-page heavy settlement before a buffered page, and the responsive curve being limited to `page`. OpenReader now seeds a protected 1ms responsive ease-out position, applies it to all three mobile text vertical modes, and settles a same-direction buffered chain only once. | **fifth P0 fix and Docker publication completed in `895d536` on 2026-07-19; user phone verification pending.** A 360-paragraph fixture at 390×844/360×800 proves first-observable-frame movement, monotonic cadence, exact single/double endpoints, no full-body animation/layer hint, no selection polling, and preserved native touch/wheel. See [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md). |
