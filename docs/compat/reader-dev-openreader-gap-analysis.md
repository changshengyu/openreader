...
| Reader content formats | Upstream `Content.vue` handles text, images/comic-like content, EPUB iframe documents, audio-related branches, read-aloud, and cross-chapter behavior. | Current `ReaderChapterContent.vue` handles text/images/volume blocks, CBZ image resources, EPUB iframe resources, a dedicated audio branch for `type === 1` chapters, and the extracted TTS/read-bar state machine. | The E4 audit found and corrected a missing image-only first EPUB spine cover: it now survives preview/import and opens through the same protected iframe capability. EPUB fragment/cross-resource and remaining real-format fixtures still need their P1-E4 gates. | `partial` for P1-E4 format audit | Keep EPUB/image/CBZ/continuous/audio/TTS browser contracts in every release candidate; complete E4 fixtures and old-volume smoke. |
