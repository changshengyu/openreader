...
| Action ownership | Upstream `BookInfo.vue` owns only BookInfo-native actions: add to shelf for non-shelf books, cover upload, local-book update, follow/update switch, and group setting. Current OpenReader still lets `Search.vue`, `Discover.vue`, `AppLayout.vue`, and `useReaderPanels.js` hand-build contextual overlay actions. | This preserves a single visible overlay, but action policy is still duplicated and can drift by entry point. | `must-fix` |
| Search/discover existing-book preview | Upstream uses the same BookInfo dialog fed through the Index workspace event bus; it does not route to a separate detail scene. Current Search/Discover correctly open the shared overlay but duplicate `查看详情`/`继续阅读` action construction. | Keep old-link and “read after add” compatibility only if generated by shared action policy rather than local page code. | `partial` |
| Route compatibility action | Upstream has no `/books/:id`; OpenReader keeps it only as old-link compatibility. Current `AppLayout.vue` adds `继续阅读` directly while opening the shared overlay. | Acceptable compatibility action, but it should come from the shared BookInfo action contract. | `must-fix` |
| Reader BookInfo actions | Upstream Reader opens the same `BookInfo.vue` for the current reading book; reader toolbar buttons such as catalog/bookmarks/settings remain separate controls. Current `useReaderPanels.js` injects catalog/bookmark/search/source/settings/cache actions into BookInfo. | Candidate for removal or explicit compatibility note; at minimum the action list must be generated centrally and must not hide reader chrome. | `must-fix` |
