...
| Online source parsing | Upstream reader3-compatible source semantics live across `AnalyzeRule` plus `BookList/BookInfo/BookChapterList/BookContent`. | Current Go parser now executes the extracted CSS/JSONPath/XPath/regex/composite/replace and pagination subsets; it preserves JS/variable fields but rejects them explicitly. | `@put`/`@get` need a bounded request-scoped runtime; upstream cross-request book/chapter variables need a separate migration and multi-user contract. `{{...}}` JavaScript stays an explicit security-gated difference, never a silent empty result. | `partial` for P2 parser | Golden parser fixtures plus request/user isolation tests; source-debug/error-redaction tests; browser source flow. |
