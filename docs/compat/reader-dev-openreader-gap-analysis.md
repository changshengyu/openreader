...
| Edit ownership | `BookEditDialog` is shared through `OverlayBookInfo`; Home and BookManage open it directly. | `partial`：structured edit 是 `acceptable-change`，但当前附带旧 `categoryIds/canUpdate` 会覆盖并发更新；前置条件与 shelf/reader/BookInfo 同步需按 [`book-edit-metadata-p2-contract.md`](book-edit-metadata-p2-contract.md) 实施。 |
