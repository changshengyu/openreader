...
| Bookshelf/BookManage/BookGroup | Upstream: `BookShelf.vue`, `BookManage.vue`, `BookGroup.vue` under Index workspace. | Current: `Home.vue`, `OverlayBookManagement.vue`, `OverlayBookGroups.vue`, shared controllers and user-scoped category store. | The 2026-07-19 full BookGroup re-audit supersedes the earlier shell-only claim: current management omits all four persisted built-in groups, audio filtering, mixed ordering, built-in rename/show, set-mode add/edit, selected-tab persistence and `bookGroup.json` round-trip. Many-to-many custom categories remain an allowed data adaptation. | `must-fix` for BookGroup full state/data flow; prior Dialog shell and custom category operations remain usable evidence only | Implement and verify [`book-group-p2-contract.md`](book-group-p2-contract.md) before restoring an aligned label. |
