...
| Vertical click frame workload | Upstream animation frames write scrollTop; the ordinary `上下滑动` scroll handler only derives page number and defers persistence. | Programmatic vertical animation now defers the heavy inner-scroller synchronization and flushes once at the final scrollTop; a late identical browser scroll event is deduplicated. | **resolved 2026-07-18 after mobile user regression.** Cubic curve, duration, page stride and native touch/wheel remain unchanged; sampled mobile frame trajectories pass. See [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md). |
