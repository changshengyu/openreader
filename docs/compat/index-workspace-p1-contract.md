...
| 侧栏内容布局 | 上游是纵向标签/设置区；当前移动端把多数管理动作重排成双列按钮，桌面还增加网格/列表视图切换。 | **must-fix（视觉与误触风险）**。这些改变没有列入允许差异，不能凭“功能可达”保留。 | P1-A 需要基于上游侧栏的间距、单列设置语义、操作顺序和最小点击区域重建；网格/列表仅在明确记录为用户可选增强后才可保留。 |
...
2. 以 `Index.vue` 的纵向信息层级和单列设置语义重建 `AppLayout.vue` 侧栏；不得把源管理/账户等还原成独立路由。
