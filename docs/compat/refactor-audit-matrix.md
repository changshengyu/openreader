...
| 书源与搜索/探索结果 | `Index.vue#toDetail`、`Index.vue#debugBookSource`、`bookSourceDebug/*`、`Debugger.kt`、`Reader.vue#getCatalog`、`App.vue#getBookContent`、`Explore.vue`、`BookSource.vue`、`BookController.kt` | `SourceManager.vue`、`SourceDebug.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`ExploreWorkspacePopover.vue`、`backend/api/source_debug.go`、`services/sourcedebug`、`remote_reader.go`、`services/remotereader`、Go source APIs | 书源 owner/API/解析主链、搜索/探索工作区和临时 Reader 第二轮保持已发布。**书源调试第二轮已按固定基准重建并通过完整非 Docker 门**：独立三列工作区、reader-dev JSON、九项命令、账号草稿/历史、save-before-debug、五入口单流自动链、variable/下一章边界、取消和脱敏日志已恢复；错误三探针 Dialog 已删除，旧探针只作无失败缓存副作用的兼容端点。 | [`source-debug-fixed-baseline-second-audit-p2-contract.md`](source-debug-fixed-baseline-second-audit-p2-contract.md) 状态 `implemented / full-regression-passed / browser-validated / Docker-pending`；frontend 724/724、Go/full/race/vet/build/diff 与四视口真实浏览器通过。共享 fetcher 安全边界和临时 Reader `30dbe53` 合同继续适用。 |
