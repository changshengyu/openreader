...
| 替换规则 | `ReplaceRule.vue`、`ReplaceRuleForm.vue`、`Reader.vue#filterContent/showTextFilterPrompt`、`ReplaceRuleController.kt`、`ReplaceRule.kt` | `OverlayReplaceRules.vue`、`useOverlayReplaceRules.js`、`replace_rules.go`、`books.go#applyUserReplaceRules`、backup/restore | **2026-07-28 固定基准复审后重新打开 / 必须重建**：历史 `57b1dc0` 只对齐了 Dialog 外壳，内部仍有上游不存在的新增/刷新/逐行删除/测试器/移动 cards；导入会伪造 name 和 trim pattern；list/apply/backup 错用 `sort_order`；scope 与 restore identity 也偏离。旧“已完成”记录及其测试不再作为正确性证据。 | 以 [`replace-rule-fixed-baseline-p2-contract.md`](replace-rule-fixed-baseline-p2-contract.md) 为唯一当前合同；严格执行测试先行，完成前不得再宣称 ReplaceRule 对齐。 |
