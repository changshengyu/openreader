...
| 书源与搜索/探索结果 | `Index.vue`、`Explore.vue`、`BookSource.vue`、`BookSourceController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`RemoteBookJsonEditorDialog.vue`、`ExploreWorkspacePopover.vue`、presentation/composable、Go source APIs | **书源 owner/API/解析主链保持已发布；搜索/探索可见工作区第二轮已作为 `c851c5f` 发布**。可见搜索现只保留 single/multi 并按场景显示控件；结果保持服务端顺序并与普通书架共用固定 380px grid/card；共享 JSON editor/add 双动作已恢复；Explore 恢复 desktop 600px/top0 和 mini 100vw 非全高、无 backdrop、多开并保留现场。P2-N1 共享 fetcher 的 body/redirect/scheme/retry/timeout/redaction 边界已经实现并作为 `981bca7` 发布。 | 可见工作区保持 `aligned`。[`shared-source-fetcher-p2-contract.md`](shared-source-fetcher-p2-contract.md) 状态为 `P2-N1 Docker-published / P2-N2 pending`；private-network/DNS/proxy allowlist 仍未签收，不能宣称共享 SSRF 已整体关闭。 |
...
| 书源解析、RSS、远程抓取 | `AnalyzeRule*`、`Rss*`、`BookSourceController.kt` | `backend/engine/source_*.go`、`rss_parser.go`、fetcher、`services/rss` | **CSS/JSONPath/XPath 书源主链和 RSS 请求页语义已实现；P2-N1 共享抓取边界已作为 `981bca7` 发布**。可见 page 1/2 只执行请求页；标准 feed 的 page>1 不发网；owned sort allowlist、事务/parser order/隐藏状态保持。共享 fetcher 现有 15 秒 timeout、16 MiB body、5 redirect、3 retry、HTTP(S)/userinfo 和跨 origin credential 边界。 | [`shared-source-fetcher-p2-contract.md`](shared-source-fetcher-p2-contract.md)：N1 全量/race/browser/new+historical volume 通过，OCI index `sha256:02160e0797b3371fdfadccb550b8766d412c3e09df632ba1e36d192b26eb500d`。P2-N2 private-network/DNS-rebinding/proxy/allowlist 仍为独立 `must-fix`。 |
