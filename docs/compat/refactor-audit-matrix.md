...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置、夜间面、远程正文和换源写入生命周期均已专项实施。**2026-09-13 真机 stale 409 回归已随 `2bbb276` 修复**：保留后端 snapshot guard，精确冲突按 Book scope 串行重取一次，取消/切书不提交，内部英文不再直接显示。 | [`reader-chapter-content-request-lifecycle-fixed-baseline-second-audit-p0-contract.md`](reader-chapter-content-request-lifecycle-fixed-baseline-second-audit-p0-contract.md) 补充合同 `e19581b` → 红测 `811d679` → 修复 `2bbb276`；frontend 752/752、build 和四视口注入 409 Chromium 通过。当前实现包含并发布于 `3e8cec7`，状态 `aligned / regression-validated / Docker-published / awaiting-device-verification`。本地章节回建 `a131aa9` 的已发布合同保持。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release workflow | `3e8cec7` 本地通过 frontend 752/752、Go focused/race/full/vet、build、Compose 和 Reader 注入 409 四视口；可信 Actions run `34747604054` 又通过 backend/frontend/build/Compose、native、fresh/portable、historical volume 与 published-platform 门。前端构建固定在 `$BUILDPLATFORM` 原生运行；Go/CGO 与最终镜像仍按目标架构生成。 | Git 实现/Actions/Docker commit 为 `3e8cec7`；已发布 `3e8cec7`/`latest` OCI index `sha256:c2c686d83ff63afb3e764e0a1878d176673d65a49d5ab7e9b5d7fc1a9d96c52d`。amd64 manifest `sha256:99f5fc4eaf26c530a57ff5796a79cdab9d59d0d7a63127210adc72e527ce2e84`，arm64 manifest `sha256:5aff01a7a24976d2011c4b469f80858f1038ae9a0c72d92717ededa833aac332`；两个 `unknown/unknown` 为 attestation，用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-09-13，user-asset lifecycle implemented）
...
- **P0 Reader 主链已覆盖**：工具层/面板状态机、正文排版、移动点击与连续滚动、设置、书签、正文
  ...
  真机暴露的 stale 409 可见回归已以 `e19581b`/`811d679`/`2bbb276` 关闭，后端保护保持，Reader 自动
  串行重取一次且不再显示首次内部冲突。
