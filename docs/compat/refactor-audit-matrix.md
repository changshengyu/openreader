...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`components/workspace/LocalStoreManager.vue`、`OverlayLocalStore.vue`、`WebDAVBrowser.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/api/localstore*`、`backend/services/localbook/*`、`engine/*_parser.go` | P1-E1…P1-E5、P2 parser 预算和 TXT 默认目录规则已发布。**2026-08-12 LocalStore 文件系统/请求边界已测试先行实施**：聚合 multipart、JSON/基数、临时文件、symlink/special-file 和 opened regular-file 边界关闭，固定上游多文件/逐文件可见语义与 private root/token 保持；无数据迁移。 | [`local-store-filesystem-request-boundary-fixed-baseline-second-audit-p2-contract.md`](local-store-filesystem-request-boundary-fixed-baseline-second-audit-p2-contract.md) 已通过 focused/full/race/vet、frontend 740/740、build、宿主和本机 arm64 candidate HTTP；实现 `bba99e1`、探针/candidate `930be4d` 已推送。fresh/historical/portable 卷门受 Docker socket 审批阻断，状态 `implementation-complete / regression-validated / Docker-release-pending`；正式发布仍是 `be83a0f`。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。公开 auth、管理员 user、用户配置、BookGroup/Category、Book、BookSource、Bookmark、RSS source/article 与 LocalStore 写入/文件系统边界均已关闭；LocalStore 正等待卷门和正式发布。 | [`rest-action-coverage-fixed-baseline-p2-contract.md`](rest-action-coverage-fixed-baseline-p2-contract.md) 持续记录动作差集。下一项须重新盘点 `server.go` 中尚未专项签约的动作，独立固定上游合同和旧实现红测，不能用模块级绿测替代动作级 wire/事务证据。 |
