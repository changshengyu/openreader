...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 继续复核**：标题排版、书源可用性、移动最终顶部顺序、浮动按钮、底部渲染页进度和竖向真实动画时长已完成；连续跨章窗口/阈值/原生滚动已对齐，但固定基准二次复审发现当前段落仍误用 32% 视口锚点、扩章期间未隔离进度写入、异步 append/retry 缺少 generation。 | [`reader-continuous-fixed-baseline-p0-contract.md`](reader-continuous-fixed-baseline-p0-contract.md) → 先补失败测试再修复；既有 continuous smoke 不能替代新增的顶部章切换、瞬态 progress 和旧事务失效门禁。 |
| Reader：EPUB、漫画/CBZ、音频、连续跨章、TTS | `Reader.vue`、`Content.vue`、本地格式解析类 | `ReaderChapterContent.vue`、`ReaderEpubContent.vue`、`ReaderAudioContent.vue`、格式 parser / cache | **EPUB 与 CBZ 固定基准切片已完成实现、三视口验证、历史卷/portable backup 和 Docker 发布**：CBZ 新导入预建不可变派生图片树，warm read 不重哈希/遍历源 archive，GET/HEAD/Range 流式；图片布局与普通漫画控制状态已分离，CBZ 保留 mode/自动阅读/TTS。音频、连续跨章和 TTS 引擎内容仍按各自合同继续复核，不由本批扩大签收。 | [`epub-fixed-baseline-catalog-reader-contract.md`](epub-fixed-baseline-catalog-reader-contract.md)、[`reader-cbz-fixed-baseline-p0-contract.md`](reader-cbz-fixed-baseline-p0-contract.md)；继续审查尚未签收的内容格式与状态机。 |
