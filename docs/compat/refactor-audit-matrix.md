...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置、夜间面和内联章节缓存均已专项发布。**主正文请求生命周期已随 `0a8a0ef` 实施**：AbortSignal/generation/book scope 拒绝迟到响应与缓存回写，换源恢复位置不被旧请求或失效 reload 改写。 | [`reader-chapter-content-request-lifecycle-fixed-baseline-second-audit-p0-contract.md`](reader-chapter-content-request-lifecycle-fixed-baseline-second-audit-p0-contract.md) 已通过前端 748/748、build、四视口延迟换源及后端 focused/race/full/vet；状态 `aligned / regression-validated / GitHub-sync-and-Docker-evidence-pending`。已签收布局、连续/点击规则、格式、工具层、换源面板和缓存可见 UX 不重开。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category/remote-add 列所有权均已实施。共享章节 loader 已补 caller/source/book/chapter/initial-variable snapshot 复验、request-context 与 staged cache transaction；legacy path 归一化改为 guarded 单列更新。 | `0a8a0ef` 已使陈旧结果返回安全 409 且零 failure/cache/variable 副作用，取消不合成响应；focused/race/full/vet 与四视口真实浏览器通过。状态 `aligned / regression-validated / GitHub-sync-and-Docker-evidence-pending`。既有 parser、缓存 rooted 文件系统、整本缓存和换源可见合同保持关闭。 |
...
## 当前整体进度快照（2026-09-09，Reader chapter-content lifecycle implemented）
...
实施并完成本地全量及四视口验证，GitHub 同步与可信 Actions/Docker 证据因 443 超时待补；剩余约 1%
还包含后续逐路由 action 审计、长尾固定基准复审与真实设备证据，不能从 direct binder、`Save` 或
