...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`Index.vue#editBook` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookEditDialog.vue` | **刷新阅读进度 P1 已随 `ed4ee27` 发布；BookEdit P2 已随 `9614538` 发布**：完整网络快照继续按上游整批替换非 pending 进度；元数据编辑现只提交四个字段，保存前解析当前书架目标，编辑期间的新分组/追更不会被旧快照覆盖。前端 523/523、Go/build、真实并发 API 三视口和卷/备份门禁通过；BookGroup 与独立 `lastCheckTime` 合同继续有效。 | [`bookshelf-refresh-progress-p1-contract.md`](bookshelf-refresh-progress-p1-contract.md)、[`book-edit-metadata-p2-contract.md`](book-edit-metadata-p2-contract.md)；继续选择下一动作级数据事务。 |
