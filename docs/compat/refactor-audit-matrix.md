...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`WebDAV.vue` | `OverlayBookImport.vue`、`LocalStore.vue`、`WebDAVBrowser.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/services/localbook/*`、`engine/*_parser.go` | **P1-E1、P1-E2、P1-E3 已完成；P1-E4 审查完成**：真实格式/旧卷合同已确认两个必须重建差异：TXT 显式规则无匹配应为可恢复的空预览、规则目录长章节应按上游切分。PDF/Markdown 是仅直接上传的 OpenReader 扩展；CBZ 严格图片过滤是显式安全收紧。尚未开始夹具测试或实现。 | `docs/compat/workspace-storage-import-p1e-contract.md`、`docs/compat/workspace-storage-import-p1e3-contract.md`、`docs/compat/local-book-p1e4-contract.md` → 真实格式夹具、旧卷/Docker smoke。 |
