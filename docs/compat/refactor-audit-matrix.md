...
| 用户、备份、RSS、书签 | `UserManage.vue`、`AddUser.vue`、`BaseController.kt#saveUserSession`、`WebDAV.vue`、`Rss*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`backend/api/auth.go`/`admin.go`、`WebDAVBrowser.vue`、`RSSManager.vue`、`components/rss/*`、`services/rss`、`OverlayBookmarks.vue` | Bookmark、Overlay 会话隔离、P2-S4 owner 隔离和 RSS 第二轮保持已发布。**UserManage 权限更新第二轮复审发现必须修复项**：上游单字段开关被改写成前端五字段 payload + 后端整行 GORM `Save`，可覆盖并发登录时间或密码重置；nullable WebDAV 响应投影也不完整。创建、删除、书源动作与可见表格不重开。 | [`user-management-partial-update-second-audit-p2-contract.md`](user-management-partial-update-second-audit-p2-contract.md) 状态 `extracted / implementation-pending`；下一门为红灯 API/前端测试、显式 SQL updates map、字段级 busy、三视口及新旧卷。RSS 继续见其 fixed-baseline 合同。 |
