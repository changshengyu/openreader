...
| 用户、备份、RSS、书签 | `UserManage.vue`、`AddUser.vue`、`BaseController.kt#saveUserSession`、`WebDAV.vue`、`Rss*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`backend/api/auth.go`/`admin.go`、`WebDAVBrowser.vue`、`RSSManager.vue`、`components/rss/*`、`services/rss`、`OverlayBookmarks.vue` | UserManage、Bookmark、Overlay 会话隔离和 P2-S4 owner 隔离保持已发布。**RSS 第二轮固定基准重建现已实现并完成回归，Docker 待发布**：恢复 500px/fullscreen sibling dialogs、`RSS订阅(N)` 四列源网格、JSON editor、零默认选择的安全勾选导入、title/date/image-only 文章列表及 content-only 正文；旧富卡片、结构化 editor、刷新/筛选/收藏/摘要/作者/footer 已删除。 | [`rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md`](rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md) 状态 `implemented / regression-validated / Docker-pending`；frontend 706/706、Go/build/diff、RSS 四视口、工作台三视口和 RSS 跨账号 pending-write 三视口通过。JWT/SQLite cache、HTML 清洗、事务/会话隔离为允许适配；共享抓取器的响应体/重定向/私网硬化仍单独开放。Bookmark 证据仍见 [`bookmark-fixed-baseline-second-audit-p2-contract.md`](bookmark-fixed-baseline-second-audit-p2-contract.md)。 |
