...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`Index.vue#editBook`、`Index.vue#deleteBook`、`BookManage.vue#deleteBookList` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookEditDialog.vue`、`stores/bookshelf.js`、`useSync.js` | **刷新阅读进度 P1 已随 `ed4ee27` 发布；BookEdit P2 已随 `9614538` 发布；删除消费者收敛 P1 合同已提取、尚未实施**：后端事务/文件清理已对齐并强化，但 WebSocket 删除目前只移除书架行，不清本地进度、不关闭目标书弹层，也不会让另一客户端的活动 Reader 停止保存并返回书架。 | [`bookshelf-refresh-progress-p1-contract.md`](bookshelf-refresh-progress-p1-contract.md)、[`book-edit-metadata-p2-contract.md`](book-edit-metadata-p2-contract.md)、[`book-deletion-consumer-convergence-p1-contract.md`](book-deletion-consumer-convergence-p1-contract.md)；下一步先补 direct/batch/sync、overlay、Reader 失败测试。 |
