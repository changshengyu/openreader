...
| 书源与搜索/探索结果 | `Index.vue`、`Explore.vue`、`BookSource.vue`、`BookSourceController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`RemoteBookJsonEditorDialog.vue`、`ExploreWorkspacePopover.vue`、presentation/composable、Go source APIs | **书源 owner/API/解析主链保持已发布；搜索/探索可见工作区第二轮已作为 `c851c5f` 发布**。可见搜索现只保留 single/multi 并按场景显示控件；结果保持服务端顺序并与普通书架共用固定 380px grid/card；共享 JSON editor/add 双动作已恢复；Explore 恢复 desktop 600px/top0 和 mini 100vw 非全高、无 backdrop、多开并保留现场。P2-N1 共享 fetcher 已发布；P2-N2 唯一管理员 allowlist、mixed-DNS/dial pinning 和 HTTP/SOCKS endpoint/target 已测试先行实现。 | 可见工作区保持 `aligned`。[`shared-source-fetcher-p2-contract.md`](shared-source-fetcher-p2-contract.md) 状态为 `P2-N1 Docker-published / P2-N2 implementation-validated / Docker-pending`；Docker LAN/public 门未过前不能宣称共享 SSRF 已整体关闭。 |
