...
| 书源与搜索结果 | `Index.vue`、`Explore.vue`、`BookSource.vue`、`BookSourceController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`SourceSwitchPanel.vue`、`backend/api/sources.go` | **P2-S1 至 P2-S4 应用实现、自动回归与双账号三视口已完成**：旧全局表已增加用户关联/namespace 和非破坏迁移；管理/调试、搜索、探索、远程书、Reader、scheduler、管理员动作及备份恢复均按用户解析，并保留既有书籍的 detached 快照。2026-07-28 发布证据复审确认通用历史 fixture 没有旧全局远程书源且已预写迁移 marker，因此此前通用 Docker smoke 不能证明 ownership 升级。 | [`book-source-ownership-p2-contract.md`](book-source-ownership-p2-contract.md)；补专门的旧全局源迁移、COW、管理员旧根/普通用户私有根、logical/portable 恢复与重启 Docker 门。通过前状态仍为 `Docker gate pending`。 |
...
| 用户、备份、RSS、替换规则、书签 | `UserManage.vue`、`WebDAV.vue`、`Rss*`、`ReplaceRule*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`WebDAVBrowser.vue`、`RSSManager.vue`、`OverlayReplaceRules.vue`、`OverlayBookmarks.vue`、Go API/services | RSS、替换规则和书签合同继续成立；**P2-S4 已纠正 logical/portable/WebDAV 恢复的全局书源读写并通过自动与真实浏览器回归**。剩余风险不是应用实现，而是旧全局源卷从无 ownership schema 升级后的真实 Docker 数据流尚无 fixture 证明。 | [`book-source-ownership-p2-contract.md`](book-source-ownership-p2-contract.md)、[`rss-source-lifecycle-p2-contract.md`](rss-source-lifecycle-p2-contract.md)、[`user-management-p2-contract.md`](user-management-p2-contract.md)、[`backup-restore-fixed-baseline-p2-contract.md`](backup-restore-fixed-baseline-p2-contract.md)；先关闭专项 Docker 升级/恢复门，再选择下一 P2 模块。 |
