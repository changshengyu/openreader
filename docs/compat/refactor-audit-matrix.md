...
| 本地导入、书仓、WebDAV | `Index.vue#importBookPreview/importMultiBooks`、`BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/api/direct_local_import_boundary.go`、`imports.go`、`localstore*`、`webdav*`、`services/localbook/*`、`engine/*_parser.go` | P1-E1…P1-E5、P2 parser 预算、TXT 默认规则、mounted LocalStore/WebDAV 和直接多选导入保持已发布。新 route/`Save` scan 发现本地单书 refresh 在 opened read/最长 1 GiB legacy parse/stage 后仍用旧整行 Book 提交，caller 取消后继续，并可复活并发删除或覆盖编辑。 | [`local-book-refresh-request-lifecycle-fixed-baseline-second-audit-p2-contract.md`](local-book-refresh-request-lifecycle-fixed-baseline-second-audit-p2-contract.md) 已固定 request context、post-stage owner/snapshot 复验、owned-field update 和 stale/cancel stage 清理；状态 `inventory-complete / tests-and-implementation-pending`。已签收 parser/rooted archive/BookInfo UI 不重开。 |
...
## 当前整体进度快照（2026-08-27，本地书 refresh lifecycle inventory 已完成）
...
入口/分页/取消生命周期和远程 BookInfo/TOC handler 取消/陈旧提交亦已关闭。剩余约 1% 当前包含已确认
的本地书 refresh 取消/陈旧提交 must-fix、后续逐路由 action 审计、长尾固定基准复审与真实设备证据，
不能从 direct binder 或 contextless 调用差集缩小推导完成。
...
## 2026-08-27 本地书 Refresh 请求生命周期固定基准复审
...
当前状态 **inventory-complete / tests-and-implementation-pending**；整体比例仍为 99%。
