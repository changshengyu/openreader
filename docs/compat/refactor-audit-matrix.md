...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 持续复核；移动点击翻页第三批代码已完成，等待 Docker 人工体感**：普通短点击不再启动选文重试；移动普通文本 page 在 touchstart 临时预热、所有非翻页分支释放；仅合成路径缩短缓入死区。分页步长、时长、终点、长按选文和原生 touch/wheel 保持。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；前端 487、Go/build、双移动输入窗口、desktop/mobile、真实 touch 选文及 continuous 合同通过。 |
