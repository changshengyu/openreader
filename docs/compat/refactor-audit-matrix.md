...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`LocalStore.vue`、`WebDAVBrowser.vue`、`LocalBookImportPreviewDialog.vue`、`backend/services/localbook/*`、`engine/*_parser.go` | **P1-E 已完成源码审查，发现必须重建**：后端暂存 token 已支持目录规则重试，但书仓/WebDAV 预览 UI 未把 token、失败行和重新解析接口接通；同时单本/多本导入状态机及额外目录操作偏离上游。 | `docs/compat/workspace-storage-import-p1e-contract.md` → token 重试失败测试 → P1-E1 → 真实文件夹/卷 smoke。 |
