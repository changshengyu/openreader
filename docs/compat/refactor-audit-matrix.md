...
| Pinia 状态、缓存、同步、数据事务 | `plugins/vuex.js`、`plugins/cache.js`、后端 controller/model | `stores/*.js`、`utils/*cache*`、`backend/models`、`services`、`sync` | **书架 P2 新鲜度/收敛切片已发布；认证运行时 scope 切片进入测试先行**：书架 network-first/revision/Hub/前台校准保持完成。新复审确认 Reader 设置、偏好、进度、分类和 profile 的迟到异步结果可跨用户提交；旧 WebSocket callback 可清除/关闭新实例并创建重复重连，均为 `must-fix`。 | 已完成 [`bookshelf-network-first-sync-p2-contract.md`](bookshelf-network-first-sync-p2-contract.md)；下一合同为 [`authenticated-runtime-scope-p2-contract.md`](authenticated-runtime-scope-p2-contract.md)，先写 operation scope 与 socket generation 失败测试。 |
