...
|---|---|---|---|---|
| Index 工作台、搜索、探索、侧边栏 | `web/src/views/Index.vue` | `layouts/AppLayout.vue`、`views/Home.vue`、`Search.vue`、`Discover.vue`、`stores/indexWorkspace.js` | **P1-A 已发布；P1-B 部分已发布**：根场景/旧链接收敛、三态结果、260/270 手势、24 并发和无书源语义已验证；续页入口、陈旧请求隔离、失效书源稳定 cursor、跨页去重与探索续页仍为 **must-fix**，全局操作继续逐个复审。 | `docs/compat/index-workspace-p1-contract.md`、`docs/compat/index-search-p1b-contract.md#7-后续续页与跨页结果复审2026-07-13仅合同尚未改动应用代码` → P1-B 测试 → 390/360/1440 浏览器流。 |
