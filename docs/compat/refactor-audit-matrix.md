...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`Index.vue#editBook`、`Index.vue#deleteBook`、`BookManage.vue#deleteBookList` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookEditDialog.vue`、`stores/bookshelf.js`、`useSync.js` | **BookEdit P2 已随 `9614538` 发布；删除消费者收敛与刷新 pending/CAS 二次修复已完成三视口真实浏览器验证**：direct/batch/sync 现汇入唯一幂等删除事务；keepalive 成功响应确认 pending，首页和 Reader 内刷新等待一次 CAS 最终赢家，无 reload 且活动正文不跳章。 | [`bookshelf-refresh-progress-p1-contract.md`](bookshelf-refresh-progress-p1-contract.md)、[`book-edit-metadata-p2-contract.md`](book-edit-metadata-p2-contract.md)、[`book-deletion-consumer-convergence-p1-contract.md`](book-deletion-consumer-convergence-p1-contract.md)；本聚合切片待本地 Docker 发布。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **iPad 关闭路径与第七批连续点击已发布；第八批点击视觉节奏已完成浏览器验证**：竖向 `page/scroll/scroll2` 采用有界非零初速、零末速的连续 Hermite 曲线，首刷新位移不超过页面步长 4%；双手机向上/向下点击、连续双击、Long Task、LayoutShift 和段落几何均通过，未观测到正文重排。 | [`reader-ipad-panel-dismiss-p0-contract.md`](reader-ipad-panel-dismiss-p0-contract.md)、[`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；待本批 Docker 发布后由用户真机复验体感。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `9614538` 已验证**；其后的删除收敛、书架刷新 pending/CAS 与移动点击速度曲线改动已完成前端 537/537、Go 全量、生产构建及相应三视口真实浏览器门禁，正在进入本地 Docker 发布闸门。 | `openreader-regression`、`openreader-docker-release`；发布完成后用新提交标签和 OCI digest 替换当前 `9614538` 记录。 |
