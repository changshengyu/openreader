...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`views/LocalStore.vue`、`OverlayLocalStore.vue`、`WebDAVBrowser.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/services/localbook/*`、`engine/*_parser.go` | **P1-E1…P1-E4 的用户可见流程、格式运行时、旧卷、portable backup 和用户隔离均已完成并发布。P1-E5 文件级复审发现结构性尾项**：`/local-store` 已正确重定向到 Index overlay，但 `views/LocalStore.vue` 仍保留不可达独立页面和 `embedded` 双壳；必须迁移为唯一 workspace body，且不得改动既有文件/导入行为。 | [`local-store-workspace-ownership-p1-contract.md`](local-store-workspace-ownership-p1-contract.md)、[`epub-fixed-baseline-catalog-reader-contract.md`](epub-fixed-baseline-catalog-reader-contract.md)、[`reader-cbz-fixed-baseline-p0-contract.md`](reader-cbz-fixed-baseline-p0-contract.md)。 |
