...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。books.go 六个 JSON control、ReplaceRule、备份 generation/list/download、公开 upload read、远程章节文本缓存和公开 capability 文件读取生命周期均已发布。下一项真实 must-fix 已收敛到本地书 archive 的 source/cache 读取、refresh stage/promote、生成式导出回退和最后引用删除：`20ba211` 的 owner-root symlink 可把读写删带到 `LibraryDir` 外。 | [`local-book-archive-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md`](local-book-archive-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md) 状态 `inventory-complete / implementation-pending`；真实 `/tmp` HTTP 探针已复现 refresh/read 200、库外 generation/metadata 写入及 batch delete 库外目录删除。上一公开 capability 切片仍为 `5e63eb1` published，不因本轮重开。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | `5e63eb1` 通过 frontend 741/741、Go full/focused race/vet、build、EPUB/CBZ/audio/真实 cover 三视口、宿主/候选 mounted probe 与远端回拉健康检查；fresh/historical/portable 覆盖 TXT/EPUB/UMD/CBZ、relative-cache、owner isolation、restart 与 portable v1/v2 assets。 | 本机 amd64/arm64 发布 `5e63eb1`/`latest`，OCI index `sha256:8b7bc4cd8542f79eccc54d393cf2d79041f5fe9a90b05776c473cd3f1e4c2cee`；远端两平台 config 已确认完整 revision。用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-08-24，`5e63eb1` 公开 capability 文件读取生命周期已发布）
...
- **尚未完成的主线**：其余尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  ...
  实现 `a90f7b3`、完整回归/卷门和 `5e63eb1` Docker 发布；下一 action 长尾已由真实探针收敛为
  本地书 archive rooted filesystem lifecycle，合同状态 `inventory-complete / implementation-pending`，
  其它 action 继续逐项取证。
  `c74be70` 已发布但尚待
  服务器部署；移动书架在
