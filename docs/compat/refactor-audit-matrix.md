...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置和夜间面此前均已专项发布；**Reader 移动端阅读内书架宽度偏差已修复并重新包含在 `77a60d8`/`latest`**：主面板为 100vw，最终左右 inset 从错误的 24px 恢复为上游 20px，390/360 列表为 350/320px；普通首页书架保持 390/360px 整行宽度。本轮完整 Reader 与普通书架浏览器合同再次通过。线上 `/api/health` 仍为 `7971e23`，判定为部署滞后。 | [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md) 状态 `aligned / Docker-republished / deployment-update-required`；当前需将线上容器真正 pull 并 force recreate 到 `77a60d8`，仅 restart 不会替换旧镜像。其它已关闭 Reader 专项不因部署漂移自动重开。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 UserManage 局部更新、Reader 移动书架及此前全部已发布能力已由本机作为 `77a60d8` 发布**：focused/full/race/frontend 707/707/build/vet/diff、UserManage 四视口、Reader/书架多视口及 amd64/arm64 推送成功。 | `77a60d8`/`latest` OCI index `sha256:a1a37b223e10a3c43febd23250dd7790394c200d69e7c9548255cf1fdba3b017`。fresh volume 的 portable v1/v2 assets、cross-user、restart 和 historical TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过。 |

## 当前整体进度快照（2026-08-09，`77a60d8`）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **89%**。UserManage 局部更新关闭的是已覆盖
模块内的并发正确性缺口，因此本轮提升质量但不虚增一个百分点。该数字表示“已完成固定基准复审、
...
- **尚未完成的主线**：尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  ...
  `77a60d8` 后再验收 Reader 内书架。上述项目继续按“合同→失败测试→实现→浏览器/新旧卷→本地
  Docker”推进。
