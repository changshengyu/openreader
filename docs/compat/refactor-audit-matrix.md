...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public `/uploads/*` | **按动作逐项复审；已关闭模块不从旧日志重开**。books.go 六个 JSON control、ReplaceRule、备份 generation/list/download 和公开 upload read 均已发布；`backend/api` direct Gin JSON binder 差集为空。远程章节文本缓存读/写/stats/clear/prune 的共享文件系统生命周期已由 `75cc238` 实现。 | [`remote-chapter-cache-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md`](remote-chapter-cache-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md) 状态 `implemented / code-regression-validated / runtime-and-release-pending`；rooted 同句柄有界读取、原子可取消写入、实际文件统计、全用户引用 fail-closed 和 write/prune 串行边界已通过专项/full/race/vet。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | `277e512` 通过 frontend 741/741、Go full/race/vet、build、Reader 资产三视口、候选与 GHCR 回拉公开读取边界；fresh/historical/portable 覆盖 TXT/EPUB/UMD/CBZ、relative-cache、owner isolation、restart 与 portable v1/v2。 | 本机 amd64/arm64 发布 `277e512`/`latest`，OCI index `sha256:ca50fd59dce4f4bb13a1450ee7ee39b2a3d7b392de3902a7f3c21272e8ac9c70`；远端两平台 config 已确认完整 revision。用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-08-17，`277e512` 公开 upload read 边界已发布）
...
- **尚未完成的主线**：其余尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  ...
  生命周期已完成合同 `c6f9de8`、旧实现红测 `f8e5c04` 和实现 `75cc238`，下一步是真实浏览器、
  mounted runtime、卷门和本地 Docker 发布；其它 action 长尾继续逐项取证。
  `c74be70` 已发布但尚待
  服务器部署；移动书架在
