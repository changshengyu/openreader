...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 第五次实机反馈修复已发布 `895d536`**：移动竖向点击分页消除首个 rAF 零位移，改用非零起步/平滑收尾曲线；`page/scroll/scroll2` 统一，缓冲双页只在最终页做一次重型结算。分页步长、配置总时长、原生 touch/wheel、选文、工具层和章末入口保持。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；前端 489、Go/build、三模式双移动逐帧、desktop/mobile、continuous、image 和 Docker 卷门禁通过，等待用户手机体感复验。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `895d536` 已验证**：前端 489/489、Go 全量、生产构建、Reader 三竖向模式双移动逐帧、文本/移动/连续/图片、历史 TXT/EPUB/UMD/CBZ/相对缓存 volume/backup、远端 amd64/arm64 manifest 均通过；以后每次发布仍必须以新提交重跑。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:155fd51087400d0831ee840661e6391a8bfd70ba2b908858057c3c05397d2804`。 |
