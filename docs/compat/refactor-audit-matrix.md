...
| 书源解析、RSS、远程抓取 | `AnalyzeRule*`、`Rss*`、`BookSourceController.kt` | `backend/engine/source_*.go`、`rss_parser.go`、fetcher、`services/rss` | **CSS/JSONPath/XPath 书源主链保持已完成；RSS 请求页语义已实现并回归**。可见 flow 的 page 1/2 各自只执行请求页；标准 feed 的 page>1 不再发网；任意外部 sort URL 在 fetch 前拒绝；页面缓存事务保留 parser order 与隐藏状态。旧 eager helper 只为隐藏兼容测试保留，不再由 endpoint 调用。 | RSS 页级证据见 [`rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md`](rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md)。本批新增 page/import/owner/rollback 门并保留 12 秒 timeout、rate/context 与清洗；共享 source fetcher 的统一响应字节上限、显式 redirect 上限和私网 SSRF 策略仍为独立 `must-fix`，不得误报为已关闭。书源证据仍见 [`online-booksource-parser.md`](online-booksource-parser.md) 与 [`booksource-metadata-normalization-p2-contract.md`](booksource-metadata-normalization-p2-contract.md)。 |
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 RSS 固定基准切片已由本机作为 `92b7034` 发布**：frontend 706/706、Go、build/diff、RSS 四视口、工作台旧链接三视口、RSS session-isolation 三视口通过；amd64/arm64 推送成功。 | `92b7034`/`latest` OCI index `sha256:095540fe28c553ab6d2cfd9ce589ec0b31809f00600cd9f59ab814b7753098ba`。本地候选的新卷 portable v1/v2 assets、cross-user、restart 和历史 TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过。 |
