...
| 书源与搜索/探索结果 | `Index.vue#toDetail`、`Reader.vue#getCatalog`、`App.vue#getBookContent`、`Explore.vue`、`BookSource.vue`、`BookController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`RemoteBookJsonEditorDialog.vue`、`ExploreWorkspacePopover.vue`、`backend/api/remote_reader.go`、`services/remotereader`、Go source APIs | **书源 owner/API/解析主链和搜索/探索可见工作区保持已发布；临时 Reader 第二轮已测试先行修复**。64 KiB 唯一 JSON、30m/4h lease、8 MiB/session、8+32 MiB/user、128+128 MiB/process LRU、自然到期 410 标记、非法 index 不续期、跨章变量、取消回滚、typed failure 与错误脱敏均已实现。 | [`remote-reader-session-fixed-baseline-second-audit-p1-contract.md`](remote-reader-session-fixed-baseline-second-audit-p1-contract.md) 状态 `implemented / regression-validated / Docker-pending`；Go full/race/vet、frontend 713/713/build、三视口 mock 及真实 Go CSS/JSONPath/XPath 链路通过。共享 fetcher 继续按 [`shared-source-fetcher-p2-contract.md`](shared-source-fetcher-p2-contract.md) 已发布合同执行。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前移动阅读内书架行轨修复及此前全部能力已由本机作为 `a445121` 发布**：Go full、frontend 713/713、build/diff、完整 Reader 与普通书架多视口、本机双架构构建和 GHCR 推送成功。 | `a445121`/`latest` OCI index `sha256:79c56060dc0101f0d5bc07f09ae4cb02b7b1a5618681eb08c13bd2fcbe7f6238`。fresh volume 的 portable v1/v2 assets、cross-user、restart 和 historical TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过。 |

## 当前整体进度快照（2026-08-09，`a445121`）
...
- **尚未完成的主线**：尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  以及后续真实设备反馈暴露出的上游可见偏差。`a445121` 已同时包含手动书架刷新和阅读内书架行轨
  修复，但服务器仍需 pull/force recreate 才能验收；移动书架等待真实设备签收。上述项目继续按“合同→失败测试→实现→浏览器/
  新旧卷→本地 Docker”推进。
