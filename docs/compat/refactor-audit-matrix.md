...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。阅读进度、外部 WebDAV、远程封面等已发布；`/ws/sync` 的 same-origin/active-user/server-only/recipient-scope 已实施验证。2026-08-11 又补齐六个精确路径：登录/注册与书签批删为技术栈/身份安全等价，旧 Category reorder 只保留兼容层；TXT 规则与 Reader 换源候选确认存在结构偏差并进入实现队列。 | 既有专项合同继续有效；新增 [`rest-action-coverage-fixed-baseline-p2-contract.md`](rest-action-coverage-fixed-baseline-p2-contract.md) 与 [`reader-source-switch-fixed-baseline-second-audit-p0-contract.md`](reader-source-switch-fixed-baseline-second-audit-p0-contract.md)。TXT/换源当前状态 `inventory-complete / implementation-pending`，不能以现有测试推定完成。 |
...
2026-08-11 长尾动作复审已将“尚未逐动作签约”进一步收敛：本轮六个精确路径已有正式合同，其中
TXT 默认目录规则缺 8 条且手动列表错误过滤禁用项，Reader 换源缺少每书候选缓存并在打开时错误
联网宽搜，均为已证实 `must-fix`。整体百分比暂不增加，须待失败测试、实现和发布门完成后再计入。
