...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`Index.vue#editBook` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookEditDialog.vue` | **刷新阅读进度 P1 已随 `ed4ee27` 发布**：完整网络快照现按上游整批替换非 pending 进度，服务器空值清除对应客户端记录；只有真实 `pendingSync` 暂存并 CAS 重试。前端 520/520、Go/build、断开 WebSocket + 2099 年旧缓存的三视口浏览器及新旧卷/备份门禁通过。BookGroup 与独立 `lastCheckTime` 合同继续有效。**新抽取的 BookEdit P2 缺口**：元数据保存仍附带旧分组/追更快照，可能覆盖并发更新；专项合同已完成，待测试先行实施。 | [`bookshelf-refresh-progress-p1-contract.md`](bookshelf-refresh-progress-p1-contract.md)、[`book-edit-metadata-p2-contract.md`](book-edit-metadata-p2-contract.md)；先关闭 BookEdit 精确字段与同步事务，再选择下一动作。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `ed4ee27` 已验证**：前端 520/520、Go 全量、生产构建、书架进度三视口专项 Chromium，以及普通和历史 TXT/EPUB/UMD/CBZ/相对缓存 volume/backup 均通过；镜像在本机完成双架构构建并上传。此前书架时间、Reader/WebDAV 门禁继续有效。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:f369cc6610312987e068dcdd015887e27569a9dcbe6c048525de12bb5ab95d89`，amd64 `sha256:a98655524d6f55680ecf14766c8f56b363f615b37f8865a43e897b9c3904e4ee`，arm64 `sha256:a6348c2ef9ae56caf6769fb0a593f9234db13a735c89ff78665cef1a3cb72166`。 |
