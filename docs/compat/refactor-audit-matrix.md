...
| 书架、BookManage、BookGroup、BookInfo | `Index.vue` 普通 shelf、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、stores | **BookManage、BookGroup、BookInfo 第二轮已分别发布；普通书架 `c851c5f` 后的 28px 移动宽度回归已修复并作为 `7971e23` 发布**。固定 380px grid、当前筛选计数、精确编辑搜索、历史 list→grid v3 迁移、独立 metadata DOM、loading/blank/night 保持；书架场景现明确覆盖通用 app-page 左右各 14px，390/360px 视口重新得到同宽书籍行。书架 freshness、进度和 lastCheckTime 专项继续保持关闭。Reader 内书架是独立 P0 合同，其每侧多缩进 4px 的偏差已作为 `5151609` 发布。 | [`bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md`](bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md) 保持 `aligned`；Reader 内书架见 [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md)，状态 `aligned / Docker-published / awaiting-device-verification`。Index 仍实测保持 390/360px 行宽。 |
| 书源与搜索/探索结果 | `Index.vue`、`Explore.vue`、`BookSource.vue`、`BookSourceController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`RemoteBookJsonEditorDialog.vue`、`ExploreWorkspacePopover.vue`、presentation/composable、Go source APIs | **书源 owner/API/解析主链保持已发布；搜索/探索可见工作区第二轮已作为 `c851c5f` 发布**。可见搜索现只保留 single/multi 并按场景显示控件；结果保持服务端顺序并与普通书架共用固定 380px grid/card；共享 JSON editor/add 双动作已恢复；Explore 恢复 desktop 600px/top0 和 mini 100vw 非全高、无 backdrop、多开并保留现场。P2-N1 共享 fetcher 的 body/redirect/scheme/retry/timeout/redaction 边界已经实现并作为 `981bca7` 发布；P2-N2 已固定唯一管理员 allowlist、mixed-DNS/dial pinning 和 HTTP/SOCKS endpoint/target 合同，尚未进入测试与实现。 | 可见工作区保持 `aligned`。[`shared-source-fetcher-p2-contract.md`](shared-source-fetcher-p2-contract.md) 状态为 `P2-N1 Docker-published / P2-N2 contract-locked`；private-network/DNS/proxy allowlist 仍未签收，不能宣称共享 SSRF 已整体关闭。 |
...
| 用户、备份、RSS、书签 | `UserManage.vue`、`AddUser.vue`、`BaseController.kt#saveUserSession`、`WebDAV.vue`、`Rss*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`backend/api/auth.go`/`admin.go`、`WebDAVBrowser.vue`、`RSSManager.vue`、`components/rss/*`、`services/rss`、`OverlayBookmarks.vue` | UserManage、Bookmark、Overlay 会话隔离和 P2-S4 owner 隔离保持已发布。**RSS 第二轮固定基准重建已作为 `92b7034` 发布**：恢复 500px/fullscreen sibling dialogs、`RSS订阅(N)` 四列源网格、JSON editor、零默认选择的安全勾选导入、title/date/image-only 文章列表及 content-only 正文；旧富卡片、结构化 editor、刷新/筛选/收藏/摘要/作者/footer 已删除。 | [`rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md`](rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md) 状态 `aligned / Docker-published / awaiting-device-verification`；frontend 706/706、Go/build/diff、RSS 四视口、工作台三视口、RSS 跨账号三视口和新旧卷门通过。`92b7034`/`latest` index 为 `sha256:095540fe28c553ab6d2cfd9ce589ec0b31809f00600cd9f59ab814b7753098ba`。JWT/SQLite cache、HTML 清洗、事务/会话隔离为允许适配；共享抓取器硬化仍单独开放。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置和夜间面此前均已专项发布；**本轮 Reader 移动端阅读内书架宽度偏差已修复并作为 `5151609` 发布**：主面板仍为 100vw，最终左右 inset 从错误的 24px 恢复为上游 20px，390/360 列表为 350/320px；1024 强制手机模式保持上游四列。 | [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md) 状态 `aligned / Docker-published / awaiting-device-verification`；Reader 桌面/双手机/自适应与强制 iPad、Index 书架四视口、frontend 706/706、Go/build、新旧卷通过。其它已关闭 Reader 专项不因本偏差自动重开。 |
...
| 书源解析、RSS、远程抓取 | `AnalyzeRule*`、`Rss*`、`BookSourceController.kt` | `backend/engine/source_*.go`、`rss_parser.go`、fetcher、`services/rss` | **CSS/JSONPath/XPath 书源主链和 RSS 请求页语义已实现；P2-N1 共享抓取边界已作为 `981bca7` 发布**。可见 page 1/2 只执行请求页；标准 feed 的 page>1 不发网；owned sort allowlist、事务/parser order/隐藏状态保持。共享 fetcher 现有 15 秒 timeout、16 MiB body、5 redirect、3 retry、HTTP(S)/userinfo 和跨 origin credential 边界。P2-N2 inventory 已固定 `OPENREADER_SOURCE_NETWORK_ALLOWLIST`、默认公网-only、DNS/dial 与代理固定策略。 | [`shared-source-fetcher-p2-contract.md`](shared-source-fetcher-p2-contract.md)：N1 全量/race/browser/new+historical volume 通过，OCI index `sha256:02160e0797b3371fdfadccb550b8766d412c3e09df632ba1e36d192b26eb500d`。P2-N2 状态为 `contract-locked / implementation-pending`，仍是独立 `must-fix`。 |
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 Reader 移动书架宽度切片已由本机作为 `5151609` 发布**：frontend 706/706、Go/build/diff、Reader 全矩阵、Index 书架四视口及 amd64/arm64 推送成功。 | `5151609`/`latest` OCI index `sha256:d3110429a422e092832afde3b7780d6a3c193c01316c5e251c7c6ba8cd85f23c`。fresh volume 的 portable v1/v2 assets、cross-user、restart 和 historical TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过。 |

## 当前整体进度快照（2026-08-09，`5151609`）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **85%**。该数字表示“已完成固定基准复审、
测试先行实现并至少发布过一次”的覆盖度，不表示剩余 15% 只是简单收尾。
...
- **P2 已覆盖的高风险项**：账号/overlay/cache 隔离、原子阅读进度、删除收敛、替换规则、书签、
  ...
  capability，以及共享抓取器 P2-N1 请求边界。
- **尚未完成的主线**：共享抓取器 P2-N2（私网、DNS rebinding、proxy 与管理员 allowlist）、更广的
  parser/upload 解压与资源上限、尚未逐动作签约的 Go REST/错误/事务语义，以及后续真实设备反馈
  暴露出的上游可见偏差。上述项目必须继续按“合同→失败测试→实现→浏览器/新旧卷→本地 Docker”推进。
