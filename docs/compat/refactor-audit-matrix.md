...
| Pinia 状态、缓存、同步、数据事务 | `plugins/vuex.js`、`plugins/cache.js`、后端 controller/model | `stores/*.js`、`utils/*cache*`、`backend/models`、`services`、`sync` | **书架、认证 scope 与阅读进度 P2 已实现，进度发布门禁中**：进度现由规范章节 + 数据库原子 CAS 决定唯一赢家，后台退出不重复请求；远端进度恢复不再经路由 watcher 回声保存。真实三视口双客户端已收敛并冷恢复；全量与格式浏览器门禁通过。 | [`reading-progress-p2-contract.md`](reading-progress-p2-contract.md)；只剩历史卷/备份与本地 Docker 门禁，通过后发布并记录摘要。 |
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/auth.go`、前端 `api/*.js` | **按动作逐项复审；阅读进度切片已实施**：GET/PUT 稳定路径与兼容冲突响应保留，服务端规范化章节身份、CAS 只广播赢家，并在现存用户 WebDAV 目录镜像。其它模块仍以专项 API 合同为证，不能用本行概括宣称全 REST 完成。 | [`reading-progress-p2-contract.md`](reading-progress-p2-contract.md)；当前只剩全量/卷/Docker 发布门禁，随后选择下一未审查 API 组。 |
