...
| 书源与搜索/探索结果 | `Index.vue#toDetail`、`Index.vue#debugBookSource`、`bookSourceDebug/*`、`Debugger.kt`、`Reader.vue#getCatalog`、`App.vue#getBookContent`、`Explore.vue`、`BookSource.vue`、`BookController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`RemoteBookJsonEditorDialog.vue`、`ExploreWorkspacePopover.vue`、`backend/api/source_debug.go`、`remote_reader.go`、`services/remotereader`、Go source APIs | 书源 owner/API/解析主链、搜索/探索工作区和临时 Reader 第二轮均保持已发布。**书源调试第二轮已重新开放**：当前三标签 Dialog 不是上游独立编辑器，也没有 save-before-debug、单流自动搜索/发现→详情→目录→正文、变量/下一章传递和阶段日志；调试远程失败还会错误写入失效源缓存。裁决为 `must-fix`，不沿用当前 Dialog 骨架。 | [`source-debug-fixed-baseline-second-audit-p2-contract.md`](source-debug-fixed-baseline-second-audit-p2-contract.md) 状态 `extracted / tests-pending / implementation-pending`；临时 Reader 继续由 [`remote-reader-session-fixed-baseline-second-audit-p1-contract.md`](remote-reader-session-fixed-baseline-second-audit-p1-contract.md) 约束并已发布 `30dbe53`。共享 fetcher 安全边界继续适用。 |
