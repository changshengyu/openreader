...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置和夜间面此前均已专项发布；Reader 移动端阅读内书架的自动化合同为主面板 100vw、最终左右 20px，390/360 列表 350/320px；普通首页书架为 390/360px 整行。线上现已运行含该修复的 `77a60d8`，但设备仍反馈偏窄，不能再以部署滞后关闭。 | [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md) 状态改为 `automated-upstream-geometry-aligned / device-feedback-open`；下一轮必须基于实际设备截图区分 Index 与 Reader shelf 并逐层取证，不能让当前测试替设备偏差背书。`43635a1`/`latest` 继续包含当前合同。 |
