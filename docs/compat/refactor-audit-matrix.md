...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`LocalStore.vue`、`WebDAVBrowser.vue`、`LocalBookImportPreviewDialog.vue`、`backend/services/localbook/*`、`engine/*_parser.go` | **P1-E1 已完成**：书仓/WebDAV 的失败预览可用同一暂存 token、已编辑规则重试并刷新目录；不会重新读取可变的挂载源文件。单本/多本上游确认状态机、未获授权的额外目录操作和完整格式/旧卷复验仍必须重建。 | `docs/compat/workspace-storage-import-p1e-contract.md` → P1-E2 导入状态机 → P1-E3 额外流程审查 → 真实文件夹/卷/Docker smoke。 |
