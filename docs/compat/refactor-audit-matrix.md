...
| 用户、备份、RSS、书签 | `UserManage.vue`、`AddUser.vue`、`BaseController.kt#saveUserSession`、`WebDAV.vue`、`Rss*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`backend/api/auth.go`/`admin.go`、`WebDAVBrowser.vue`、`RSSManager.vue`、`components/rss/*`、`services/rss`、`OverlayBookmarks.vue` | Bookmark 可见流程、会话/owner 隔离和 RSS 可见第二轮保持已发布；**RSS 写入/缓存并发边界现为 inventory-complete / implementation-pending**：已取证 source/article body 无界或多 JSON、同 URL create/import 竞争、source/article 全行 `Save` 覆盖与删除后复活风险，必须先红测再实现每用户 source 事务和 article 列所有权。Bookmark 写入边界继续由 `a9a55db` 签收。 | [`rss-write-boundary-fixed-baseline-second-audit-p2-contract.md`](rss-write-boundary-fixed-baseline-second-audit-p2-contract.md) 是下一 must-fix；本合同阶段未修改应用/测试。已发布 RSS UI/requested-page 和 Bookmark 回归不得重开；完成后须跑 focused race/full、frontend/build、RSS 四视口、真实 HTTP/SQLite trigger、新旧卷及本机 Docker。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。公开 auth、管理员 user、用户配置、BookGroup/Category、Book、BookSource 与 Bookmark 写边界均已关闭并发布。当前下一项是 RSS source/article 写边界：补单 JSON/actual-read、同用户 URL 身份串行、显式列 SQL 和网络请求后 liveness recheck。 | RSS 写入合同当前 `inventory-complete / implementation-pending`；下一门是独立提交旧实现红测，再实施，不能用既有 RSS 可见绿测代替 wire/并发证据。 |
| 书源解析、RSS、远程抓取 | `AnalyzeRule*`、`Rss*`、`BookSourceController.kt` | `backend/engine/source_*.go`、`rss_parser.go`、fetcher、`services/rss` | **CSS/JSONPath/XPath 书源主链、RSS 可见请求页语义和 P2-N1/P2-N2 抓取边界均已发布**。本轮新发现只在持久提交：refresh 全行 `Save` 可覆盖并发 read/favourite，content cache 可覆盖 refresh/state 或复活删除行，fetch 期间删除 source 后仍可能写孤儿文章。 | 抓取预算/SSRF 合同不重开；[`rss-write-boundary-fixed-baseline-second-audit-p2-contract.md`](rss-write-boundary-fixed-baseline-second-audit-p2-contract.md) 要求 refresh 只拥有 remote 列、content 只拥有正文列，并在网络工作后复验 source/article 存活。 |
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **Bookmark 写入边界已在 `a9a55db` 完成完整回归和 Docker 门**：frontend 740/740、Go full/focused race/vet、build、Reader 多视口 payload、宿主/本地容器/GHCR 回拉容器真实 HTTP、SQLite trigger，以及 fresh portable-v1/v2/cross-user/restart 和 historical TXT/EPUB/UMD/CBZ/relative-cache/owner-isolation 均通过。 | 本机 amd64/arm64 已发布 `a9a55db` 与 `latest`，OCI index `sha256:944a85881170bc900c1fda0acb885bedc1dc4b17ed4e635305988163e1b635e5`；两平台 revision label 均为 `a9a55db16d490af61b31b5e1470ee477e2bba613`。用户生产环境当前运行提交仍未知，发布不等于服务器已升级。 |

## 当前整体进度快照（2026-08-12，`a9a55db` 发布签收 pass）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **95%**。该数字表示固定基准合同和测试先行
实现覆盖度；用户配置、BookGroup/Category、Book、BookSource 与 Bookmark 写入/导入边界均完成实现、
全量、运行时、新旧卷和正式 Docker 发布；剩余 5% 仍包含未逐动作签约的长尾，不是简单收尾。
...
- **P2 已覆盖的高风险项**：账号/overlay/cache 隔离、原子阅读进度、删除收敛、替换规则、书签、
  ...
  note-only/并发删除边界，以及 server-only WebSocket/recipient scope 和备份事务 worker。
