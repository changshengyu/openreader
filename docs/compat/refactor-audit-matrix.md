...
| 书源与搜索/探索结果 | `Index.vue#toDetail`、`Index.vue#debugBookSource`、`bookSourceDebug/*`、`Debugger.kt`、`Reader.vue#getCatalog`、`App.vue#getBookContent`、`Explore.vue`、`BookSource.vue`、`BookController.kt` | `SourceManager.vue`、`SourceDebug.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`ExploreWorkspacePopover.vue`、`backend/api/source_debug.go`、`services/sourcedebug`、`remote_reader.go`、`services/remotereader`、Go source APIs | 书源 owner/API/解析主链、搜索/探索工作区和临时 Reader 第二轮保持已发布。**书源调试第二轮已按固定基准重建并发布**：独立三列工作区、reader-dev JSON、九项命令、账号草稿/历史、save-before-debug、五入口单流自动链、variable/下一章边界、取消和脱敏日志已恢复；错误三探针 Dialog 已删除，旧探针只作无失败缓存副作用的兼容端点。 | [`source-debug-fixed-baseline-second-audit-p2-contract.md`](source-debug-fixed-baseline-second-audit-p2-contract.md) 状态 `aligned / Docker-published / awaiting-device-verification`；frontend 724/724、Go/full/race/vet/build/diff、四视口真实浏览器与新旧卷通过。`f8f263d` OCI index `sha256:9c83821de9e5f4df223b6e69a6d67eff512fa55d4a271f544718ccad8ae58ba1`。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **书源调试第二轮及此前全部能力已由本机作为 `f8f263d` 发布**：Go full/focused race/vet、frontend 724/724、build、书源调试四视口、完整 Reader 桌面/手机/iPad、本机双架构构建和 GHCR 回读成功。 | `f8f263d`/`latest` OCI index `sha256:9c83821de9e5f4df223b6e69a6d67eff512fa55d4a271f544718ccad8ae58ba1`。fresh volume 的 portable v1/v2 assets、cross-user、restart 和 historical TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过。 |

## 当前整体进度快照（2026-08-09，`f8f263d`）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **90%**。书源调试器此前被第二轮复审判定为
结构级错误，本批完成从合同、测试、实现到 Docker 的完整重建，因此关闭一个独立 P2 模块并提升一个百分点。该数字表示“已完成固定基准复审、
...
- **尚未完成的主线**：尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  以及后续真实设备反馈暴露出的上游可见偏差。`f8f263d` 已在线运行并包含阅读内书架行轨、临时 Reader
  第二轮与书源调试第二轮；移动书架在 390×844 线上真实账号复测中保持上游 390/350px 几何和内容高度行轨，
  但设备“明显窄”的反馈仍待截图区分具体界面；临时阅读和调试器继续等待真实设备签收。上述项目继续按“合同→失败测试→实现→浏览器/
  新旧卷→本地 Docker”推进。
