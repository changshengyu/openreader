...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十三次移动点击已发布 `49a273e`；第八轮夜间实际文字承载层修复已发布 `6fde5ab`**：黑色内容面按最终“不透明纯黑页面 + 无背景图”接管；普通正文 `p/span/mark` 与 EPUB 实际后代节点自身均显式纯黑/纯白，不再依赖祖先透明穿透，退出夜间完整恢复作者样式。非黑自定义主题与背景图保持不变。 | [`reader-night-contrast-p0-contract.md`](reader-night-contrast-p0-contract.md)：frontend 652/652、Go/build、普通 Reader 与真实 EPUB 1440×900/390×844/360×800 及新旧卷通过；`6fde5ab`/`latest` index 为 `sha256:4ace53462bc85e5f05e4441a5d250edd96f5b72d406b0ab109830501a129c4c0`。线上 `openreader.yuchsh.top` 仍需由部署端 pull/force-recreate 后才能从旧 `b78d39c` 升级。 |
