...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public `/uploads/*` | **按动作逐项复审；已关闭模块不从旧日志重开**。books.go 六个 JSON control、ReplaceRule、备份 generation/list/download 均已发布；`backend/api` direct Gin JSON binder 差集为空。公开 upload read 已按 `d0c948c` 合同与 `7181634` 红测完成显式 GET/HEAD、rooted same-file open、special-file 404 和标准 ServeContent 实现。 | [`upload-public-read-filesystem-boundary-fixed-baseline-second-audit-p2-contract.md`](upload-public-read-filesystem-boundary-fixed-baseline-second-audit-p2-contract.md) 状态 `aligned / regression-validated / Docker-pending`；真实 HTTP/浏览器与卷门待执行，upload write/delete、BookInfo/Reader/portable v2 不重开。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | `2986357` 通过 frontend 741/741、Go full/race/vet、build、真实 HTTP list/download 边界；fresh/historical/portable 覆盖 TXT/EPUB/UMD/CBZ、relative-cache、owner isolation、restart 与 portable v1/v2。 | 本机 amd64/arm64 发布 `2986357`/`latest`，OCI index `sha256:bdb8195077000a898569e0f3f6664a5760c2b56058d67b2d6ae1d4aaf42fea5e`；远端两平台 config 已确认完整 revision。用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-08-17，`2986357` 备份 list/download 边界已发布）
...
- **尚未完成的主线**：其余尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  ...
  ReplaceRule 五路、备份生成与 backup list/download rooted opened-file 边界均已完成合同、红测、实现、
  runtime、卷门和发布；公开 upload resource rooted opened-file 边界已完成合同、红测、实现与全量回归，
  尚待 runtime/卷门/发布，其它 action 长尾继续逐项取证。
  `c74be70` 已发布但尚待
  服务器部署；移动书架在
