...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置和夜间面此前均已专项发布；Reader 移动端阅读内书架横向仍为 100vw、左右 20px、390/360 列表 350/320px。**本轮真实账号复现出此前测试遗漏的多条目 Grid 行轨坍缩**：389 本书时条目/章节被压缩并互相覆盖；候选已恢复上游内容高度行轨、顶部对齐与 16px 行距。普通首页书架继续为 390/360px 整行。 | [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md) 当前为 `browser-validated / Docker-pending / awaiting-device-verification`；旧实现 390px 条目 25px 的失败证据已固定，修复后完整 Reader 与普通书架生产浏览器合同通过。发布后仍需真实设备最终签收。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前手动书架刷新及此前全部能力已由本机作为 `43635a1` 发布**：Go full/race/vet、frontend 713/713、build/diff、完整 Reader 多视口、本机双架构构建和 GHCR 推送成功。 | `43635a1`/`latest` OCI index `sha256:0f75a0434d209af901cde81f86127f8e62fa78d6cb3610d6c10ef2e0863053c0`。fresh volume 的 portable v1/v2 assets、cross-user、restart 和 historical TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过。 |

## 当前整体进度快照（2026-08-09，`43635a1`）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **89%**。手动书架刷新关闭的是已覆盖
模块内的行为、事务与并发正确性缺口，因此本轮提升质量但不虚增一个百分点。该数字表示“已完成固定基准复审、
...
- **尚未完成的主线**：尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  以及后续真实设备反馈暴露出的上游可见偏差。线上已运行 `77a60d8`，但仍需部署 `43635a1` 才能
  验收本轮手动刷新；Reader 内书架偏窄反馈保持打开。上述项目继续按“合同→失败测试→实现→浏览器/
  新旧卷→本地 Docker”推进。
