...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十三次移动点击已发布 `49a273e`；第四轮夜间实际文字层候选已完成实现与真实浏览器验证**：此前 `59e11a9` 只按非 custom 身份接管，漏掉可被设为黑夜默认的纯黑自定义方案；现在按最终“夜间 + 不透明纯黑页面 + 无背景图”统一接管普通正文和 EPUB 作者后代。 | [`reader-night-contrast-p0-contract.md`](reader-night-contrast-p0-contract.md)：frontend 644/644、Go/build、TXT 桌面/两手机/iPad 与真实 EPUB 1440/390/360 已通过；自定义图片和非黑自定义夜间继续保留。当前 Docker-pending，设备验收应等待新不可变标签。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前线上运行时为 `59e11a9`；第四轮纯黑自定义夜间修复已通过候选门**：frontend 644/644、Go、build、TXT 桌面/390/360/iPad 与真实 EPUB 1440/390/360 均通过，且背景图自定义夜间保持非接管。 | `openreader-regression`、`openreader-docker-release`；当前代码尚未构建发布新镜像。下一步本地 buildx、卷/备份兼容和 GHCR 发布后补录不可变标签与 digest。 |
