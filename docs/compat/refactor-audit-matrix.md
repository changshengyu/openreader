...
| 书架、BookManage、BookGroup、BookInfo | `Index.vue` 普通 shelf、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、stores | **BookManage、BookGroup、BookInfo 第二轮已分别发布；普通书架 `c851c5f` 后的 28px 移动宽度回归已修复并作为 `7971e23` 发布**。固定 380px grid、当前筛选计数、精确编辑搜索、历史 list→grid v3 迁移、独立 metadata DOM、loading/blank/night 保持；书架场景现明确覆盖通用 app-page 左右各 14px，390/360px 视口重新得到同宽书籍行。书架 freshness、进度和 lastCheckTime 专项继续保持关闭。Reader 内书架是独立 P0 合同，其每侧多缩进 4px 的偏差已作为 `5151609` 发布并继续包含在 `e7f168e`。 | [`bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md`](bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md) 保持 `aligned`；Reader 内书架见 [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md)，最后一次线上核验仍是 `7971e23`，需部署 `e7f168e` 后验收。Index 线上实测保持 390px 满宽。 |
| 书源与搜索/探索结果 | `Index.vue`、`Explore.vue`、`BookSource.vue`、`BookSourceController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`RemoteBookJsonEditorDialog.vue`、`ExploreWorkspacePopover.vue`、presentation/composable、Go source APIs | **书源 owner/API/解析主链保持已发布；搜索/探索可见工作区第二轮已作为 `c851c5f` 发布**。可见搜索现只保留 single/multi 并按场景显示控件；结果保持服务端顺序并与普通书架共用固定 380px grid/card；共享 JSON editor/add 双动作已恢复；Explore 恢复 desktop 600px/top0 和 mini 100vw 非全高、无 backdrop、多开并保留现场。P2-N1/P2-N2 共享 fetcher 均已发布；P2-N2 唯一管理员 allowlist、mixed-DNS/dial pinning、ambient proxy 隔离和 HTTP/SOCKS endpoint/target 已完成 Docker 网络与新旧卷门。 | 可见工作区保持 `aligned`。[`shared-source-fetcher-p2-contract.md`](shared-source-fetcher-p2-contract.md) 状态为 `aligned / Docker-published / awaiting-device-verification`；`d198c2e`/`latest` OCI index 为 `sha256:021817e602aa589c1583ec7ccb65828172c1a2afe1e038e23651dd51c455fcc1`。 |
...
| 用户、备份、RSS、书签 | `UserManage.vue`、`AddUser.vue`、`BaseController.kt#saveUserSession`、`WebDAV.vue`、`Rss*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`backend/api/auth.go`/`admin.go`、`WebDAVBrowser.vue`、`RSSManager.vue`、`components/rss/*`、`services/rss`、`OverlayBookmarks.vue` | UserManage、Bookmark、Overlay 会话隔离和 P2-S4 owner 隔离保持已发布。**RSS 第二轮固定基准重建已作为 `92b7034` 发布**：恢复 500px/fullscreen sibling dialogs、`RSS订阅(N)` 四列源网格、JSON editor、零默认选择的安全勾选导入、title/date/image-only 文章列表及 content-only 正文；旧富卡片、结构化 editor、刷新/筛选/收藏/摘要/作者/footer 已删除。 | [`rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md`](rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md) 状态 `aligned / Docker-published / awaiting-device-verification`；frontend 706/706、Go/build/diff、RSS 四视口、工作台三视口、RSS 跨账号三视口和新旧卷门通过。JWT/SQLite cache、HTML 清洗、事务/会话隔离为允许适配；共享抓取器 P2-N1/N2 已由 `d198c2e` 关闭本合同范围的请求边界。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置和夜间面此前均已专项发布；**Reader 移动端阅读内书架宽度偏差已修复并继续包含在 `e7f168e`/`latest`**：主面板仍为 100vw，最终左右 inset 从错误的 24px 恢复为上游 20px，390/360 列表为 350/320px；1024 强制手机模式保持上游四列。最后一次线上只读核验发现部署仍为 `7971e23`，390px 下仍呈现旧 24px/342px 几何，判定为部署滞后。 | [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md) 状态 `aligned / Docker-republished / deployment-update-required`；当前需将线上容器真正拉取并重建到 `e7f168e`，仅 restart 不会替换旧镜像。其它已关闭 Reader 专项不因部署漂移自动重开。 |
...
| Pinia 状态、缓存、同步、数据事务 | `plugins/vuex.js`、`plugins/cache.js`、后端 controller/model | `stores/*.js`、`utils/*cache*`、`backend/models`、`services`、`sync` | 书架、认证 scope 与阅读进度 P2 已完成并发布；**WebSocket 协议第二轮已完成 inventory，实施待测试先行**：当前存在任意客户端事件 relay、无条件 Origin 和全局 `users_update` 三项 must-fix；服务端事件 type/payload、同用户收敛、重连 REST 权威和数据格式必须保留。 | [`reading-progress-p2-contract.md`](reading-progress-p2-contract.md)、[`websocket-sync-p2-contract.md`](websocket-sync-p2-contract.md)；WebSocket 当前状态 `audit-complete / implementation-pending`，不能沿用 `BroadcastAll` 或未使用的 client send。 |
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。阅读进度、外部 WebDAV、远程封面等已发布；当前 `/ws/sync` 已提取 same-origin/active-user/server-only/recipient-scope 合同，尚未实施。其它未命名动作仍不能由“全量测试通过”推定兼容。 | 既有专项合同继续有效；新增 [`websocket-sync-p2-contract.md`](websocket-sync-p2-contract.md) 和更新后的 [`api-diff.md`](api-diff.md)。下一门为失败握手/relay/recipient 测试。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 P2 parser 预算与 Reader 移动书架修复已由本机作为 `e7f168e` 发布**：focused/full/race/frontend 706/706/build/vet/diff、真实导入三视口及 amd64/arm64 推送成功。 | `e7f168e`/`latest` OCI index `sha256:8d64bbb187f65c433388bddc5385ce68d42e8b40d9b397787e4c1d354c892dac`。fresh volume 的 portable v1/v2 assets、cross-user、restart 和 historical TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过。 |

## 当前整体进度快照（2026-08-09，`e7f168e`）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **88%**。该数字表示“已完成固定基准复审、
测试先行实现并至少发布过一次”的覆盖度，不表示剩余 12% 只是简单收尾。
...
- **P2 已覆盖的高风险项**：账号/overlay/cache 隔离、原子阅读进度、删除收敛、替换规则、书签、
  ...
  capability，共享抓取器 P2-N1/N2 请求与私网/DNS/代理边界，以及本地格式 parser 的输入、解码
  文本、章节与历史读取预算。
- **尚未完成的主线**：尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  以及后续真实设备反馈暴露出的上游可见偏差。最后一次线上核验仍运行 `7971e23`，需部署
  `e7f168e` 后再验收 Reader 内书架。上述项目继续按“合同→失败测试→实现→浏览器/新旧卷→本地
  Docker”推进。
