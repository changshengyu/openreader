...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。阅读进度、外部 WebDAV、远程封面等已发布；`/ws/sync` 的 same-origin/active-user/server-only/recipient-scope 已实施验证。其它未命名动作仍不能由“全量测试通过”推定兼容。 | 既有专项合同继续有效；新增 [`websocket-sync-p2-contract.md`](websocket-sync-p2-contract.md) 和更新后的 [`api-diff.md`](api-diff.md)。当前待关闭备份事务 vet 债务和 Docker 门。 |
