...
| 本地导入、书仓、WebDAV | `Index.vue#importBookPreview/importMultiBooks`、`BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/api/direct_local_import_boundary.go`、`imports.go`、`localstore*`、`webdav*`、`services/localbook/*`、`engine/*_parser.go` | P1-E1…P1-E5、P2 parser 预算、TXT 默认规则和 mounted LocalStore/WebDAV 边界保持已发布。**直接浏览器导入第二轮已实现并完成回归**：恢复 1..64 多选、单本直达确认、多本共享批量/逐一状态机、同名稳定 identity、token-only 确认；三个 direct multipart 路由补齐认证后 actual-read 包络、严格 shape/metadata/category 和 handler-owned 临时文件清理。 | [`direct-local-book-import-multipart-workflow-fixed-baseline-second-audit-p1-contract.md`](direct-local-book-import-multipart-workflow-fixed-baseline-second-audit-p1-contract.md) 状态 `implementation-complete / regression-validated / Docker-release-pending`；frontend 737/737、Go full/race/vet、build、真实 HTTP 与三视口 direct/storage flow 均通过。LocalStore/WebDAV、parser、token/snapshot 和 `65a9870` 历史镜像证据不重开。 |
...
## 当前整体进度快照（2026-08-16，direct import 实现/回归 pass，Docker 待发布）
...
- **P1 Index 工作台主链已覆盖**：侧边栏、普通书架、搜索/探索、唯一 BookInfo、BookManage、
  ...
  第二轮均已发布；直接浏览器导入的多选、共享确认状态机和 multipart 边界已完成实现/运行时签收，
  当前等待本轮卷门与 Docker 发布。
