...
| 书源与搜索结果 | `Index.vue`、`Explore.vue`、`BookSource.vue`、`BookSourceController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`SourceSwitchPanel.vue`、`backend/api/sources.go` | **P1-C 工作台收敛、P2 失效缓存、Reader 换源、CSS/JSONPath/XPath 主解析链和脚本能力透明化均已实施验证并发布 `cad1f40`**；动态 Header、`loginCheckJs`、规则脚本/模板和 WebView 的安全差异已显式。下一项已确认差距是书名/作者/简介未执行上游统一后处理，且 evaluator 错把 `canReName` 配置当布尔规则执行；全局书源 + 用户编辑权限仍是已记录的数据模型重设计。 | [`booksource-metadata-normalization-p2-contract.md`](booksource-metadata-normalization-p2-contract.md)；先补黄金与 API 副作用测试，再统一解析器元数据出口，不修改路由或存量数据。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `cad1f40` 已验证**：前端 448/448、Go 全量、生产构建、书源工作台三视口、当前镜像 volume/backup、远端 amd64/arm64 manifest 均通过；以后每次发布仍必须以新提交重跑。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:9ebe3763476f79bd7b745e03efd5f50ff07a83e9eb0b8e705df6f98f49112bc9`。 |
