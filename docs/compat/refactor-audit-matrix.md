...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`main.js#getCover`、`BookController#getBookCover` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、`stores/bookshelf.js`、`useSync.js` | **BookManage 第二轮固定基准重建已完成并通过候选门**：桌面/手机统一同一表格，恢复 normal gate、动态几何、标题/作者搜索、查询保留、每次 force shelf read、精确列/动作/footer/确认与缓存中取消状态；移动卡片、阅读进度/类型/最新章、无选择禁用及“停止 n/total”已删除。JWT/SQLite、多用户、many-to-many 幂等移除和缓存安全为明确技术适配。书架/BookInfo 既有结论保留；BookGroup 完整管理面仍待独立第二轮复审。 | [`book-management-fixed-baseline-second-audit-p2-contract.md`](book-management-fixed-baseline-second-audit-p2-contract.md)：frontend 661/661、Go/build，mock 1440/390/360/两种 iPad 与真实 Go/SQLite 1440/390/360 均通过；状态 `aligned / Docker pending`。BookInfo 仍见 [`bookinfo-action-ownership-cleanup-p1-contract.md`](bookinfo-action-ownership-cleanup-p1-contract.md) 与 [`book-cover-proxy-p2-contract.md`](book-cover-proxy-p2-contract.md)。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十三次移动点击已发布 `49a273e`；第八轮夜间实际文字承载层修复始于 `6fde5ab` 并已包含在当前 `f2f0d6e`**：黑色内容面按最终“不透明纯黑页面 + 无背景图”接管；普通正文 `p/span/mark` 与 EPUB 实际后代节点自身均显式纯黑/纯白，不再依赖祖先透明穿透，退出夜间完整恢复作者样式。非黑自定义主题与背景图保持不变。 | [`reader-night-contrast-p0-contract.md`](reader-night-contrast-p0-contract.md)：frontend 652/652、Go/build、普通 Reader 与真实 EPUB 1440×900/390×844/360×800 及新旧卷通过。当前 `f2f0d6e`/`latest` index 为 `sha256:cfaba7d453bde2a4b44198aa57ef8ef5ecbaa3b4cce0ab77b4c816311455c736`；部署端 pull/force-recreate 且 `/api/health` 返回 `f2f0d6e` 后才代表已升级。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前书内搜索第二轮、Bookmark 第二轮与夜间文字节点内容面修复已由本机作为 `1801037` 发布**：frontend 659/659、Go、build、普通 Reader 桌面/手机/iPad、真实 EPUB 1440/390/360、新卷 portable v1/v2 assets/跨用户/重启和历史 TXT/EPUB/UMD/CBZ/相对缓存/owner isolation 均通过。 | `1801037`/`latest` 共同指向 amd64/arm64 OCI index `sha256:5d2fdb171e734d5debece77f91ae31495fc1ba7ee9eec28c88aa2b3f41eeeee5`；等待设备验收。 |
