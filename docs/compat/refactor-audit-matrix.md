...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book metadata patch 与独立 category action 已由 `4b0a599` 关闭 full-row `Save`：request-context transaction 重读 owner Book，显式列 update，删除不复活，成功重载权威 response/event。 | [`book-patch-category-write-lifecycle-fixed-baseline-second-audit-p2-contract.md`](book-patch-category-write-lifecycle-fixed-baseline-second-audit-p2-contract.md)；旧实现红测、focused/race/full/vet、BookManage/BookInfo 三视口及可信卷门通过，状态 `aligned / regression-validated / Docker-published / awaiting-device-verification`。旧 wire/UI/data 合同不重开。 |
...
## 当前整体进度快照（2026-08-30，Book patch/category lifecycle 已发布）
...
并发提交亦已关闭。剩余约 1% 当前包含后续逐路由 action 审计、长尾固定基准复审与真实设备证据，
