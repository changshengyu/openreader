...
| 书架、BookManage、BookGroup、BookInfo | `Index.vue` 普通 shelf、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、stores | **BookManage、BookGroup、BookInfo 第二轮已分别发布；普通书架 `c851c5f` 后的 28px 移动宽度回归已修复并验证，等待 Docker 发布**。固定 380px grid、当前筛选计数、精确编辑搜索、历史 list→grid v3 迁移、独立 metadata DOM、loading/blank/night 保持；书架场景现明确覆盖通用 app-page 左右各 14px，390/360px 视口重新得到同宽书籍行。书架 freshness、进度和 lastCheckTime 专项继续保持关闭。 | [`bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md`](bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md)：frontend 701/701、Go/build/diff、书架四视口、Reader 移动书架内层宽度和 Index 工作台合同通过；当前状态 `regression-fixed / awaiting Docker publish`。 |
