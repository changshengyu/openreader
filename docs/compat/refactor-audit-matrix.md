...
| 用户、备份、RSS、替换规则、书签 | `UserManage.vue`、`WebDAV.vue`、`Rss*`、`ReplaceRule*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`WebDAVBrowser.vue`、`RSSManager.vue`、`OverlayReplaceRules.vue`、`OverlayBookmarks.vue`、Go API/services | RSS、替换规则和书签合同继续成立；**P2-S4 的 logical/portable/WebDAV owner 隔离及旧全局源升级已随 `0db752e` 完成 Docker 证明**。管理员旧根只保留路径兼容，内容仍限定管理员；普通用户私有根及恢复均不跨账号。 | [`book-source-ownership-p2-contract.md`](book-source-ownership-p2-contract.md)、[`rss-source-lifecycle-p2-contract.md`](rss-source-lifecycle-p2-contract.md)、[`user-management-p2-contract.md`](user-management-p2-contract.md)、[`backup-restore-fixed-baseline-p2-contract.md`](backup-restore-fixed-baseline-p2-contract.md)；选择下一尚未完整复审的 P2 动作。 |
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十三次移动点击候选已实施并发布 `49a273e`；夜间实际文字层第三轮已发布 `9a13d8e`，两项均待实机复验**：移动点击沿用已记录的绘制/定位优化；内置夜间现在强制 shell/page/普通正文后代和 EPUB iframe 根层为纯黑，实际文字后代为纯白透明面，并可逆恢复日间 EPUB 作者背景。 | 移动点击继续按 [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md) 实机复验；夜间按 [`reader-night-contrast-p0-contract.md`](reader-night-contrast-p0-contract.md) 用 `9a13d8e`/`latest` 复验。custom 夜间保持用户资源与对比保护，是明确允许差异。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 `9a13d8e` 已从本机完成并发布**：frontend 640/640、Go、build、普通正文/EPUB 桌面、手机与 iPad 浏览器门、首次登录并发零 500、新卷、历史 TXT/EPUB/UMD/CBZ/相对缓存、portable v1/v2、跨用户、恢复与重启均通过；`9a13d8e`/`latest` 共同指向 `sha256:777bcb96fa59d718b413b22756b3b30696b891bed7826930af168ce15d0e6bed`。 | `openreader-regression`、`openreader-docker-release`；继续保留 Reader 真实手机体感与夜间设备复验，不能用自动门替代设备结论。 |
...
| 标题元素和排版 | `Content.vue` 渲染章节标题为 `h3`；CSS 固定 `font-size:28px; line-height:1.2; margin:1em 0; text-align:center`。 | 当前普通/卷/错误章节均为 `h3[data-pos="0"]` 且样式一致；书签、搜索、TTS 和 viewport progress 已同步，但 `useReaderNavigation#paragraphByChapterPosition` 仍残留不存在的 `h1[data-pos]`。 | **局部 must-fix**：渲染与排版已对齐，只需关闭字符位置恢复的残留 `h1` consumer；见 [`reader-text-position-selector-p0-contract.md`](reader-text-position-selector-p0-contract.md)。 |
