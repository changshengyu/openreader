...
| 本地导入、书仓、WebDAV | `Index.vue#importBookPreview/importMultiBooks`、`BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`OverlayStorageImport.vue`、`useOverlayBookImport.js`、`useStorageImportWorkflow.js`、`backend/api/imports.go`、`localstore*`、`webdav*`、`services/localbook/*`、`engine/*_parser.go` | P1-E1…P1-E5、P2 parser 预算、TXT 默认规则和 mounted LocalStore/WebDAV 边界保持已发布。**直接浏览器导入已在第二轮重开**：固定上游一次可多选，单本直达确认，多本进入共享批量/逐本状态机；当前仍是独立单文件表单。三个 direct multipart 路由还缺 actual-read 总包络、严格 file/token shape 和 handler-owned 临时文件清理。 | [`direct-local-book-import-multipart-workflow-fixed-baseline-second-audit-p1-contract.md`](direct-local-book-import-multipart-workflow-fixed-baseline-second-audit-p1-contract.md) 状态 `inventory-complete / implementation-pending`；下一步必须先提交前端共享状态机与后端 multipart 正式红测。已发布的 LocalStore/WebDAV、parser、token/snapshot、卷门和 `65a9870` 镜像证据不重开。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；已关闭模块不从旧日志重开**。WebDAV mounted import/restore 保持 `65a9870` 已发布；重新差集后 direct local import 成为下一项：三个路由在 Gin multipart 完整解析前没有 actual-read 上限，并静默采用重复/额外 part 的首值。 | [`rest-action-coverage-fixed-baseline-p2-contract.md`](rest-action-coverage-fixed-baseline-p2-contract.md) 第 18 节与 direct 专项合同状态 `inventory-complete / implementation-pending`。其正式红测/实现后才继续 source multipart、remote-work JSON、progress 与其它 batch JSON。 |
