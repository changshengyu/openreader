...
| 用户、备份、RSS、替换规则、书签 | `UserManage.vue`、`WebDAV.vue`、`Rss*`、`ReplaceRule*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`OverlayBackups.vue`、`RSSManager.vue`、`OverlayReplaceRules.vue`、`OverlayBookmarks.vue`、Go API/services | **用户、RSS、替换规则、书签自身交互及 portable v1 已完成；普通备份/恢复 bridge 于 2026-07-22 重新打开。** 复审确认当前第二套备份管理器、上游单数文件名/字段缺失、生成吞错/非原子、内容错误部分恢复及 `canEditSources` 绕过均不符合固定基线。原始 WebDAV 协议不重开。 | 新权威为 [`backup-restore-fixed-baseline-p2-contract.md`](backup-restore-fixed-baseline-p2-contract.md)；其测试、实现、三视口和旧卷门禁完成前不得再宣称整行完成。 |
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十批 Docker-published / awaiting device verification**：animator 已按固定上游改为回调执行时钟，首回调不再因陈旧 rAF 时间戳写回起点；正文父层移除 `filter:brightness(...)`，以无事件黑色遮罩保留亮度。三文字模式、移动两尺寸、桌面、iPad、连续、图片和真实后端 EPUB 已通过；原生手指/滚轮连续滚动继续保留。 | [`reader-ipad-panel-dismiss-p0-contract.md`](reader-ipad-panel-dismiss-p0-contract.md)、[`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；必须以新镜像的真实手机体感决定是否关闭该项。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `0a77632` 已验证**：前端 534/534、Go 全量、生产构建、桌面/移动/iPad 文字模式、首回调时钟、无过滤滚动父层、连续跨章、图片和真实后端 EPUB 通过；本地候选的新旧卷、TXT/EPUB/UMD/CBZ、相对缓存、用户隔离、重启、便携恢复均通过。镜像由本机完成双架构构建与上传。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:226f982cf31ef20d5d6c9c2e6e5c03c18da19bcc8a23db38acc3fcb43a177c11`，amd64 `sha256:815e0d27623e9ce6f02bcd9bf3880916e24af8843476e45e6c5f11acbb030393`，arm64 `sha256:70aa13fcaa9b6ebee1a27391003168df20ac25dbc4fb601645fbf139066ff609`。 |
