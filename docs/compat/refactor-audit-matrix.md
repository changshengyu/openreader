...
| 用户、备份、RSS、替换规则、书签 | `UserManage.vue`、`AddUser.vue`、`BaseController.kt#saveUserSession`、`WebDAV.vue`、`Rss*`、`ReplaceRule*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`backend/api/auth.go`/`admin.go`、`WebDAVBrowser.vue`、`RSSManager.vue`、`OverlayReplaceRules.vue`、`OverlayBookmarks.vue`、Go services | RSS、替换规则和书签合同继续成立；P2-S4 owner 隔离和用户书源动作已发布。2026-07-28 UserManage 复审删除移动卡片/合并权限列，恢复桌面移动共用表格、固定移动首列、上游列/底栏顺序和显式取消；登录成功持久更新 canonical `lastLoginAt`，`lastActiveAt` 仅留作兼容别名。全部账号私有 Overlay 已叠加 lifecycle identity guard；六条真实 pending/续登/换号浏览器流程已实现但最终三视口复跑仍待获批。 | **UserManage 已随 `f44447f` 发布**：Go、frontend 641/641、build、1440/1024/390/360 浏览器以及新旧卷门通过；index `sha256:a3fb3e55d0e7002390f34e2bb30fb54adda9b5a64e2a8e1ca87b0f7941f7a64c`。弹层会话隔离按 [`workspace-overlay-authenticated-session-p1-contract.md`](workspace-overlay-authenticated-session-p1-contract.md) 保持 candidate，不在浏览器全矩阵前发布 Docker。其余模块继续按各自专项合同复审。 |
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十三次移动点击候选已实施并发布 `49a273e`；夜间实际文字层修复已重新发布于 `59e11a9`，两项仍待用户实机体感/设备复验**：内置夜间强制 shell/page/普通正文后代和 EPUB iframe 根层为纯黑，实际文字后代为纯白透明面，并可逆恢复日间 EPUB 作者背景。 | 移动点击继续按 [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md) 实机复验；夜间设备验收必须先确认 `/api/health` 为 `59e11a9`，不能继续用线上旧 `a90d10b` 判断当前修复。custom 夜间保持用户资源与对比保护。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前运行时 `59e11a9` 已从本机完成并发布**：frontend 643/643、Go、build、Reader 401 四视口、TXT/EPUB 夜间实际文字层、新卷、历史 TXT/EPUB/UMD/CBZ/相对缓存、portable v1/v2、跨用户、恢复与重启均通过；本批再关闭 Index cache/session 的 1440/390/360 浏览器证据。`59e11a9`/`latest` 共同指向 `sha256:8ce5f345fb376ac13e0b5f80d246a7421c18bb2cf0647039d73298d3255b511b`。 | `openreader-regression`、`openreader-docker-release`；线上当前仍是 `a90d10b`，必须替换容器并确认 `/api/health` 为 `59e11a9`。本批只有 smoke/docs，无运行时代码差异，不重复发布同一镜像。 |
