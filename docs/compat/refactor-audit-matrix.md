...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、格式运行时、设置、夜间面、远程正文和换源写入生命周期均已专项实施。**2026-09-14 真机 stale 409 二次修复已在 `e1631d0` 实施并发布**。同日桌面翻页反馈已完成本地实现：纵向文本恢复 document-root，点击区恢复 30/40/30，滚轮/触控板保持原生且不在边界合成切章；用户要求的固定章节运行栏保留。 | [`reader-remote-chapter-concurrency-device-feedback-p0-contract.md`](reader-remote-chapter-concurrency-device-feedback-p0-contract.md) 已发布并等待真机复验；桌面翻页见 [`reader-desktop-pagination-device-feedback-p0-contract.md`](reader-desktop-pagination-device-feedback-p0-contract.md)，frontend 754/754、Go/build/diff、桌面双视口、连续四视口、移动/iPad、设置位置、内联缓存与深章节性能合同通过，状态 `implemented / regression-validated / Docker-pending / awaiting-device-verification`。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category/remote-add、章节正文/换源/本地 cache 与用户资产生命周期均已关闭。资产上传/删除、Book/Setting 引用和 portable 使用 rooted current-entry I/O、staged no-overwrite publication 与 caller-scoped coordinator。 | [`user-asset-filesystem-reference-lifecycle-fixed-baseline-second-audit-p2-contract.md`](user-asset-filesystem-reference-lifecycle-fixed-baseline-second-audit-p2-contract.md) 按合同 `478654a` → 红测 `947dfcb` → 实现 `3e8cec7` 落地；focused/race、API/backup full、Go full/vet、frontend 752/752、build/Compose 通过。状态 `aligned / regression-validated / Docker-published / awaiting-device-verification`。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release workflow | `e1631d0` 本地通过 frontend 753/753、Go focused/race/full/vet、build、Compose、Reader 连续/移动/缓存四视口；可信 Actions run `34794997078` 又通过 backend/frontend/build/Compose、native、fresh/portable、historical volume 与 published-platform 门。前端构建固定在 `$BUILDPLATFORM` 原生运行；Go/CGO 与最终镜像仍按目标架构生成。 | Git 实现/Actions/Docker commit 为 `e1631d0`；已发布 `e1631d0`/`latest` OCI index `sha256:94030bd8f72dcb5135ade46571a9b81d686da616fc704e4144dc78414a33c3ee`。amd64 manifest `sha256:956ce9596243c2b25b1a7cc1d884d1231c1c25e06df5e2ce3289dd595c8ab2d4`，arm64 manifest `sha256:0075d8160137bcc2e4cdb0d8924438a3421e02a673f2f0f6ff02ed35727059b9`；`unknown/unknown` 为 attestation，用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-09-14，Reader 真机回归本地修复已实施）
...
已按 `478654a`/`947dfcb`/`3e8cec7` 完成实现、本地回归和可信双架构发布；下一项须从剩余
逐路由 action 差集继续取证，不能凭当前组件或旧日志推定。
...
- **P0 Reader 主链已覆盖**：工具层/面板状态机、正文排版、移动点击与连续滚动、设置、书签、正文
  ...
  `2bbb276` 的一次前端重试未通过真机验收；二次合同/红测/实现已由
  `9b96c50`/`ebf548b`/`9fea3cf`/`e1631d0` 收敛为同书抓取门、执行前状态重读和新 cache 复用，真实
  lifecycle 409 保护保持。本地书占位回填、完整 EPUB fragment 身份和左上章节运行标注也已在
  `e1631d0` 实施并完成本地全量与四视口回归，当前等待可信 Actions 镜像和用户真机签收。
...
| 远程相邻章节并发 | 上游连续窗口并发加载，但不会因另一个相邻章节更新临时 Book variable 而把正常请求判成版本冲突。 | 同一 user/book 的未缓存远程抓取已串行；等待者重读状态并优先复用新 cache，不同书仍独立。 | **2026-09-14 已实施、回归并发布 `e1631d0`**：保留真实生命周期 409；用户真机待验证；见 [`reader-remote-chapter-concurrency-device-feedback-p0-contract.md`](reader-remote-chapter-concurrency-device-feedback-p0-contract.md)。 |
