...
| Pinia 状态、缓存、同步、数据事务 | `plugins/vuex.js`、`plugins/cache.js`、后端 controller/model | `stores/*.js`、`utils/*cache*`、`backend/models`、`services`、`sync` | **书架 P2 新鲜度/收敛切片已实施验证，其它项继续复审**：保留 revision、本地即时变更与重连强刷；冷启动恢复网络优先、旧缓存仅失败 fallback；Hub 慢客户端断开；connected 不再阻止成功节流的前台校准；同用户首次设置写用原子 upsert 消除真实双客户端 500。 | [`bookshelf-network-first-sync-p2-contract.md`](bookshelf-network-first-sync-p2-contract.md)；单元、Go API/Hub 和真实双客户端三视口均通过，待本批 Docker 卷/备份和 GHCR 门禁。 |
