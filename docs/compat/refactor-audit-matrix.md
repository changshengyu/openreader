...
| 用户、备份、RSS、替换规则、书签 | `UserManage.vue`、`WebDAV.vue`、`Rss*`、`ReplaceRule*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`WebDAVBrowser.vue`、`RSSManager.vue`、`OverlayReplaceRules.vue`、`OverlayBookmarks.vue`、Go API/services | **已抽取 P2 行为均完成并随 `0f636f1` 发布**：RSS 生命周期复审补齐名称/URL 校验、同地址保存、异步列表请求所有权和删除事务，三层 Dialog 与导入默认值保持上游语义；用户、备份、替换规则、书签合同继续成立。全局书源加编辑权限是明确的数据模型重设计，不伪造上游单用户书源删除按钮。 | [`rss-source-lifecycle-p2-contract.md`](rss-source-lifecycle-p2-contract.md)、[`user-management-p2-contract.md`](user-management-p2-contract.md)、[`backup-restore-fixed-baseline-p2-contract.md`](backup-restore-fixed-baseline-p2-contract.md)、[`portable-appearance-assets-p2b-contract.md`](portable-appearance-assets-p2b-contract.md)；RSS 通过 579 项前端、全量 Go、生产构建、三视口延迟竞态浏览器门禁及新旧卷测试；镜像 index 为 `sha256:25398a86aa809b5ce1157f64dee1fd0d85673089480223a7375eae9bb86c6f5c`。 |
...
| Reader：登录失效与账号切换 | `plugins/axios.js` 的 `NEED_LOGIN`、根 `App.vue#login`、`Reader.vue#loginAuth` | `api/client.js`、`App.vue`、`AuthDialog.vue`、`stores/user.js`、Reader lifecycle/progress、`stores/overlay.js` | **2026-07-27 已审查，P0 待实施**：401/store/WS operation scope 已完成，但 token 清空后旧 Reader 仍渲染；新 token 写入后的整页刷新可让旧退出钩子按新 scope 保存，旧 overlay 也可能重新挂载。 | [`reader-reauthentication-isolation-p0-contract.md`](reader-reauthentication-isolation-p0-contract.md)；先补失败状态机测试，再实现同步 suspend、私有 DOM/overlay 卸载、同账号重挂载和异账号返回书架。 |
