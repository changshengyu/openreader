...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`main.js#getCover`、`BookController#getBookCover` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、`stores/bookshelf.js`、`useSync.js` | **BookManage 第二轮已发布 `af4e2a4`；BookGroup 第二轮重建已完成，等待本批 Docker**。BookGroup 已恢复 normal gate、动态几何、唯一条件列表、Element selection、名称内拖动、同一 Sortable/set 禁用、单 footer、精确动作和 watcher 状态；修复 Sortable/Vue 双重位移、失效 token 自动切组及两条隐藏自定义组恢复丢失 false。统一投影、四内置、多用户、多对多、混合事务排序和同步/备份架构继续保留。 | BookManage 证据见 [`book-management-fixed-baseline-second-audit-p2-contract.md`](book-management-fixed-baseline-second-audit-p2-contract.md)，状态 `aligned / Docker-published`。BookGroup 见 [`book-group-fixed-baseline-second-audit-p2-contract.md`](book-group-fixed-baseline-second-audit-p2-contract.md)：frontend 663/663、Go/build、真实五视口 manager、三视口 BookManage/BookInfo set 与 mock 五视口通过，状态 `aligned / implementation-complete / Docker pending`。BookInfo 仍见 [`bookinfo-action-ownership-cleanup-p1-contract.md`](bookinfo-action-ownership-cleanup-p1-contract.md) 与 [`book-cover-proxy-p2-contract.md`](book-cover-proxy-p2-contract.md)。 |
