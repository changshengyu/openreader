...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category/remote-add、章节正文/换源/本地 cache、用户资产和管理员删除用户工作区 lifecycle 均已关闭。第 49 项以 rooted parent identity detach 修复中间 symlink 根外删除，并让旧用户名安全投影碰撞 fail closed。 | [`admin-user-workspace-cleanup-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md`](admin-user-workspace-cleanup-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md) 已按合同 `386f555` → 红测 `0f61853` → 实现 `016a346` 落地；focused/race/full/vet、frontend 754/754、build/Compose 和可信 Actions 全部门通过。状态 `aligned / regression-validated / Docker-published / awaiting-device-verification`。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release workflow | `016a346` 本地通过 frontend 754/754、Go focused/race/full/vet、build 与 Compose。可信 Actions run `34825873958` 又通过 backend/frontend/build/Compose、native、fresh/portable、historical volume 与 published-platform 门。 | Docker commit 为 `016a346`；已发布 `016a346`/`latest` OCI index `sha256:50031b016e22c18d6c06634ed4e809be9570bff48f8840dffae3d460b1595881`。amd64 manifest `sha256:6c68839b7906baad032e5c11edb1f4d50d27786fe796c2a11126857905bec767`，arm64 manifest `sha256:a2dc36bfdcdf5f6fe98c8199c3fecaa93fd181b777bae2de7393e7b1c14ae20c`；文档记录提交将晚于镜像 commit，用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-09-14，管理员工作区清理已实施发布）
...
投影碰撞，并由可信 Actions 完成双架构发布；下一项仍须从剩余 action 差集重新取证。
...
- **P0 Reader 主链已覆盖**：工具层/面板状态机、正文排版、移动点击与连续滚动、设置、书签、正文
  ...
  transport 与调用方订阅，13 秒延迟正文已通过三视口 Chromium，等待可信 Actions 与真机复验。
