...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue` | **BookManage / BookGroup 与 BookInfo 书架态 P2 均已验证**：管理与分组为 Index 内单一 Dialog；BookInfo 五入口共享同一动作状态机；封面只精确更新 `customCoverUrl` 且新资产按用户隔离，追更只更新 `canUpdate`，分组和本地刷新保持既有事务/同步边界。 | [`bookinfo-shelf-mutations-p2-contract.md`](bookinfo-shelf-mutations-p2-contract.md)；后续保持浏览器、资产/旧卷和本地 refresh 回归，不重开重复 BookInfo 流程。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 持续复核；移动点击翻页与章末入口已修复并发布 `79645c8`**：竖向程序动画不再逐帧执行重型 DOM/进度同步，结束后只结算一次；page 正文流恢复 `加载下一章` 和末章边界提示。连续 touch/wheel、曲线、时长、步长不变。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；前端 471/471、Go/build、text/mobile/continuous/image/volume、真实 EPUB/CBZ 浏览器合同和历史卷/备份门禁通过，待用户移动端验证。 |
...
| Pinia 状态、缓存、同步、数据事务 | `plugins/vuex.js`、`plugins/cache.js`、后端 controller/model | `stores/*.js`、`utils/*cache*`、`backend/models`、`services`、`sync` | **书架 P2 与认证运行时 scope 已实现；阅读进度并发切片审查中**：书架 network-first/revision/Hub/前台校准保持完成；异步提交已绑定账号身份与 operation revision。进度保存仍缺少数据库原子 CAS，并有退出重复请求。 | [`bookshelf-network-first-sync-p2-contract.md`](bookshelf-network-first-sync-p2-contract.md)、[`authenticated-runtime-scope-p2-contract.md`](authenticated-runtime-scope-p2-contract.md)、[`reading-progress-p2-contract.md`](reading-progress-p2-contract.md)；下一步先加失败测试。 |
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/auth.go`、前端 `api/*.js` | **按动作逐项复审；当前阅读进度切片已完成合同、尚未实施**：稳定路径/冲突响应保留，但章节规范化、原子 CAS、单次广播和既有 WebDAV 目录镜像为 `must-fix`。其它已完成模块仍以各专项 API 合同为证，不能用本行概括宣称全 REST 完成。 | [`reading-progress-p2-contract.md`](reading-progress-p2-contract.md)；完成 Go/前端/双客户端/卷门禁后再关闭本切片，随后选择下一未审查 API 组。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `79645c8` 已验证**：前端 471/471、Go 全量、生产构建、Reader 三视口与真实 EPUB/CBZ、本地历史 volume/backup、远端 amd64/arm64 manifest 均通过；以后每次发布仍必须以新提交重跑。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:075a69352e7aa862383408a765c35f4638dc1bbbc45fc4e6dde8d11095803670`。 |
