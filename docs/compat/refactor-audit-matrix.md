...
| Reader：EPUB、漫画/CBZ、音频、连续跨章、TTS | `Reader.vue`、`Content.vue`、本地格式解析类 | `ReaderChapterContent.vue`、`ReaderEpubContent.vue`、`ReaderAudioContent.vue`、格式 parser / cache | **EPUB Bug 1 与固定目录/resource 边界已实现并重跑三视口**：跨 XHTML bridge 不写 iframe/browser 章节历史，返回直达书架；同 XHTML 正文连续、跨 resource 跳章、相对资源、工具层与当前段落书签均通过。其余格式仍尚未逐项验证。 | [`epub-mobile-back-bug1-contract.md`](epub-mobile-back-bug1-contract.md)、[`epub-fixed-baseline-catalog-reader-contract.md`](epub-fixed-baseline-catalog-reader-contract.md)；后续转向漫画/音频/连续跨章/TTS 独立合同。 |
