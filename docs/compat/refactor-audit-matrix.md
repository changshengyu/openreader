...
| 书架、BookManage、BookGroup、BookInfo | `Index.vue` 普通 shelf、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、stores | **BookManage、BookGroup、BookInfo 第二轮已分别发布；普通书架 `c851c5f` 后的 28px 移动宽度回归已修复并作为 `7971e23` 发布**。固定 380px grid、当前筛选计数、精确编辑搜索、历史 list→grid v3 迁移、独立 metadata DOM、loading/blank/night 保持；书架场景现明确覆盖通用 app-page 左右各 14px，390/360px 视口重新得到同宽书籍行。书架 freshness、进度和 lastCheckTime 专项继续保持关闭。Reader 内书架是独立 P0 合同，其每侧多缩进 4px 的偏差已完成实现与浏览器验证。 | [`bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md`](bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md) 保持 `aligned`；Reader 内书架见 [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md)，状态 `implemented / regression-validated / Docker-pending`。Index 仍实测保持 390/360px 行宽。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置和夜间面此前均已专项发布；**本轮 Reader 移动端阅读内书架宽度偏差已修复并通过真实浏览器**：主面板仍为 100vw，最终左右 inset 从错误的 24px 恢复为上游 20px，390/360 列表为 350/320px；1024 强制手机模式保持上游四列。 | [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md) 状态 `implemented / regression-validated / Docker-pending`；Reader 桌面/双手机/自适应与强制 iPad、Index 书架四视口、frontend 706/706、Go/build 通过。其它已关闭 Reader 专项不因本偏差自动重开。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 P2-N1 半模块切片已由本机作为 `981bca7` 发布**：frontend 706/706、Go/race/build/diff、Source/RSS/Index 真实浏览器流程及 amd64/arm64 推送成功。 | `981bca7`/`latest` OCI index `sha256:02160e0797b3371fdfadccb550b8766d412c3e09df632ba1e36d192b26eb500d`。fresh volume 的 portable v1/v2 assets、cross-user、restart 和 historical TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过；当前新发现的 Reader 移动书架宽度修复尚未进入该镜像。 |
