...
| 外部 WebDAV 协议 | `WebdavController.kt` 的 `/reader3/webdav*`、Basic、OPTIONS/PROPFIND/MKCOL/PUT/GET/DELETE/MOVE/COPY/LOCK/UNLOCK | `backend/api/server.go`、`backend/api/webdav.go`、`middleware/webdav_auth.go`、`services/webdavfs` | 双前缀、认证、DAV 方法、caller scope 与可见状态保持签收；**第 50 项 DELETE lifecycle 已随 `daa435d` 关闭**：从受信 boundary 打开 root/parent/target，在同一 parent fd 内复验 identity、detach 并 handle-relative 删除，验证后 root/parent/target 替换均 fail closed。 | [`webdav-delete-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md`](webdav-delete-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md) 已按合同/红测/实现顺序完成；Actions run `34960341835` 全部门通过，OCI index `sha256:f563313d1d38358ba354189a62fd47beda2ba4f83d2c1e9f15ddd8ae131cd417`，状态 `aligned / Docker-published / awaiting-device-verification`。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、格式运行时、设置、夜间面、远程正文和换源写入生命周期均已专项实施。**第三次章节真机反馈后，`c7fbf73` 又恢复固定上游 30 秒正文预算，并让同章新 generation 接管共享请求而不复用已取消 Promise**；`0ecc4d9` 的同章 gate、相邻章并行和生命周期 CAS 保留。桌面翻页维持 document-root、30/40/30 点击区和原生滚轮/触控板；固定章节运行栏保留。 | 可信 Actions run `34853164985` 已发布 `c7fbf73`/`latest` amd64/arm64 OCI index `sha256:874b0262c48a19c6861f80ca8cfdb157b6c419ebc46e7a7f4dd320358b8897ca`；frontend 756/756、Go full/vet、build/Compose、13 秒延迟章节三视口及全部卷门通过，状态 `implemented / regression-validated / Docker-published / awaiting-device-verification`。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category/remote-add、章节正文/换源/本地 cache、用户资产、管理员工作区和 WebDAV DELETE lifecycle 均已关闭。第 50 项把文件/目录删除纳入 rooted current-identity 事务。 | [`webdav-delete-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md`](webdav-delete-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md) 已按 `7d364ff` → `2c96481` → `daa435d` 落地；focused/race/full/vet、frontend 757/757、build/Compose、真实协议及可信 Actions 全部门通过。状态 `aligned / regression-validated / Docker-published / awaiting-device-verification`。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release workflow | `daa435d` 本地通过 frontend 757/757、Go focused/race/full/vet、Linux 双架构编译、build、Compose 与真实 WebDAV 协议。可信 Actions run `34960341835` 又通过 backend/frontend/build/Compose、native、fresh/portable、historical volume 与 published-platform 门。 | Docker commit 为 `daa435d`；已发布 `daa435d`/`latest` OCI index `sha256:f563313d1d38358ba354189a62fd47beda2ba4f83d2c1e9f15ddd8ae131cd417`。amd64 manifest `sha256:ecb4ed2cb283233708a63ef194d0c8ca1ec4bdfc447cea9e43d1fe3b4ddcd998`，arm64 manifest `sha256:8a5d7dd1d3e6fb5d871a1cad4b72cf8d6e3559b11798412139d3f1346e4e4d4a`；文档记录提交将晚于镜像 commit，用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-09-15，WebDAV DELETE lifecycle 已实施发布）
...
已按 `478654a`/`947dfcb`/`3e8cec7` 完成实现、本地回归和可信双架构发布。第 49 项又以
`386f555`/`0f61853`/`016a346` 关闭管理员删除用户工作区的 lexical `RemoveAll` 根外删除和历史用户名
投影碰撞，并由可信 Actions 完成双架构发布。

第 50 项又以 `7d364ff`/`2c96481`/`daa435d` 关闭 WebDAV DELETE 验证后父级替换导致的根外删除，
并由可信 Actions 完成卷门和双架构发布；剩余约 1% 仍从未覆盖 action 和真实设备证据重新取证。
...
- **P0 Reader 主链已覆盖**：工具层/面板状态机、正文排版、移动点击与连续滚动、设置、书签、正文
  ...
  `2bbb276` 的一次前端重试、`e1631d0` 的整书抓取门和 `0ecc4d9` 的同章 gate 修订均未独立通过真机
  验收；历史差异已证明整书 gate 会把
  相邻章节排在同一队列，而浏览器 12 秒超时短于单次书源 15 秒预算。`981d400`/`99cfc88`/`0ecc4d9`
  已按合同、红测、实现把 gate 收缩到同章，相邻章恢复并行，真实 lifecycle 409、状态重读和 cache
  复用保持。第三次反馈又确认 shelf/temporary 章节 GET 错误继承通用 12 秒，且同章新 generation 会
  复用被旧订阅取消的 Promise；`1512534`/`46e4933`/`c7fbf73` 已恢复上游 30 秒正文预算并拆分共享
  transport 与调用方订阅，13 秒延迟正文已通过三视口 Chromium 和可信 Actions，仅待真机复验。
  本地书占位回填、完整 EPUB fragment 身份和左上章节运行标注仍维持 `e1631d0` 的实现，不因本次
  请求预算/所有权修复重开。
...
- **P2 已覆盖的高风险项**：账号/overlay/cache 隔离、原子阅读进度、删除收敛、替换规则、书签、
  ...
  以及用户资产 rooted stage/publish/delete、Book/Setting 精确引用协调和 portable 同句柄导出均已实现；
