...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置和夜间面此前均已专项发布；**Reader 移动端阅读内书架宽度偏差已修复并重新包含在 `2ea6e8c`/`latest`**：主面板为 100vw，最终左右 inset 从错误的 24px 恢复为上游 20px，390/360 列表为 350/320px；1024 强制手机模式保持上游四列。本轮完整 Reader 与普通书架浏览器合同再次通过。最后一次线上只读核验发现部署仍为 `7971e23`，390px 下仍呈现旧 24px/342px 几何，判定为部署滞后。 | [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md) 状态 `aligned / Docker-republished / deployment-update-required`；当前需将线上容器真正 pull 并 force recreate 到 `2ea6e8c`，仅 restart 不会替换旧镜像。其它已关闭 Reader 专项不因部署漂移自动重开。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 P2 parser 预算、Reader 移动书架、WebSocket 边界与备份事务 worker 已由本机作为 `2ea6e8c` 发布**：focused/full/race/frontend 706/706/build/vet/diff、Reader/书架多视口及 amd64/arm64 推送成功。 | `2ea6e8c`/`latest` OCI index `sha256:678b019c34ac1f92a38dbd650de48867002ae6425a4206aff2e8f315e189d6ac`。fresh volume 的 portable v1/v2 assets、cross-user、restart 和 historical TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过。 |

## 当前整体进度快照（2026-08-09，`2ea6e8c`）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **89%**。该数字表示“已完成固定基准复审、
测试先行实现并至少发布过一次”的覆盖度，不表示剩余 11% 只是简单收尾。
...
- **P2 已覆盖的高风险项**：账号/overlay/cache 隔离、原子阅读进度、删除收敛、替换规则、书签、
  ...
  章节与历史读取预算，以及 server-only WebSocket/recipient scope 和备份事务 worker。
- **尚未完成的主线**：尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  以及后续真实设备反馈暴露出的上游可见偏差。最后一次线上核验仍运行 `7971e23`，需部署
  `2ea6e8c` 后再验收 Reader 内书架。上述项目继续按“合同→失败测试→实现→浏览器/新旧卷→本地
  Docker”推进。
