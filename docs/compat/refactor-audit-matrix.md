...
| 书源与搜索/探索结果 | `Index.vue#toDetail`、`Index.vue#debugBookSource`、`Index.vue:720-890` 的书源管理器、`bookSourceDebug/*`、`Debugger.kt`、`Reader.vue#getCatalog`、`App.vue#getBookContent`、`Explore.vue`、`BookSource.vue`、`BookController.kt` | `SourceManager.vue`、`OverlaySources.vue`、`SourceDebug.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`ExploreWorkspacePopover.vue`、`backend/api/source_debug.go`、`services/sourcedebug`、`remote_reader.go`、`services/remotereader`、Go source APIs | 书源 owner/API/解析主链、搜索/探索、临时 Reader 和独立书源调试器继续保持已发布；**可见书源管理器第二轮复审判定为结构级错误，当前进入 `contract-complete / implementation-pending`**：旧 P1-C 对移动卡片、结构化 Drawer、管理器内 import/remote/debug 和额外 batch/health 控件的“允许差异”已撤销。必须恢复单 table、动态正常/失败标题、固定列/分组/footer、通用 reader-dev JSON 编辑器及四个独立 Index 流程，同时保留多用户事务、安全抓取、失败缓存、同步和旧 URL。 | [`source-manager-fixed-baseline-second-audit-p1-contract.md`](source-manager-fixed-baseline-second-audit-p1-contract.md) 已锁定源码、状态、API/data、安全、红测试和浏览器门禁；下一步先提交合同，再写当前实现必失败的结构/API 测试。独立调试器仍由 [`source-debug-fixed-baseline-second-audit-p2-contract.md`](source-debug-fixed-baseline-second-audit-p2-contract.md) 约束并保持 `Docker-published`。 |
