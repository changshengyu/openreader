...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 持续复核；新增两个 must-fix**：连续跨章事务已完成并发布，但 2026-07-18 用户实测和源码复审确认，竖向点击动画每帧仍执行重型 DOM/进度同步，且 page 正文流缺失上游 `加载下一章` 入口。 | 先按 [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md) 增加失败测试，再实施动画期间延迟同步、完成后单次结算和章末入口；三视口触控回归后发布。 |
