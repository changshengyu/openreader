...
| 书源与搜索结果 | `Index.vue`、`Explore.vue`、`BookSource.vue`、`BookSourceController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`SourceSwitchPanel.vue`、`backend/api/sources.go` | **P1-C 工作台收敛、P2 失效缓存、Reader 换源、CSS/JSONPath/XPath 主解析链和脚本能力透明化均已实施验证并发布 `cad1f40`**；动态 Header、`loginCheckJs`、规则脚本/模板和 WebView 仍可无损保存，但不会被默认选为当前可运行源，编辑/debug 会提前显示安全边界；全局书源 + 用户编辑权限是已记录的数据模型重设计。 | [`booksource-script-transparency-p2-contract.md`](booksource-script-transparency-p2-contract.md)；后续只继续抽取尚无合同的新解析入口，不恢复宿主内任意脚本执行。 |
