...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`WebDAV.vue` | `OverlayBookImport.vue`、`LocalStore.vue`、`WebDAVBrowser.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/services/localbook/*`、`engine/*_parser.go` | **P1-E1、P1-E2 已完成**：失败预览可用同一暂存 token 重试；两来源收敛到根工作台唯一导入 controller，复刻单本直达、多本批量/逐本选择、取消和分组默认值，并保持逐项 token 写入。额外目录操作、完整格式和旧卷复验仍未开始。 | `docs/compat/workspace-storage-import-p1e-contract.md` → P1-E3 额外流程审查 → P1-E4 真实文件夹/卷/Docker smoke。 |
