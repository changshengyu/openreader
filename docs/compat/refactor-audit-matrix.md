...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category/remote-add、章节正文/换源/本地 cache 与用户资产 lifecycle 已关闭。当前第 49 项确认管理员删除用户工作区仍以词法路径调用 `RemoveAll`，中间 symlink 可删除配置根外数据，旧用户名安全投影碰撞也可误删存活用户目录。 | [`admin-user-workspace-cleanup-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md`](admin-user-workspace-cleanup-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md) 已完成固定上游/当前反例/API/数据/测试门 inventory，状态 `inventory-complete / implementation-pending`；下一步严格先提交旧实现红测，再以 rooted parent identity detach 实施。 |
...
## 当前整体进度快照（2026-09-14，管理员工作区清理已完成取证）
...
已按 `478654a`/`947dfcb`/`3e8cec7` 完成实现、本地回归和可信双架构发布。第 49 项已从管理员删除
用户工作区确认 lexical `RemoveAll` 的根外删除反例和历史用户名投影碰撞，当前完成合同取证，待严格
按红测再实施；不能凭当前组件或旧日志推定其它 action 完成。
