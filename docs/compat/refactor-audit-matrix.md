...
| 用户、备份、RSS、替换规则、书签 | `UserManage.vue`、`AddUser.vue`、`BaseController.kt#saveUserSession`、`WebDAV.vue`、`Rss*`、`ReplaceRule*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`backend/api/auth.go`/`admin.go`、`WebDAVBrowser.vue`、`RSSManager.vue`、`OverlayReplaceRules.vue`、`OverlayBookmarks.vue`、Go services | RSS、替换规则和书签合同继续成立；P2-S4 owner 隔离和用户书源动作已发布。2026-07-28 UserManage 复审删除移动卡片/合并权限列，恢复桌面移动共用表格、固定移动首列、上游列/底栏顺序和显式取消；登录成功持久更新 canonical `lastLoginAt`，`lastActiveAt` 仅留作兼容别名。 | **UserManage 已随 `f44447f` 发布**：Go、frontend 641/641、build、1440/1024/390/360 浏览器以及新旧卷门通过；index `sha256:a3fb3e55d0e7002390f34e2bb30fb54adda9b5a64e2a8e1ca87b0f7941f7a64c`。其余模块继续按各自专项合同复审。 |
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十三次移动点击候选已实施并发布 `49a273e`；夜间实际文字层修复已发布 `2c4ede6`，两项均待实机复验**：移动点击沿用已记录的绘制/定位优化；内置夜间强制 shell/page/普通正文后代和 EPUB iframe 根层为纯黑，实际文字后代为纯白透明面，并可逆恢复日间 EPUB 作者背景。 | 移动点击继续按 [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md) 实机复验；夜间按 [`reader-night-contrast-p0-contract.md`](reader-night-contrast-p0-contract.md) 用 `2c4ede6` 或后续 `latest` 复验。custom 夜间保持用户资源与对比保护，是明确允许差异。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 `f44447f` 已从本机完成并发布**：继承 `2c4ede6` 的夜间修复，并新增 UserManage/last-login 契约；frontend 641/641、Go、build、1440/1024/390/360 用户管理门、新卷、历史 TXT/EPUB/UMD/CBZ/相对缓存、portable v1/v2、跨用户、恢复与重启均通过。`f44447f`/`latest` 共同指向 `sha256:a3fb3e55d0e7002390f34e2bb30fb54adda9b5a64e2a8e1ca87b0f7941f7a64c`。 | `openreader-regression`、`openreader-docker-release`；线上部署后必须确认 `/api/health` 为 `f44447f`，继续保留 Reader 真实手机体感与夜间设备复验，不能用自动门替代设备结论。 |
