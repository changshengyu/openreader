...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue` | **Dialog/BookInfo 主流程已验证，但 BookManage 缓存动作于 2026-07-18 重新打开**：当前只缓存 20/100 章、单全局任务、浏览器不可取消、删除无确认且暴露非上游 batch/JSON 菜单；不能用已对齐的 Dialog shell 概括缓存语义。 | [`book-management-cache-p2-contract.md`](book-management-cache-p2-contract.md)；先补整本、多任务/取消、确认、动作顺序和规范 SSE 计数，再重跑 BookManage/Reader cache 三视口。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 持续复核；移动点击翻页第二批已发布 `18d9183`**：移动普通文本 page 点击改为正文合成 transform，结束/拖动取消只提交一次真实位置；触摸仅在确实拖动后取消，快速同向点击有一页有界缓冲。原生 touch/wheel、EPUB、图片和其他模式不走该合成路径。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；前端 479、Go/build、双移动 cadence、desktop/mobile、continuous、image、真实 EPUB 及 Docker 卷/备份门禁通过，镜像双架构摘要已记录，等待用户体感验证。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `9f19d21` 已验证**：前端 474/474、Go 全量、生产构建、Reader 与真实双客户端三视口、真实 EPUB/CBZ、本地新/历史 volume/backup、远端 amd64/arm64 manifest 均通过；以后每次发布仍必须以新提交重跑。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:433f64126c65bc82b456da2be8e1cea644b1c53affcfbb98e3a8a4326cfc57cb`。 |
