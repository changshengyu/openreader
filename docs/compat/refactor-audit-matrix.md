...
| 书源与搜索/探索结果 | `Index.vue`、`Explore.vue`、`BookSource.vue`、`BookSourceController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultGroups.vue`、`ExploreWorkspacePopover.vue`、`SourceSwitchPanel.vue`、Go source APIs | **书源 owner/API/解析主链保持已发布；搜索/探索可见工作区第二轮已重新打开**。固定上游要求 single/multi 两种可见设置、同一 shelf title、扁平固定 380px result grid、结果 JSON editor/add 双动作，以及 desktop 600px/top0、mini 100vw 非全高的 Explore popover。当前 source regroup 富卡片、独立 page header/empty/loading、三种可见搜索模式、520px/100vh modal chooser 和 accordion 均为错误重构。 | [`index-search-explore-visible-workspace-fixed-baseline-second-audit-p1-contract.md`](index-search-explore-visible-workspace-fixed-baseline-second-audit-p1-contract.md) 状态 `audit-complete / implementation-pending`；先替换冲突测试再改 UI。P2-S1…S4 owner 隔离、稳定 cursor、JWT REST、临时 Reader 与安全 Go exploreUrl 解析不得回退。 |
