...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`main.js#getCover`、`BookController#getBookCover` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、`stores/bookshelf.js`、`useSync.js` | **BookManage 第二轮已发布 `af4e2a4`；BookGroup 第二轮审计已完成但必须重建**。BookGroup 当前双表、自绘/整行选择、独立拖动列、计数、switch 文本、extra empty、手机 footer、扩展文案、无 normal gate/动态几何/每次 force load，以及失效 token 自动切组均偏离固定上游。数据复审还实证 reader-dev 与 categories-only 恢复会丢失隐藏自定义组的 `show=false`。统一投影、四内置、多用户、多对多、混合事务排序和同步/备份架构继续保留。 | BookManage 证据见 [`book-management-fixed-baseline-second-audit-p2-contract.md`](book-management-fixed-baseline-second-audit-p2-contract.md)，状态 `aligned / Docker-published`。BookGroup 见 [`book-group-fixed-baseline-second-audit-p2-contract.md`](book-group-fixed-baseline-second-audit-p2-contract.md)，状态 `audit-complete / must-fix / implementation pending`；旧 `book-group-p2-contract.md` 的前端完成结论已被取代。BookInfo 仍见 [`bookinfo-action-ownership-cleanup-p1-contract.md`](bookinfo-action-ownership-cleanup-p1-contract.md) 与 [`book-cover-proxy-p2-contract.md`](book-cover-proxy-p2-contract.md)。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十三次移动点击已发布 `49a273e`；第八轮夜间实际文字承载层修复始于 `6fde5ab` 并已包含在当前 `af4e2a4`**：黑色内容面按最终“不透明纯黑页面 + 无背景图”接管；普通正文 `p/span/mark` 与 EPUB 实际后代节点自身均显式纯黑/纯白，不再依赖祖先透明穿透，退出夜间完整恢复作者样式。非黑自定义主题与背景图保持不变。 | [`reader-night-contrast-p0-contract.md`](reader-night-contrast-p0-contract.md)：frontend 652/652、Go/build、普通 Reader 与真实 EPUB 1440×900/390×844/360×800 及新旧卷通过。当前 `af4e2a4`/`latest` index 为 `sha256:64f594571618c232113b4211054b7c14718d3fddddc489f267333109fe20f29e`；部署端 pull/force-recreate 且 `/api/health` 返回 `af4e2a4` 后才代表已升级。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 BookManage 第二轮与夜间文字节点内容面修复已由本机作为 `af4e2a4` 发布**：frontend 661/661、Go、build、BookManage mock 五视口、真实 Go/SQLite 三视口、新卷 portable v1/v2 assets/跨用户/重启和历史 TXT/EPUB/UMD/CBZ/相对缓存/owner isolation 均通过。 | `af4e2a4`/`latest` 共同指向 amd64/arm64 OCI index `sha256:64f594571618c232113b4211054b7c14718d3fddddc489f267333109fe20f29e`；等待设备验收。 |
