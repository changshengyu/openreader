...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`components/workspace/LocalStoreManager.vue`、`OverlayLocalStore.vue`、`WebDAVBrowser.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/api/localstore*`、`backend/api/webdav*`、`backend/services/webdavfs`、`services/localbook/*`、`engine/*_parser.go` | P1-E1…P1-E5、P2 parser 预算和 TXT 默认目录规则已发布。LocalStore 与 WebDAV mounted import/restore 文件系统/请求边界均已测试先行实施并发布：actual-read single JSON、200 项完整计划、symlink/special-file 拒绝、opened regular read、token-only source independence 和 restore private snapshot 已关闭；不重开原生 DAV 协议或 ZIP 事务。 | LocalStore 与 [`webdav-import-restore-filesystem-request-boundary-fixed-baseline-second-audit-p2-contract.md`](webdav-import-restore-filesystem-request-boundary-fixed-baseline-second-audit-p2-contract.md) 均通过 focused/full/race/vet、frontend 740/740、build、宿主/candidate HTTP、三视口工作台及 fresh/historical/portable 卷门。状态 `aligned / Docker-published / awaiting-device-verification`；`65a9870`/`latest` OCI index 为 `sha256:255c81b43dbb7f49c707d6c609b920aa183b730401ad1c1ca32157eb0a945c71`。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。已关闭的 auth/admin/settings/group/book/source/bookmark/RSS/asset/LocalStore 及 WebDAV mounted import/restore 动作不重开。WebDAV 三动作的 bounded JSON、cardinality、caller-rooted opened-file 与 restore snapshot 已在 `616a076` 关闭并随 `65a9870` 发布。 | [`rest-action-coverage-fixed-baseline-p2-contract.md`](rest-action-coverage-fixed-baseline-p2-contract.md) 第 17 节及 WebDAV 专项合同状态 `aligned / Docker-published / awaiting-device-verification`。direct/source multipart、remote-work JSON、progress 与其它 batch JSON 仍待重新差集排序。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | `65a9870` 通过 frontend 740/740、Go full/focused/race/vet、build、LocalStore/WebDAV 宿主与 candidate 真实 HTTP、三视口工作台、fresh/historical/portable 三卷、跨用户与重启；GHCR 回拉 health revision 正确。 | 本机发布 `65a9870` 与 `latest`；amd64/arm64 OCI index `sha256:255c81b43dbb7f49c707d6c609b920aa183b730401ad1c1ca32157eb0a945c71`。用户生产环境当前运行提交仍未知，发布不等于服务器已升级。 |

## 当前整体进度快照（2026-08-16，`65a9870` 发布签收 pass）
