...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十批 browser-validated / awaiting device verification**：animator 已按固定上游改为回调执行时钟，首回调不再因陈旧 rAF 时间戳写回起点；正文父层移除 `filter:brightness(...)`，以无事件黑色遮罩保留亮度。三文字模式、移动两尺寸、桌面、iPad、连续、图片和真实后端 EPUB 已通过；原生手指/滚轮连续滚动继续保留。 | [`reader-ipad-panel-dismiss-p0-contract.md`](reader-ipad-panel-dismiss-p0-contract.md)、[`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；发布新 Docker 后必须以真实手机体感决定是否关闭该项。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `fe32c00` 已验证**：前端 532/532、Go 全量、生产构建、移动三视口文字模式/工具层/iPad/连续跨章合同，以及新卷和历史 TXT/EPUB/UMD/CBZ/相对缓存、用户隔离、重启、便携恢复均通过；镜像由本机完成双架构构建与上传，远端 arm64 拉回后重复通过卷门禁。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:09394c2183212b130db1eb8150952ca6ce688c5baf9ea8d6d47136f93db90617`，amd64 `sha256:a3205b085bda3faf78426098d4aa3fba3b70e70bb2e9ea73f021850110c5c77e`，arm64 `sha256:74048c04ca9e581439eb2255c74aca9b26f43b149391e75a3a0bcfb8562cf27c`。 |
