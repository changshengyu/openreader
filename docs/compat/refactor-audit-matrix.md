...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置和夜间面此前均已专项发布；Reader 移动端阅读内书架横向仍为 100vw、左右 20px、390/360 列表 350/320px。多条目 Grid 行轨坍缩已由 `a445121` 修复。**2026-08-11 又确认内联章节缓存状态机尚未第二轮签收**：完整目标区间、本地书、取消即时恢复、精确反馈和陈旧任务隔离存在固定上游偏差。 | 书架见 [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md)，状态仍为 `aligned / Docker-published / awaiting-device-verification`。章节缓存见新增 [`reader-inline-chapter-cache-fixed-baseline-second-audit-p0-contract.md`](reader-inline-chapter-cache-fixed-baseline-second-audit-p0-contract.md)，当前为 `inventory-complete / implementation-pending`；不得用旧内联几何 smoke 代签引擎状态机。 |
...
2026-08-11 的下一长尾切片已选定 Reader 内联章节缓存：固定上游合同取证完成，确认完整后续区间、
已入架本地书、取消即时恢复、精确完成反馈和跨书/跨身份迟到任务隔离为 `must-fix`；临时 Reader
继续保持 `no-store`。当前状态仅为 `inventory-complete / implementation-pending`，实现需先补失败测试。
