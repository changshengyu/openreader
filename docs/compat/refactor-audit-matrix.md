...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 持续复核；移动点击翻页第二批已完成代码验证**：移动普通文本 page 点击改为正文合成 transform，结束/拖动取消只提交一次真实位置；触摸仅在确实拖动后取消，快速同向点击有一页有界缓冲。原生 touch/wheel、EPUB、图片和其他模式不走该合成路径。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；前端 479、Go/build、双移动 cadence、desktop/mobile、continuous、image 及真实 EPUB 合同通过，等待本地 Docker/卷门禁和用户验证。 |
