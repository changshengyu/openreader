...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 第六次实机反馈修复已发布 `a92f6a1`**：正数竖向点击的最终位置先呈现，章节/窗口/布局/进度结算移到后续可取消任务；`0ms`、一次缓冲、原生手势和三模式语义不变。4× CPU 最长动画 rAF 从约 35ms 降到 0.88ms，draw 最大约 0.33ms。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；前端 492、Go/build、三模式双移动、desktop/mobile、continuous 和远端/卷门禁通过，等待本批 Docker 真机体感复验。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `a92f6a1` 已验证**：前端 492/492、Go 全量、生产构建、Reader 三竖向模式双移动逐帧、文本/移动/连续、4× CPU trace、历史 TXT/EPUB/UMD/CBZ/相对缓存 volume/backup、远端 amd64/arm64 manifest 均通过；以后每次发布仍必须以新提交重跑。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:704563d2f5d4fe8224977cd0e3d04f79cb6296b44f1300ddcfa30eebecf8c8b3`。 |
