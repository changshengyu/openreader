...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book patch/category 已发布；Category patch 列所有权亦已随 `090a643` 关闭：request-context transaction 内重读 owner target，只更新显式 `name/color/show`，空 patch 不 UPDATE，并发删除不复活。 | [`category-patch-write-lifecycle-fixed-baseline-second-audit-p2-contract.md`](category-patch-write-lifecycle-fixed-baseline-second-audit-p2-contract.md) 通过旧实现红测、focused/race/full/vet、五视口真实 BookGroup 与可信卷门；状态 `aligned / regression-validated / Docker-published / awaiting-device-verification`。旧 BookGroup wire/UI/data 合同不重开。 |
...
## 当前整体进度快照（2026-08-31，Category patch lifecycle published）
...
并发提交亦已关闭；Category patch full-row `Save` 差集现亦已测试先行实施、发布。剩余约 1% 包含后续逐路由
