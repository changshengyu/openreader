...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。frontend/static/router 与 public 子树已随 `bf114a6`、`5163262` 关闭；认证会话生命周期亦随 `a0edce3` 关闭：REST/WebDAV Bearer/WS 共用用户、代次和持久 session 校验，logout、改密和删用户撤销已落地。 | [`authenticated-session-lifecycle-fixed-baseline-second-audit-p2-contract.md`](authenticated-session-lifecycle-fixed-baseline-second-audit-p2-contract.md) 已通过 Go full/race/vet、frontend 742/742、build、四视口浏览器、真实 API、fresh/historical/portable 卷和 GHCR 回拉；状态 `aligned / Docker-published / awaiting-device-verification`，OCI index `sha256:5d7fe23ba96107c5c545e9e44815514fe277e5a6f83eb25cb006859c5d515d78`。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release workflow | `a0edce3` 通过 frontend 742/742、Go full/race/vet、build、Compose、四视口重新认证、真实 API 会话合同与 GHCR 回拉容器；受信 Actions run `32914105929` 通过原生候选、fresh portable 和 historical volume 门禁后发布。 | amd64/arm64 发布 `a0edce3`/`latest`，OCI index `sha256:5d7fe23ba96107c5c545e9e44815514fe277e5a6f83eb25cb006859c5d515d78`；manifests 分别为 `sha256:dc50d7db9eb1bddb8c3a094fd507fa1e1f4631b633cacc58d635070b1ba606ab`、`sha256:1785dee2ec3d62894a20b35c33535fc1bd2c9167585b12bc567b71c71bff13df`。用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-08-26，认证会话生命周期已发布）
...
router 失败分流、public 静态子树和认证会话生命周期亦已关闭。剩余约 1% 为后续逐路由 action 审计、
长尾固定基准复审与真实设备证据，不能从 direct binder 差集为空推导完成。
