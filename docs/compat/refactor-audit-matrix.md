...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue` | 正文缓存/图片能力保留；2026-07-19 BookGroup 全量复审确认旧“已对齐”只覆盖 Dialog 外壳与自定义分类，四内置分组、音频筛选、混合排序、内置改名/显隐、设置模式添加/编辑、当前 tab 持久化及 `bookGroup.json` 均缺失，现进入测试先行重建。 | [`book-group-p2-contract.md`](book-group-p2-contract.md) 是当前 BookGroup 权威门禁；缓存合同继续独立有效。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **iPad Pro 宽屏主面板修复已发布 `b8b70f9`**：`isMobileReader` 现在统一控制根布局、桌面/移动工具和四个主面板；1024×1366、1366×1024 不再出现移动面板已打开但顶部关闭工具被宽度 CSS 隐藏。此前 `a92f6a1` 点击翻页最终帧修复保持不变。 | [`reader-mobile-controls-p0-contract.md`](reader-mobile-controls-p0-contract.md)、[`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；五视口真实浏览器、前端 494、Go/build 和卷门禁通过，等待 iPad 实机复验。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/auth.go`、前端 `api/*.js` | **按动作逐项复审；阅读进度切片已完成并发布**：GET/PUT 稳定路径与兼容冲突响应保留，服务端规范化章节身份、CAS 只广播赢家，并在现存用户 WebDAV 目录镜像。其它模块仍以专项 API 合同为证，不能用本行概括宣称全 REST 完成。 | [`reading-progress-p2-contract.md`](reading-progress-p2-contract.md)；随后选择下一未审查 API 组。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `b8b70f9` 已验证**：干净提交前端 494/494、Go 全量、生产构建、Reader 桌面/双手机/iPad Pro 双方向、普通与历史 TXT/EPUB/UMD/CBZ/相对缓存 volume/backup 均通过；远端 amd64/arm64 manifest 已确认。GHCR Docker daemon 拉取端临时 502 已单独记录，不冒充镜像失败。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:2711516edc2587ef14cdf4f84be26f950365928ff10f5a3f5d3004a084ef5759`。 |
