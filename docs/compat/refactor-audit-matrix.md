...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置、夜间面和内联章节缓存均已专项发布。**当前新差集是主正文请求生命周期**：固定上游以当前 `bookUrl/index` 拒绝迟到响应；OpenReader 主 loader 无 generation/AbortSignal，换源后旧 Promise 仍可覆盖正文或重建已清 browser cache。 | [`reader-chapter-content-request-lifecycle-fixed-baseline-second-audit-p0-contract.md`](reader-chapter-content-request-lifecycle-fixed-baseline-second-audit-p0-contract.md) 已固定前端 current-scope-only commit 与后端持久 variable/cache snapshot guard；状态 `inventory-complete / tests-and-implementation-pending`。已签收布局、连续/点击规则、格式、工具层、换源面板和缓存可见 UX 不重开。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category/remote-add 列所有权均已实施。当前共享章节 loader 的持久提交仍未复验 source/book/chapter/variable snapshot，legacy path 归一化还会 full-row `Save` Chapter。 | 新章节正文生命周期合同要求 request-context、owned-column、current-snapshot commit，陈旧结果安全 409 且零 failure/cache/variable 副作用；状态 `inventory-complete / tests-and-implementation-pending`。既有 parser、缓存 rooted 文件系统、整本缓存和换源可见合同保持关闭。 |
...
## 当前整体进度快照（2026-09-09，Reader chapter-content lifecycle inventory）
...
触发可信工作流。当前已确认的下一项 must-fix 是 Reader 主正文迟到响应和共享 loader 的持久
variable/cache 提交生命周期；剩余约 1% 还包含后续逐路由 action 审计、长尾固定基准复审与真实设备证据，
...
- **P0 Reader 主链已覆盖**：工具层/面板状态机、正文排版、移动点击与连续滚动、设置、书签、正文
  ...
  后端 persistent-variable/cache stale commit 是本轮新 inventory，不推翻已签收的可见模块。
