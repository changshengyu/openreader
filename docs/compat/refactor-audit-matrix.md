...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`main.js#getCover`、`BookController#getBookCover` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、`stores/bookshelf.js`、`useSync.js` | **书架/BookInfo 业务状态已完成，但 2026-07-27 发现远程封面传输仍是结构级缺口**：当前浏览器直连第三方且 CSS background 失败后为空；固定上游使用同源 `/cover` 缓存代理和 `noCover` 回退。裁决为 P2 must-fix，但公开任意 URL 的上游 SSRF 形态不得照搬。 | 既有书架合同保持；新增 [`book-cover-proxy-p2-contract.md`](book-cover-proxy-p2-contract.md) 先以不透明 capability、原始 URL/投影分离、SSRF/大小/缓存边界和三视口失败回退测试重新实现。 |
...
| 用户、备份、RSS、替换规则、书签 | `UserManage.vue`、`WebDAV.vue`、`Rss*`、`ReplaceRule*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`WebDAVBrowser.vue`、`RSSManager.vue`、`OverlayReplaceRules.vue`、`OverlayBookmarks.vue`、Go API/services | **已抽取 P2 行为均完成并进入发布镜像**：用户账户/权限/安全删除、RSS 三层 Dialog、替换规则、书签、普通备份 bridge 与 portable v1/v2 均有专项合同。第二套备份管理器已删除；单复数 alias、上游字段、原子生成、内容事务、进度新旧合并和 `canEditSources` 权限已验证。全局书源加编辑权限是明确的数据模型重设计，不伪造上游单用户书源删除按钮。 | [`user-management-p2-contract.md`](user-management-p2-contract.md)、[`backup-restore-fixed-baseline-p2-contract.md`](backup-restore-fixed-baseline-p2-contract.md)、[`portable-appearance-assets-p2b-contract.md`](portable-appearance-assets-p2b-contract.md)；最新三视口工作台和新旧卷门禁随 `54a528f` 完成。 |
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **正文/翻页第十一次候选已通过自动门禁、等待实机；外观资产 P2-A/P2-B 均已发布**：portable v2 携带当前用户实际引用的封面/背景/字体并跨 user ID 重写，普通 ZIP 与已有 v1 不变。移动 page/scroll/scroll2 继续保持根文档滚动与点击分页。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md) 保持；P2-A/P2-B 证据见 [`reader-appearance-assets-p2-contract.md`](reader-appearance-assets-p2-contract.md) 和 [`portable-appearance-assets-p2b-contract.md`](portable-appearance-assets-p2b-contract.md)。`54a528f` 已通过新旧卷并发布；点击体感仍等待用户实机结论。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；阅读进度与外部 WebDAV 协议均已发布，远程封面动作新发现 must-fix**：当前没有 `getBookCover` 等价动作。将保留 OpenReader `/api` 与多用户边界，以签发式图片 capability 替代上游公开 `path` SSRF 接口。其它模块仍只能以专项 API 合同为证。 | [`reading-progress-p2-contract.md`](reading-progress-p2-contract.md)、[`webdav-protocol-p2-contract.md`](webdav-protocol-p2-contract.md)、[`book-cover-proxy-p2-contract.md`](book-cover-proxy-p2-contract.md)。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **`54a528f` 已从本机完成并发布**：frontend 558/558、Go、build、外观资产/BookInfo/备份工作台真实三视口、Reader 桌面/手机/iPad、新卷与历史卷 v1/v2/旧本地书/用户隔离均通过；`54a528f`/`latest` 共同指向 `sha256:047f9636a78604a1d5320da2972d0b16256b95d47253320b79095eaf6101a571`。 | `openreader-regression`、`openreader-docker-release`；等待用户实机验证外观资产事务和既有根滚动点击体感，不能以自动门禁替代设备结论。 |
