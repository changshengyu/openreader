...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；已关闭模块不从旧日志重开**。books.go 六个 JSON control、ReplaceRule 五路和备份 generation lifecycle 均已发布；`backend/api` direct Gin JSON binder 差集为空。backup list/download 已按 `b9deec2` 合同与 `d7810ca` 旧实现红测完成严格 ZIP 名称、scoped same-file open、同句柄 format/download 与固定安全错误实现。 | Generation 合同状态 `aligned / Docker-published`。[`backup-list-download-filesystem-request-boundary-fixed-baseline-second-audit-p2-contract.md`](backup-list-download-filesystem-request-boundary-fixed-baseline-second-audit-p2-contract.md) 状态 `aligned / regression-validated / Docker-pending`；真实 HTTP 与卷门待执行，generation/restore/格式/唯一工作台不重开。 |
...
- **尚未完成的主线**：其余尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  ...
  ReplaceRule 五路与备份生成均已完成合同、红测、实现、runtime、卷门和发布；backup list/download
  rooted opened-file 边界已完成合同、红测、实现与全量回归，尚待 runtime/卷门/发布，其它 action 长尾
  继续逐项取证。
  `c74be70` 已发布但尚待
  服务器部署；移动书架在
