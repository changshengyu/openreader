...
| 用户、备份、RSS、书签 | `UserManage.vue`、`AddUser.vue`、`BaseController.kt#saveUserSession`、`WebDAV.vue`、`Rss*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`backend/api/auth.go`/`admin.go`、`WebDAVBrowser.vue`、`RSSManager.vue`、`components/rss/*`、`services/rss`、`OverlayBookmarks.vue` | Bookmark 可见流程、会话/owner 隔离和 RSS 第二轮保持已发布；**Bookmark 写入边界又随 `a9a55db` 关闭**：四路 actual-read 单 JSON、2,000 原始项、显式 note-only payload/SQL、fresh row 和并发删除不复活均已测试先行实施。UserManage 单字段 payload、字段级 busy/rollback 和显式列更新继续保持。 | [`bookmark-write-boundary-fixed-baseline-second-audit-p2-contract.md`](bookmark-write-boundary-fixed-baseline-second-audit-p2-contract.md) 状态 `implemented / regression-validated / Docker-published / awaiting-device-verification`；full/race/vet、frontend 740/740、build、Reader 浏览器、宿主/容器/回拉 HTTP、SQLite trigger 与 fresh/historical volume 通过；OCI index `sha256:944a85881170bc900c1fda0acb885bedc1dc4b17ed4e635305988163e1b635e5`。UserManage/RSS 继续见各专项合同。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。公开 auth、管理员 user、用户配置、BookGroup/Category、Book、BookSource 与 Bookmark 写边界均已关闭并发布。Bookmark 现在拒绝超限/多 JSON/2,001 项和非显式 note patch，且不再用全行 `Save` 覆盖并发 context 或复活删除行。 | Bookmark 专项合同当前 `implemented / regression-validated / Docker-published / awaiting-device-verification`；下一 REST 长尾必须从固定上游重新提取动作和反例后再选，不能凭旧日志重开已签收模块。 |
