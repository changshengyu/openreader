...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`WebDAV.vue` | `OverlayBookImport.vue`、`LocalStore.vue`、`WebDAVBrowser.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/services/localbook/*`、`engine/*_parser.go` | **P1-E1、P1-E2、P1-E3 已完成；P1-E4 固定 EPUB 目录纠正也已实现并通过自动化/三视口浏览器门禁**：新目录按 href 去重，支持 TOC-only resource 和实际标题副作用；纯 `toc` 空目录语义、单 resource Reader/搜索、升级前 staged snapshot 与历史 fragment rows 均有契约。显式刷新不改 archive，并按 resource path 重绑进度/书签。 | `docs/compat/epub-fixed-baseline-catalog-reader-contract.md` → 本批历史 volume/portable backup + Docker；其余格式/工作台后续矩阵不重开已完成 EPUB 合同。 |
...
| Reader：EPUB、漫画/CBZ、音频、连续跨章、TTS | `Reader.vue`、`Content.vue`、本地格式解析类 | `ReaderChapterContent.vue`、`ReaderEpubContent.vue`、`ReaderAudioContent.vue`、格式 parser / cache | **EPUB 与 CBZ 固定基准切片已完成实现和三视口验证**：CBZ 新导入预建不可变派生图片树，warm read 不重哈希/遍历源 archive，GET/HEAD/Range 流式；图片布局与普通漫画控制状态已分离，CBZ 保留 mode/自动阅读/TTS。音频、连续跨章和 TTS 引擎内容仍按各自合同继续复核，不由本批扩大签收。 | [`epub-fixed-baseline-catalog-reader-contract.md`](epub-fixed-baseline-catalog-reader-contract.md)、[`reader-cbz-fixed-baseline-p0-contract.md`](reader-cbz-fixed-baseline-p0-contract.md)；本批仅剩历史卷/portable backup 与 Docker 闸门。 |
