...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`components/workspace/LocalStoreManager.vue`、`OverlayLocalStore.vue`、`WebDAVBrowser.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/api/localstore*`、`backend/api/webdav*`、`backend/services/webdavfs`、`services/localbook/*`、`engine/*_parser.go` | P1-E1…P1-E5、P2 parser 预算和 TXT 默认目录规则已发布。LocalStore 与 WebDAV mounted import/restore 文件系统/请求边界均已测试先行实施并回归：actual-read single JSON、200 项完整计划、symlink/special-file 拒绝、opened regular read、token-only source independence 和 restore private snapshot 已关闭；不重开原生 DAV 协议或 ZIP 事务。 | LocalStore 与 [`webdav-import-restore-filesystem-request-boundary-fixed-baseline-second-audit-p2-contract.md`](webdav-import-restore-filesystem-request-boundary-fixed-baseline-second-audit-p2-contract.md) 均通过 focused/full/race/vet、frontend 740/740、build、宿主 HTTP 和三视口工作台。WebDAV 实现 `616a076` 已推送；两项共用的 Docker 卷门及正式发布仍待执行，正式 GHCR 暂仍是 `be83a0f`。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。已关闭的 auth/admin/settings/group/book/source/bookmark/RSS/asset/LocalStore 及 WebDAV mounted import/restore 动作不重开。WebDAV 三动作的 bounded JSON、cardinality、caller-rooted opened-file 与 restore snapshot 已在 `616a076` 关闭。 | [`rest-action-coverage-fixed-baseline-p2-contract.md`](rest-action-coverage-fixed-baseline-p2-contract.md) 第 17 节及 WebDAV 专项合同状态 `implementation-complete / regression-validated / Docker-release-pending`。direct/source multipart、remote-work JSON、progress 与其它 batch JSON 仍待重新差集排序。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | `616a076` 已通过 frontend 740/740、Go full/focused/race/vet、build、WebDAV 宿主真实 HTTP 与三视口工作台；同时包含先前未发布的 LocalStore 边界实现。下一步用当前 HEAD 构建本地 candidate 并顺序执行 fresh/historical/portable 三卷门。 | 文档提交前正式 GHCR 仍是 `be83a0f` 与 `latest`，OCI index `sha256:e1f31f3dd728bc27fbc89bbc8c21f81e8c5511c5e99196891feb21cd47138b73`；新 candidate/卷门通过前不发布。用户生产环境当前运行提交仍未知，发布不等于服务器已升级。 |
...
740/740、build、真实 HTTP 和导入/重试/恢复三视口通过。当前状态为 **implementation-complete /
regression-validated / Docker-release-pending**；卷门与发布尚未完成，整体比例仍保持 96%。
