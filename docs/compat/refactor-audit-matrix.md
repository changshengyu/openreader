...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`components/workspace/LocalStoreManager.vue`、`OverlayLocalStore.vue`、`WebDAVBrowser.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/services/localbook/*`、`engine/*_parser.go` | **P1-E1…P1-E5 与 P2 parser 预算已发布；2026-08-11 又完成 TXT 默认目录规则第二轮 must-fix**：恢复固定上游 18 条及八条手动规则，自动探测仍只使用启用项，双标题跨行规则使用有界兼容匹配。没有数据/API 路径/备份迁移。 | [`rest-action-coverage-fixed-baseline-p2-contract.md`](rest-action-coverage-fixed-baseline-p2-contract.md) 的 TXT 切片已通过精确/正反 fixture、相关包/full/race/vet、frontend 730/730、build 和真实 Go 三视口导入；当前 `Docker-release-pending`。既有 parser 预算发布证据继续见 [`local-text-parser-budget-p2-contract.md`](local-text-parser-budget-p2-contract.md)。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。阅读进度、外部 WebDAV、远程封面等已发布；`/ws/sync` 的 same-origin/active-user/server-only/recipient-scope 已实施验证。2026-08-11 补齐六个精确路径后，登录/注册、书签批删和旧 Category reorder 已裁决；TXT 规则 must-fix 已实施并通过回归，Reader 换源候选仍待实现。 | 既有专项合同继续有效；新增 [`rest-action-coverage-fixed-baseline-p2-contract.md`](rest-action-coverage-fixed-baseline-p2-contract.md) 与 [`reader-source-switch-fixed-baseline-second-audit-p0-contract.md`](reader-source-switch-fixed-baseline-second-audit-p0-contract.md)。TXT 当前 `implementation-complete / Docker-release-pending`；换源当前 `inventory-complete / implementation-pending`。 |
...
## 当前整体进度快照（2026-08-11，`6419807` + TXT 候选实现）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **92%**。该数字表示固定基准合同和测试先行
实现覆盖度；本轮 TXT 目录规则实现与回归已经完成，但在本机 Docker 发布前仍保留 release-pending，
不表示剩余 8% 只是简单收尾。
...
目录规则缺项、禁用项过滤和双标题语义已完成实现及三视口真实导入，因此计入一个百分点。Reader
换源仍缺少每书候选缓存，并在打开时错误联网宽搜，是当前下一项已证实 `must-fix`；移动书架设备
反馈仍需完整截图区分首页书架、Reader 内书架或设备可见层。
