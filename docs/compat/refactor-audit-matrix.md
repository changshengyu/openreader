...
| 书架、BookManage、BookGroup、BookInfo | `Index.vue` 普通 shelf、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、stores | **BookManage、BookGroup、BookInfo 第二轮已分别发布，但普通书架可见布局第二轮为 implementation-pending**。新审查推翻历史 320–360 自适应网格/list 结论：固定 380px 轨道、当前筛选计数、精确编辑搜索、卡片 metadata DOM、loading/blank/night 和单一 750px 响应合同尚未实现。书架 freshness、进度和 lastCheckTime 专项仍保持关闭。 | 待实施差异见 [`bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md`](bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md)；BookManage/BookGroup/BookInfo 既有完成状态不因本轮重开。必须先替换 list/skeleton/窄静态几何假设，再改 `Home.vue`。 |
