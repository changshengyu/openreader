...
| 书源与搜索/探索结果 | `Index.vue#toDetail`、`Reader.vue#getCatalog`、`App.vue#getBookContent`、`Explore.vue`、`BookSource.vue`、`BookController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`RemoteBookJsonEditorDialog.vue`、`ExploreWorkspacePopover.vue`、`backend/api/remote_reader.go`、`services/remotereader`、Go source APIs | **书源 owner/API/解析主链和搜索/探索可见工作区保持已发布；临时 Reader 第二轮已测试先行修复并发布 `30dbe53`**。64 KiB 唯一 JSON、30m/4h lease、8 MiB/session、8+32 MiB/user、128+128 MiB/process LRU、自然到期 410 标记、非法 index 不续期、跨章变量、取消回滚、typed failure 与错误脱敏均已实现。 | [`remote-reader-session-fixed-baseline-second-audit-p1-contract.md`](remote-reader-session-fixed-baseline-second-audit-p1-contract.md) 状态 `aligned / Docker-published / awaiting-device-verification`；Go full/race/vet、frontend 713/713/build、三视口 mock、真实 Go CSS/JSONPath/XPath、新旧卷均通过。OCI index `sha256:9c07871ef7d3c8d99733fcecea205336576c081db651dada13eaeedafda76365`。共享 fetcher 继续按已发布合同执行。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **临时 Reader 第二轮及此前全部能力已由本机作为 `30dbe53` 发布**：Go full/focused race/vet、frontend 713/713、build、临时 Reader 三视口与真实解析链路、本机双架构构建和 GHCR 回读成功。 | `30dbe53`/`latest` OCI index `sha256:9c07871ef7d3c8d99733fcecea205336576c081db651dada13eaeedafda76365`。fresh volume 的 portable v1/v2 assets、cross-user、restart 和 historical TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过。 |

## 当前整体进度快照（2026-08-09，`30dbe53`）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **89%**。临时 Reader 第二轮关闭的是已覆盖
搜索/探索模块内的生命周期、内存预算与安全正确性缺口，因此本轮提升质量但不虚增一个百分点。该数字表示“已完成固定基准复审、
...
- **P1 Index 工作台主链已覆盖**：侧边栏、普通书架、搜索/探索、唯一 BookInfo、BookManage、
  ...
  lastCheckTime 专项已关闭；临时 Reader 的预算、TTL/LRU、变量、取消和脱敏第二轮已发布。
...
- **尚未完成的主线**：尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  以及后续真实设备反馈暴露出的上游可见偏差。`30dbe53` 已包含此前阅读内书架行轨与本次临时 Reader
  第二轮，服务器仍需 pull/force recreate 才能验收；移动书架和临时阅读等待真实设备签收。上述项目继续按“合同→失败测试→实现→浏览器/
  新旧卷→本地 Docker”推进。
...
| 主 Popover 的移动端根几何 / iPad 场景 | `Reader.vue` 传入 `popperWidth = windowWidth - 33`；但 `App.vue` 的 `.mini-interface .popper-component { left:0; top:0; width:100vw !important; }` 是最终权威 CSS。该 mini 形态仅在自适应宽度 `<=750px` 或显式强制时出现。 | `responsive.js` 已使用同一 `<=750px` 宽度 predicate；自适应 1024/1366 为桌面场景，显式 `mobile` 才在宽屏进入完整手机结构。桌面共享面板另保留同工具、外点和 44×44 可见关闭路径。 | **2026-08-09 第二轮源码复审一致，当前浏览器重跑待完成**；旧“宽屏 iPad 自动进入手机面板”结论已过期。见 [`reader-ipad-panel-fixed-baseline-second-audit-p0-contract.md`](reader-ipad-panel-fixed-baseline-second-audit-p0-contract.md)。不得重新引入 UA/iPad 分支或用专用 CSS 改写共享 predicate。 |
