...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`main.js#getCover`、`BookController#getBookCover` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、`RemoteBookResultGroups.vue`、`stores/bookshelf.js`、`useSync.js` | **BookManage 第二轮已发布 `af4e2a4`；BookGroup 第二轮已发布 `5459f02`；BookInfo 第二轮重建已完成、待发布**。BookInfo 恢复 500px/手机 fullscreen、唯一上游骨架、自然比例封面、完整 kind/逐行简介、所有书架书追更和 URL 权威身份；搜索/探索结果卡先选分组，BookInfo 直接加入。 | [`bookinfo-fixed-baseline-second-audit-p2-contract.md`](bookinfo-fixed-baseline-second-audit-p2-contract.md) 已通过 frontend 668/668、Go、build、工作台/临时 Reader 三视口与真实 Go/SQLite 五视口；当前状态 `implementation-complete / release-pending`。 |
