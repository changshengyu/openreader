...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`Index.vue#editBook`、`Index.vue#deleteBook`、`BookManage.vue#deleteBookList` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookEditDialog.vue`、`stores/bookshelf.js`、`useSync.js` | **BookEdit P2 已随 `9614538` 发布；删除消费者收敛与刷新 pending/CAS 二次修复已随 `a54bd72` 发布**：direct/batch/sync 现汇入唯一幂等删除事务；keepalive 成功响应确认 pending，首页和 Reader 内刷新等待一次 CAS 最终赢家，无 reload 且活动正文不跳章。 | [`bookshelf-refresh-progress-p1-contract.md`](bookshelf-refresh-progress-p1-contract.md)、[`book-edit-metadata-p2-contract.md`](book-edit-metadata-p2-contract.md)、[`book-deletion-consumer-convergence-p1-contract.md`](book-deletion-consumer-convergence-p1-contract.md)；等待用户在真实多客户端复验。 |
