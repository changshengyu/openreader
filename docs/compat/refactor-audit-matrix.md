...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十三次移动点击已发布 `49a273e`；夜间实际文字层修复已包含在 `a7abcdd`**：2026-07-28 最新禁缓存健康检查仍显示线上运行旧 `59e11a9`，所以只按非 custom 身份接管并漏掉纯黑自定义黑夜默认；当前镜像按最终“夜间 + 不透明纯黑页面 + 无背景图”统一接管普通正文和 EPUB 作者后代，Compose 也已增加始终拉取防呆。 | [`reader-night-contrast-p0-contract.md`](reader-night-contrast-p0-contract.md)：frontend 649/649、Go/build、候选容器 TXT/Reader 与真实 EPUB 1440×900/390×844/360×800、新旧卷均通过；`a7abcdd`/`latest` index 为 `sha256:93840cf72e9a0a783333ac5ab485551d892e42b9bf2e8eb2e2a1039e56b5dd53`。线上只有在 `/api/health` 返回 `a7abcdd` 后才进入设备复验。 |
