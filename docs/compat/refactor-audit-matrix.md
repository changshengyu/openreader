...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **正文/翻页第十一次候选已通过自动门禁、等待实机；外观资产 P2-A 已实施并通过非 Docker 门禁**：移动 page/scroll/scroll2 保持根文档滚动与点击分页；背景/五个上游字体槽位现以设置精确持久化为成功边界，自身同步广播不再抢占保存，失败回滚清理孤儿；后端已补内容签名与图片尺寸校验。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md) 保持；P2-A 证据与 P2-B 分界见 [`reader-appearance-assets-p2-contract.md`](reader-appearance-assets-p2-contract.md)。P2-B 仍只允许另行版本化设计，不能把现有 URL-only portable v1 描述为资产恢复。 |
