...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 持续复核；连续跨章二次纠正已完成**：标题排版、书源可用性、移动最终顶部顺序、浮动按钮、底部渲染页进度和竖向真实动画时长已完成；连续模式现按顶部安全区确定章/offset，窗口替换期隔离进度，并以 book/mode/generation 丢弃旧 append/retry。 | [`reader-continuous-fixed-baseline-p0-contract.md`](reader-continuous-fixed-baseline-p0-contract.md)；单元 434/434、continuous/mobile/image/text/TTS/volume/audio、真实 EPUB/CBZ 浏览器回归通过，等待本批 Docker。 |
| Reader：EPUB、漫画/CBZ、音频、连续跨章、TTS | `Reader.vue`、`Content.vue`、本地格式解析类 | `ReaderChapterContent.vue`、`ReaderEpubContent.vue`、`ReaderAudioContent.vue`、格式 parser / cache | **EPUB、CBZ 与连续跨章固定基准切片已完成实现及三视口验证**：EPUB/CBZ 已通过历史卷、portable backup 和 Docker；连续跨章已完成顶部章边界、稳定进度事务和旧异步失效，等待本批 Docker。音频和 TTS 引擎内容仍按各自合同继续复核。 | [`epub-fixed-baseline-catalog-reader-contract.md`](epub-fixed-baseline-catalog-reader-contract.md)、[`reader-cbz-fixed-baseline-p0-contract.md`](reader-cbz-fixed-baseline-p0-contract.md)、[`reader-continuous-fixed-baseline-p0-contract.md`](reader-continuous-fixed-baseline-p0-contract.md)；继续审查尚未签收的音频/TTS 内容状态机。 |
