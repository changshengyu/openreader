...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public `/uploads/*` | **按动作逐项复审；已关闭模块不从旧日志重开**。books.go 六个 JSON control、ReplaceRule、备份 generation/list/download 和公开 upload read 均已发布；`backend/api` direct Gin JSON binder 差集为空。远程章节文本缓存读/写/stats/clear/prune 的共享文件系统生命周期已由 `75cc238` 实现并随 `3cef8df` 发布。 | [`remote-chapter-cache-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md`](remote-chapter-cache-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md) 状态 `aligned / regression-validated / Docker-published / awaiting-device-verification`；rooted 同句柄有界读取、原子可取消写入、实际文件统计、全用户引用 fail-closed 和 write/prune 串行边界已通过专项/full/race/vet、浏览器、runtime 和卷门。 |
