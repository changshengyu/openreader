...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第十三次移动点击已发布 `49a273e`；第四轮夜间实际文字层已发布 `3ee3a82`**：此前 `59e11a9` 只按非 custom 身份接管，漏掉可被设为黑夜默认的纯黑自定义方案；现在按最终“夜间 + 不透明纯黑页面 + 无背景图”统一接管普通正文和 EPUB 作者后代。 | [`reader-night-contrast-p0-contract.md`](reader-night-contrast-p0-contract.md)：frontend 644/644、Go/build、TXT 桌面/两手机/iPad 与真实 EPUB 1440/390/360 已通过；自定义图片和非黑自定义夜间继续保留。设备验收使用不可变标签 `3ee3a82`。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **第四轮纯黑自定义夜间修复已从本机发布**：frontend 644/644、Go、build、TXT 桌面/390/360/iPad、真实 EPUB 1440/390/360、新卷 portable v1/v2 assets/跨用户/重启和历史 TXT/EPUB/UMD/CBZ/相对缓存/owner isolation 均通过。 | `3ee3a82`/`latest` 共同指向 amd64/arm64 OCI index `sha256:23454f80db395e45c660e41b9fe5a314936be89a0acbe27eeab0e4761a332f18`；线上需替换容器并以 `/api/health` 确认 `3ee3a82`。 |
