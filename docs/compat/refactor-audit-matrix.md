...
| 书架、BookManage、BookGroup、BookInfo | `Index.vue` 普通 shelf、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、stores | **BookManage、BookGroup、BookInfo 第二轮已分别发布；普通书架主体曾作为 `60984b6` 发布，但 `c851c5f` 后确认移动宽度回归，修复中**。固定 380px grid、当前筛选计数、精确编辑搜索、历史 list→grid v3 迁移、独立 metadata DOM、loading/blank/night 保持；Home 样式全局化后被 `.mobile-shell .app-page` 重新叠加左右各 14px，390px 行实测仅 362px。书架 freshness、进度和 lastCheckTime 专项继续保持关闭。 | [`bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md`](bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md) 已记录 2026-08-05 发布后勘误；当前状态 `regression-confirmed / fix in progress`。原 `60984b6` 验证仍是历史证据，不能证明 `c851c5f`/`latest` 的移动书架宽度。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前搜索/探索可见工作区切片已由本机作为 `c851c5f` 发布**：frontend 701/701、Go、build、Index 四视口、Remote Reader 三视口、双 session isolation、真实三类书源工作流、新卷 portable v1/v2 assets/跨用户/重启和历史 TXT/EPUB/UMD/CBZ/相对缓存/owner isolation 均通过。 | `c851c5f`/`latest` 共同指向 amd64/arm64 OCI index `sha256:f964b155447fe3660d72de292b100d27788812cce42bafc55f3237190bdc97e0`；等待设备验收。 |
