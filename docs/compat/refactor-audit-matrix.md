...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`WebDAV.vue` | `OverlayBookImport.vue`、`LocalStore.vue`、`WebDAVBrowser.vue`、`LocalBookImportPreviewDialog.vue`、`backend/services/localbook/*`、`engine/*_parser.go` | **P1-E1 已完成，P1-E2 已审查**：书仓/WebDAV 的失败预览可用同一暂存 token、已编辑规则重试并刷新目录；不会重新读取可变的挂载源文件。当前单一批量预览/确认、继承筛选分组和双入口状态机与上游分支不符，必须重建为共享 controller。额外目录操作和完整格式/旧卷复验仍未开始。 | `docs/compat/workspace-storage-import-p1e-contract.md` → P1-E2 失败测试与共享状态机 → P1-E3 额外流程审查 → 真实文件夹/卷/Docker smoke。 |
