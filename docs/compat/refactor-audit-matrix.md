...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **第九次实机复验重新打开点击翻页 P0**：`a54bd72` 的 Hermite 非零初速、一次同向缓冲、after-paint 立即结算及全章段落测量均偏离固定上游；现改按上游 cubic ease-in-out、动画互斥、延迟保存和命中即停段落查找重建。iPad 关闭路径仍为已验证。 | [`reader-ipad-panel-dismiss-p0-contract.md`](reader-ipad-panel-dismiss-p0-contract.md)、[`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；`must-fix`，不得再以 `a54bd72` 的 Chromium rAF 采样声称真机体感完成。 |
