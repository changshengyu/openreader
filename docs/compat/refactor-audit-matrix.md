...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 第六次实机反馈修复已完成发布前门禁**：正数竖向点击的最终位置先呈现，章节/窗口/布局/进度结算移到后续可取消任务；`0ms`、一次缓冲、原生手势和三模式语义不变。4× CPU 最长动画 rAF 从约 35ms 降到 0.88ms，draw 最大约 0.33ms。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；前端 492、Go/build、三模式双移动、desktop/mobile 与 continuous 合同通过，等待本批 Docker 真机体感复验。 |
