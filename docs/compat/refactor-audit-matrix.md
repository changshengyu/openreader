...
| 书源与搜索/探索结果 | `Index.vue#toDetail`、`Reader.vue#getCatalog`、`App.vue#getBookContent`、`Explore.vue`、`BookSource.vue`、`BookController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`RemoteBookResultList.vue`、`RemoteBookJsonEditorDialog.vue`、`ExploreWorkspacePopover.vue`、`backend/api/remote_reader.go`、Go source APIs | **书源 owner/API/解析主链和搜索/探索可见工作区保持已发布**；临时 Reader 主流程是技术栈等价，但 2026-08-09 第二轮发现 create body、单会话/用户/进程内存预算缺失，以及非法章节索引会提前续期。跨章变量、取消、failure cache 和错误脱敏实现方向正确但缺专门失败测试。 | [`remote-reader-session-fixed-baseline-second-audit-p1-contract.md`](remote-reader-session-fixed-baseline-second-audit-p1-contract.md) 状态 `inventory-complete / implementation-pending`；先提交合同，再补预算/TTL/LRU/变量/取消/脱敏测试，实施前不沿用现有通过用例作为对齐证据。共享 fetcher 仍按 [`shared-source-fetcher-p2-contract.md`](shared-source-fetcher-p2-contract.md) 已发布合同执行。 |
