...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book patch/category 已发布。当前差集确认 `PUT /api/categories/:id` 虽有部分 DTO，仍将 transaction 前完整 Category 以 `Save` 写回，可覆盖并发 reorder/partial update 或复活删除目标。 | [`category-patch-write-lifecycle-fixed-baseline-second-audit-p2-contract.md`](category-patch-write-lifecycle-fixed-baseline-second-audit-p2-contract.md) 已固定 request-context transaction、owner 重读、显式 `name/color/show` update、空 patch 和权威 event；状态 `inventory-complete / tests-and-implementation-pending`。旧 BookGroup wire/UI/data 合同不重开。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release workflow | `4b0a599` 通过 frontend 742/742、Go full/race/vet、build、Compose、BookManage/BookInfo 三视口和 GHCR 回拉容器；受信 Actions run `33308641504` 通过 native、fresh portable、historical volume 与 published-platform 门后发布。 | amd64/arm64 发布 `4b0a599`/`latest`，OCI index `sha256:03158e390e967f6ef4f6addc9125de504bdd781aa81e85ed5aaa403d58ead0fd`；manifests 分别为 `sha256:44f5c8ae8334de6468e830477308b3d3be17cf0e8f18133030dfdf0df19bdeb2`、`sha256:1f0699b40542b295b4be810c463562d78179dfa2847b92c2a7166be08dd0d9c5`。发布后的 browser smoke 提交为 `0edbbb5`；用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-08-30，Category patch lifecycle inventory）
...
并发提交亦已关闭。剩余约 1% 当前包含已确认的 Category patch full-row `Save` must-fix、后续逐路由
action 审计、长尾固定基准复审与真实设备证据，不能从 direct binder、`Save` 或 contextless 调用差集
缩小推导完成。
