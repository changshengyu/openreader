...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **iPad 响应判定已重新打开为 P0**：`b8b70f9` 只修正 mini 状态的 DOM/CSS 一致性；实机反馈证明当前 UA/touch 自动 mini 仍让 1024/1366px iPad 使用覆盖上半屏的手机主面板，并把其它对话框也切成手机形态。固定上游在自适应模式下为纯 `<=750px` 判定。 | [`reader-ipad-responsive-p0-contract.md`](reader-ipad-responsive-p0-contract.md)、[`reader-mobile-controls-p0-contract.md`](reader-mobile-controls-p0-contract.md)；先写宽度/显式手机模式失败合同，再做 iPad 横竖屏与双手机浏览器回归。 |
