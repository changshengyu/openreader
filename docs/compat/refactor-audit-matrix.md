...
| 用户、备份、RSS、书签 | `UserManage.vue`、`AddUser.vue`、`BaseController.kt#saveUserSession`、`WebDAV.vue`、`Rss*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`backend/api/auth.go`/`admin.go`、`WebDAVBrowser.vue`、`RSSManager.vue`、`OverlayBookmarks.vue`、Go services | UserManage、Bookmark、Overlay 会话隔离和 P2-S4 owner 隔离保持已发布。**RSS 于 2026-08-09 第二轮固定基准复审后重新打开并判定必须重建**：三层弹窗所有权可保留，但 760/900px 双层壳、富源卡片、结构化 editor、直接全量导入、文章筛选/收藏/摘要/作者、正文 footer，以及一次 refresh 最多抓 1000 页均不是上游等价。 | [`rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md`](rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md) 状态 `audit-complete / implementation-pending`；必须先替换冲突测试，再重建 500px/fullscreen 源/文章/正文 sibling dialogs、JSON editor、勾选导入和按请求页抓取。既有 JWT/SQLite 缓存、安全限制、事务删除、会话隔离为条件保留，不代表 RSS 可见工作区已对齐。Bookmark 证据仍见 [`bookmark-fixed-baseline-second-audit-p2-contract.md`](bookmark-fixed-baseline-second-audit-p2-contract.md)。 |
...
| 书源解析、RSS、远程抓取 | `AnalyzeRule*`、`Rss*`、`BookSourceController.kt` | `backend/engine/source_*.go`、`rss_parser.go`、fetcher | **CSS/JSONPath/XPath 书源主链保持已完成；RSS 分页语义重新判定为 `must-fix`**。当前规则 RSS refresh 会沿 next-page 循环直至结束或 1000 页，而固定上游每次 `getRssArticles(page)` 只处理请求页；有上限不等于按页契约一致。 | RSS 页级重建见 [`rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md`](rss-visible-workspace-fixed-baseline-second-audit-p2-contract.md)；保留 timeout/size/redirect/SSRF/rate/parser-work 安全门，但 page 1/2 必须各自成为一个有界请求操作。书源证据仍见 [`online-booksource-parser.md`](online-booksource-parser.md) 与 [`booksource-metadata-normalization-p2-contract.md`](booksource-metadata-normalization-p2-contract.md)。 |
