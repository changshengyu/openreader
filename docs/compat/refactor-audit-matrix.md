...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、格式运行时、设置、夜间面、远程正文和换源写入生命周期均已专项实施。**2026-09-14 真机 stale 409 二次修复已在 `e1631d0` 实施并发布**。同日桌面翻页真机反馈重新打开：桌面纵向文本仍误用内部滚动容器，点击区为 35/30/35 而非上游 30/40/30，滚轮边界还会合成切章。 | [`reader-remote-chapter-concurrency-device-feedback-p0-contract.md`](reader-remote-chapter-concurrency-device-feedback-p0-contract.md) 已发布并等待真机复验；桌面翻页见 [`reader-desktop-pagination-device-feedback-p0-contract.md`](reader-desktop-pagination-device-feedback-p0-contract.md)，状态 `inventory-complete / implementation-pending`，须按根滚动、精确点击区和纯原生滚轮合同先补红测。 |
