...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`Index.vue#editBook`、`Index.vue#deleteBook`、`BookManage.vue#deleteBookList` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookEditDialog.vue`、`stores/bookshelf.js`、`useSync.js` | **刷新阅读进度 P1 已随 `ed4ee27` 发布；BookEdit P2 已随 `9614538` 发布；删除消费者收敛 P1 已实施并完成三视口真实双客户端验证**：direct/batch/sync 现汇入唯一幂等事务，清书架/章节缓存/本地进度并冻结用户 scope；目标书弹层定向关闭，BookManage 选择收敛；活动 Reader 先挂起进度代际再 `replace` 回 Home，删除后无进度回写。 | [`bookshelf-refresh-progress-p1-contract.md`](bookshelf-refresh-progress-p1-contract.md)、[`book-edit-metadata-p2-contract.md`](book-edit-metadata-p2-contract.md)、[`book-deletion-consumer-convergence-p1-contract.md`](book-deletion-consumer-convergence-p1-contract.md)；本切片待最终提交/发布门禁。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **iPad 关闭路径与第七批移动连续点击修复已随 `544e1fb` 发布**：iPad 四个共享面板已有 44px 关闭目标和明确层级；竖向 `page/scroll/scroll2` 的缓冲页现于最终 rAF 内续接，交接窗口点击可取消旧结算并立即接管。前端 509/509、Go、构建、iPad Pro 横竖屏、双手机、强制移动 iPad、Docker 新旧卷/备份门禁全部通过。 | [`reader-ipad-panel-dismiss-p0-contract.md`](reader-ipad-panel-dismiss-p0-contract.md)、[`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；等待用户在真实手机复验 `544e1fb` 的连续点击体感。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `9614538` 已验证**：前端 523/523、Go 全量、生产构建、BookEdit 并发事务三视口真实 Go/Chromium，以及普通和历史 TXT/EPUB/UMD/CBZ/相对缓存 volume/backup 均通过；镜像在本机完成双架构构建并上传。此前书架刷新/时间、Reader/WebDAV 门禁继续有效。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:2ea7c5db548f6fb27f78ca9a17903f5ffe2a14b35dbd0569d998379d98dc8b9b`，amd64 `sha256:d2b39b3e0627fc8925336dcf09471e497658512ea571a22decd76f594a478c09`，arm64 `sha256:d31a491ff7aecc7c03f80be33c92f1480587e90fbd16adacd53fa68289489070`。 |
