...
| 书源与搜索结果 | `Index.vue`、`Explore.vue`、`BookSource.vue`、`BookSourceController.kt` | `SourceManager.vue`、`Search.vue`、`Discover.vue`、`SourceSwitchPanel.vue`、`backend/api/sources.go` | **P1-C 工作台收敛、P2 失效缓存、Reader 换源和 CSS/JSONPath/XPath 主解析链已实施验证**；全局书源 + 用户编辑权限是已记录的数据模型重设计。当前 must-fix 是脚本能力提示：导入预览会默认勾选后端确定拒绝的 `<js>` Header / `loginCheckJs` 源，编辑/debug 缺少明确安全说明。 | [`booksource-script-transparency-p2-contract.md`](booksource-script-transparency-p2-contract.md)；先补兼容分析、导入选择、编辑/debug 失败合同，再修改 UI。 |
