...
| Reader：设置第二轮复审 | `ReadSettings.vue`、`config.js`、`vuex.js#setConfig/setNightTheme`、`Reader.vue` TTS/read bar | `ReaderSettingsPanel.vue`、`ReaderSettingStepper.vue`、`stores/reader.js`、`ReaderTTSBar.vue`、appearance/mode composables | **2026-08-02 已完成只读审查，实施待开始**：标题/滚动宿主、行几何、工具层并存已对齐；重置数据丢失、方案字段污染、新增方案状态、normal/kindle 双快照、主题资源、14 内置背景、重复 TTS 和精确顺序仍是 `must-fix`。亮度、可编辑 stepper、字体预览/字号预设和纯黑白夜间为用户明确允许差异。 | [`reader-settings-fixed-baseline-second-audit-p0-contract.md`](reader-settings-fixed-baseline-second-audit-p0-contract.md)；下一门为失败测试 → store/面板实现 → 五视口与新旧卷，当前不得标记 aligned 或发布本模块 Docker。 |
