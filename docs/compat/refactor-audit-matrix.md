...
| 书架、BookManage、BookGroup、BookInfo | `Index.vue` 普通 shelf、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、stores | **BookManage、BookGroup、BookInfo 第二轮已分别发布；普通书架可见布局第二轮已实现并完成浏览器验收，等待发布**。固定 380px grid、当前筛选计数、精确编辑搜索、历史 list→grid v3 迁移、独立 metadata DOM、loading/blank/night 和单一 750px 响应合同均已落地；外部 scoped CSS 的夜间选择器编译偏差也已由 computed-color smoke 修复。书架 freshness、进度和 lastCheckTime 专项继续保持关闭。 | [`bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md`](bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md)：frontend 689/689、Go/build/diff、1440×900/1024×1366/390×844/360×800 专项及移动侧栏、Index 操作面、工作台、刷新竞态全部通过；状态 `implemented / browser-validated / release-pending`，发布前仍需本地镜像和新旧卷/备份门。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 Index 设置/操作面切片已由本机作为 `3746d62` 发布**：frontend 685/685、Go、build、Index/缓存/工作台三视口、移动侧边栏双视口、新卷 portable v1/v2 assets/跨用户/重启和历史 TXT/EPUB/UMD/CBZ/相对缓存/owner isolation 均通过。 | `3746d62`/`latest` 共同指向 amd64/arm64 OCI index `sha256:eb57e0094baeb7d0cc354a0b97e5d366059fe47032d83fd2b5f42819a3d9e23b`；等待设备验收。 |
