...
| 书架、BookManage、BookGroup、BookInfo | `Index.vue` 普通 shelf、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、stores | **BookManage、BookGroup、BookInfo 第二轮已分别发布；普通书架 `c851c5f` 后的 28px 移动宽度回归已修复并作为 `7971e23` 发布**。固定 380px grid、当前筛选计数、精确编辑搜索、历史 list→grid v3 迁移、独立 metadata DOM、loading/blank/night 保持；书架场景现明确覆盖通用 app-page 左右各 14px，390/360px 视口重新得到同宽书籍行。书架 freshness、进度和 lastCheckTime 专项继续保持关闭。Reader 内书架是独立 P0 合同，其每侧多缩进 4px 的偏差已作为 `5151609` 发布并重新包含在 `d198c2e`。 | [`bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md`](bookshelf-visible-layout-fixed-baseline-second-audit-p1-contract.md) 保持 `aligned`；Reader 内书架见 [`reader-mobile-shelf-width-p0-contract.md`](reader-mobile-shelf-width-p0-contract.md)，当前线上仍是 `7971e23`，需部署 `d198c2e` 后验收。Index 线上实测保持 390px 满宽。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 P2-N2 与 Reader 移动书架修复已由本机作为 `d198c2e` 发布**：代码级 focused/full/race/frontend 706/706/build/diff、Docker 网络策略及 amd64/arm64 推送成功。 | `d198c2e`/`latest` OCI index `sha256:021817e602aa589c1583ec7ccb65828172c1a2afe1e038e23651dd51c455fcc1`。fresh volume 的 portable v1/v2 assets、cross-user、restart 和 historical TXT/EPUB/UMD/CBZ、relative-cache、owner-isolation 门全部通过。 |

## 当前整体进度快照（2026-08-09，`d198c2e`）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **87%**。该数字表示“已完成固定基准复审、
测试先行实现并至少发布过一次”的覆盖度，不表示剩余 13% 只是简单收尾。
