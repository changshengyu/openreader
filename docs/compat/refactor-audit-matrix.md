...
| Pinia 状态、缓存、同步、数据事务 | `plugins/vuex.js`、`plugins/cache.js`、后端 controller/model | `stores/*.js`、`utils/*cache*`、`backend/models`、`services`、`sync` | **书架 P2 与认证运行时 scope 均已实现**：书架 network-first/revision/Hub/前台校准保持完成；Reader 设置、偏好、进度、分类和 profile 的异步提交现绑定账号身份与操作 revision，WebSocket callback 绑定 candidate/generation/token/scope。 | [`bookshelf-network-first-sync-p2-contract.md`](bookshelf-network-first-sync-p2-contract.md)、[`authenticated-runtime-scope-p2-contract.md`](authenticated-runtime-scope-p2-contract.md)；提交 `24feff5`，前端 466 项、Go 全量和生产构建通过，真实账号切换 smoke 仍列入下一综合浏览器门禁。 |
