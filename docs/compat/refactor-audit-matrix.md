...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`main.js#getCover`、`BookController#getBookCover` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、`RemoteBookResultGroups.vue`、`stores/bookshelf.js`、`useSync.js` | **BookManage 第二轮已发布 `af4e2a4`；BookGroup 第二轮已发布 `5459f02`；BookInfo 第二轮审计已重开**。BookInfo 唯一全局 host 正确，但固定基准发现 480px、未使用详情骨架、裁剪封面、额外字数/提示、kind 截断、简介压缩、本地书缺追更、ID 优先误认、隐形缓存扫描和加书动作所有权互换。上游是“结果卡加书先选分组；BookInfo 加书直接写入”，当前恰好相反。 | BookManage/BookGroup 状态不变。BookInfo 必须按 [`bookinfo-fixed-baseline-second-audit-p2-contract.md`](bookinfo-fixed-baseline-second-audit-p2-contract.md) 测试先行重建；当前状态 `audit-complete / implementation-pending`，旧 BookInfo 加书确认测试与 API 文档不再作为正确证据。 |
