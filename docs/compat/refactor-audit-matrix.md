...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`main.js#getCover`、`BookController#getBookCover` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、`stores/bookshelf.js`、`useSync.js` | **BookManage 第二轮固定基准重建已完成并发布 `af4e2a4`**：桌面/手机统一同一表格，恢复 normal gate、动态几何、标题/作者搜索、查询保留、每次 force shelf read、精确列/动作/footer/确认与缓存中取消状态；移动卡片、阅读进度/类型/最新章、无选择禁用及“停止 n/total”已删除。JWT/SQLite、多用户、many-to-many 幂等移除和缓存安全为明确技术适配。书架/BookInfo 既有结论保留；BookGroup 完整管理面仍待独立第二轮复审。 | [`book-management-fixed-baseline-second-audit-p2-contract.md`](book-management-fixed-baseline-second-audit-p2-contract.md)：frontend 661/661、Go/build，mock 1440/390/360/两种 iPad、真实 Go/SQLite 1440/390/360及新旧卷均通过；状态 `aligned / Docker-published`，OCI index `sha256:64f594571618c232113b4211054b7c14718d3fddddc489f267333109fe20f29e`。BookInfo 仍见 [`bookinfo-action-ownership-cleanup-p1-contract.md`](bookinfo-action-ownership-cleanup-p1-contract.md) 与 [`book-cover-proxy-p2-contract.md`](book-cover-proxy-p2-contract.md)。 |
