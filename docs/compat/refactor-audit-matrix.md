...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue` | **整本正文缓存已发布 `06ac89b`；正文图片切片已实现并发布 `32dc616`**：图片按用户/书隔离去重、请求受 SSRF/凭证/重定向/大小/MIME 限制，章节响应以短期 capability 映射且不改变原正文/位置，EPUB 只嵌入已验证缓存；清缓存、删书/用户、换源均接入清理。 | [`book-management-cache-p2-contract.md`](book-management-cache-p2-contract.md)、[`book-cache-images-p2-contract.md`](book-cache-images-p2-contract.md)；历史卷/portable backup 和远端双架构 manifest 已通过。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 持续复核；第四次用户实测后的修复已发布 `32dc616`**：移除把超长整章提升为 WAAPI transform 图层的路径；移动普通文本 page 复用已减负的 `scrollTop` 帧动画和快速起步曲线。分页步长、0/100/300/500ms、快速双击缓冲、长按选文和原生 touch/wheel 保持。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；前端 489、Go/build、360 段长章节双移动触控、desktop/mobile、continuous 和 Docker 门禁通过，等待用户手机体感复验。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前发布 `32dc616` 已验证**：前端 489/489、Go 全量、生产构建、Reader 文本/移动/连续/图片三视口、历史 TXT/EPUB/UMD/CBZ/相对缓存 volume/backup、远端 amd64/arm64 manifest 均通过；以后每次发布仍必须以新提交重跑。 | `openreader-regression`、`openreader-docker-release`；当前 index `sha256:e5db5dd67e9dafc93803230ec2dba9c4ce09dc39632fcec3d9882b47a6ae781d`。 |
