...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category 单项 patch 列所有权已发布；`POST /api/books/batch` 三个 category action 亦随 `95aa598` 关闭：relation 读写进入 request-context 同一 tx，完整 Book `Save` 改为 guarded `category_id`，取消/错误回滚且删除不复活。 | [`batch-book-category-write-lifecycle-fixed-baseline-second-audit-p2-contract.md`](batch-book-category-write-lifecycle-fixed-baseline-second-audit-p2-contract.md) 已通过旧实现红测、focused/race/full/vet、frontend 742/742、build/Compose 与 BookManage 三视口真实 API；状态 `aligned / regression-validated / Docker-publication-pending-verification`。已签收 wire/UI、单本 category 与 delete/cache 合同不重开。 |
...
## 当前整体进度快照（2026-08-31，Batch Book Category lifecycle implementation）
...
的 Actions 最终状态与 digest 待复核。剩余约 1% 包含后续逐路由 action 审计、长尾固定基准复审与真实设备证据，
