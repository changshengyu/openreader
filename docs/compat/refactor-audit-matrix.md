...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue` | **BookManage 整本正文缓存切片已实现并发布 `06ac89b`**：服务器/浏览器从 0 覆盖整本，按用户/书独立任务可取消且跨 Dialog，删除确认、菜单/footer/TXT/EPUB 和规范 SSE 计数已恢复；旧 batch API 仅作隐藏兼容。 | [`book-management-cache-p2-contract.md`](book-management-cache-p2-contract.md)；Go/frontend/build、三视口 BookManage、Reader cache 和历史卷/备份回归通过。上游正文图片 `saveImages` 仍是明确下一切片，不能宣称完整离线图片对齐。 |
...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 持续复核；移动点击翻页第三批已发布 `06ac89b`，等待用户手机体感**：普通短点击不再启动选文重试；移动普通文本 page 在 touchstart 临时预热、所有非翻页分支释放；仅合成路径缩短缓入死区。分页步长、时长、终点、长按选文和原生 touch/wheel 保持。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；前端 487、Go/build、双移动输入窗口、desktop/mobile、真实 touch 选文、continuous 及历史卷合同通过。 |
