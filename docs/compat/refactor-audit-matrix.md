...
| Reader：书内正文搜索 | `SearchBookContent.vue`、`Reader.vue#showSearchContent`、`BookController.searchBookContent/searchChapter`、`SearchResult.kt` | `OverlayBookContentSearch.vue`、`useBookContentSearch.js`、`useReaderSearchNavigation.js`、`readerBookSearch.js`、`backend/api/books.go` | **P2 固定上游审计完成，重新标记 must-fix**：当前搜索的是应用替换规则后的正文；模糊/非重叠匹配偏离上游精确重叠语义；行点击依赖 `router.push`，导致重复同一结果 no-op、双 watcher 和返回历史污染；缺失书源未前置失败，legacy 字段/片段不完整。根 Dialog、取消、有界扫描、完整性提示可保留。 | 按 [`book-content-search-p2-contract.md`](book-content-search-p2-contract.md) 先补原始正文、精确重叠、缺失书源、legacy 字段和可重复 Reader intent 失败合同，再修改应用；三视口必须验证重复结果和一次返回。 |
