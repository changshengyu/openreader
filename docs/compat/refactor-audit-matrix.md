...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **第十一次候选已通过 frontend 544/544、Go、build 及文字/移动/iPad/连续/图片浏览器门禁；已发布镜像仍为 `0a77632`**。本轮尚未完成本地镜像和卷/便携备份门禁，因此不得提前报告新标签。 | `openreader-regression`、`openreader-docker-release`；完成本地 Docker 门禁后发布根滚动候选并记录 digest，再等待用户实机验收。 |
