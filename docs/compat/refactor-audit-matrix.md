...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public `/uploads/*` | **按动作逐项复审；已关闭模块不从旧日志重开**。books.go 六个 JSON control、ReplaceRule、备份 generation/list/download 和公开 upload read 均已发布；`backend/api` direct Gin JSON binder 差集为空。下一项 must-fix 已收敛为远程章节文本缓存读/写/stats/clear/prune 的共享文件系统生命周期。 | [`remote-chapter-cache-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md`](remote-chapter-cache-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md) 状态 `inventory-complete / tests-and-implementation-pending`；当前 lexical prefix + `os.ReadFile/Stat/Remove/WriteFile` 与引用查询失败继续删除已确定为 must-fix。 |
...
- **尚未完成的主线**：其余尚未逐动作签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
  ...
  生命周期已完成 inventory，下一步是旧实现红测；其它 action 长尾继续逐项取证。
  `c74be70` 已发布但尚待
  服务器部署；移动书架在
