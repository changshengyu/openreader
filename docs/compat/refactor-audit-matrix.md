...
| 用户、备份、RSS、替换规则、书签 | `UserManage.vue`、`AddUser.vue`、`BaseController.kt#saveUserSession`、`WebDAV.vue`、`Rss*`、`ReplaceRule*`、`Bookmark*` 及对应 Kotlin 控制器 | `OverlayUserManagement.vue`、`backend/api/auth.go`/`admin.go`、`WebDAVBrowser.vue`、`RSSManager.vue`、`OverlayReplaceRules.vue`、`OverlayBookmarks.vue`、Go services | RSS、替换规则和书签合同继续成立；P2-S4 owner 隔离和用户书源动作已发布。2026-07-28 UserManage 复审进一步删除移动卡片/合并权限列，恢复桌面移动共用表格、固定移动首列、上游列/底栏顺序和显式取消；登录成功持久更新 canonical `lastLoginAt`，`lastActiveAt` 仅留作兼容别名。 | [`user-management-p2-contract.md`](user-management-p2-contract.md)、[`book-source-ownership-p2-contract.md`](book-source-ownership-p2-contract.md)；本批 Go、frontend 641/641、build 和 1440/1024/390/360 浏览器门已通过，待 Docker 卷门后发布。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **当前 `2c4ede6` 已从本机完成并发布**：frontend 641/641、Go、build、普通正文/EPUB 桌面、手机与 iPad 夜间实际承载层浏览器门、新卷、历史 TXT/EPUB/UMD/CBZ/相对缓存、portable v1/v2、跨用户、恢复与重启均通过；`2c4ede6`/`latest` 共同指向 `sha256:04ba137d8da8988ae7e2053b6c9051d438f3357ca35df8aad371cd11aced071d`。线上只读取证发现用户实例仍报告旧版本 `a90d10b`；设备复验前必须先确认宿主已经重建到 `2c4ede6`。 | `openreader-regression`、`openreader-docker-release`；继续保留 Reader 真实手机体感与夜间设备复验，不能用自动门替代设备结论。 |
