...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **移动点击体感重新标记 must-fix；外观资产 P2-A/P2-B 均已发布**：第十三次实测确认整章渐变/阴影/遮罩造成约 235ms RasterTask，深章节立即结算和保存分别从头读取约 1736 次段落几何，并出现 136ms Long Task。动画公式、300ms、步长和根滚动已对齐，但绘制面与空闲结算仍未对齐。 | 先按 [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md) 第十三次边界补深章节、绘制结构和空闲取消合同，再实施上游小纹理平铺、视口遮罩、单调段落二分和可取消结算。P2-A/P2-B 证据见 [`reader-appearance-assets-p2-contract.md`](reader-appearance-assets-p2-contract.md) 与 [`portable-appearance-assets-p2b-contract.md`](portable-appearance-assets-p2b-contract.md)。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **`ceb4baa` 已从本机完成并发布**：frontend 561/561、Go、build、cover race/vet、封面/Index/BookInfo/Reader audio 三视口、新卷、历史卷、portable v1/v2、旧本地书与用户隔离均通过；`ceb4baa`/`latest` 共同指向 `sha256:c5cace40e21a9b30b4f2f7cdd9219a59ff16525b173bcf79d5994e950ff56fd2`。 | `openreader-regression`、`openreader-docker-release`；等待用户实机验证第三方封面冷缓存、外观资产事务和既有根滚动点击体感，不能以自动门禁替代设备结论。 |
