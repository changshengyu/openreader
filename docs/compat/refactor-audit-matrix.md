...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`EpubFile.kt`、`UmdFile.kt`、`CbzFile.kt`、`LocalStore.vue`、`WebDAV.vue` | `OverlayBookImport.vue`、`components/workspace/LocalStoreManager.vue`、`OverlayLocalStore.vue`、`WebDAVBrowser.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/api/localstore*`、`backend/api/webdav.go`、`backend/services/localbook/*`、`engine/*_parser.go` | P1-E1…P1-E5、P2 parser 预算和 TXT 默认目录规则已发布。LocalStore 文件系统/请求边界已测试先行实施，等待卷门。**2026-08-16 WebDAV mounted import/restore 动作 inventory 完成**：三个 JSON handler 的 actual-read/cardinality 与目录内 symlink/special-file/opened snapshot 为下一 must-fix；不重开原生 DAV 协议或 ZIP 事务。 | LocalStore 合同已通过 focused/full/race/vet、frontend/build、宿主/candidate HTTP，`930be4d` 已推送但未发布。WebDAV 合同见 [`webdav-import-restore-filesystem-request-boundary-fixed-baseline-second-audit-p2-contract.md`](webdav-import-restore-filesystem-request-boundary-fixed-baseline-second-audit-p2-contract.md)，状态 `inventory-complete / implementation-pending`；正式 GHCR 仍是 `be83a0f`。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。已关闭的 auth/admin/settings/group/book/source/bookmark/RSS/asset/LocalStore 动作不重开。新差集把 WebDAV import-preview/import/restore-webdav 的 bounded JSON、cardinality 和 mounted opened-file 选为下一 must-fix。 | [`rest-action-coverage-fixed-baseline-p2-contract.md`](rest-action-coverage-fixed-baseline-p2-contract.md) 第 17 节及 WebDAV 专项合同已完成 inventory；下一步必须先以旧实现失败测试证明缺口，再修改 handler/service。direct/source multipart、remote-work JSON、progress 与其它 batch JSON 仍待后续排序。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | `930be4d` LocalStore candidate 已通过 frontend 740/740、Go full/focused race/vet、build、宿主与 candidate 真实 HTTP；本机 arm64 OCI revision 正确。fresh/historical/portable 卷门因 Docker socket 提升权限额度审批被拒，未把 candidate 误报为发布。 | 正式 GHCR 仍是 `be83a0f` 与 `latest`，OCI index `sha256:e1f31f3dd728bc27fbc89bbc8c21f81e8c5511c5e99196891feb21cd47138b73`；`930be4d` 未发布。用户生产环境当前运行提交仍未知，发布不等于服务器已升级。 |
...
candidate HTTP 均通过。fresh/historical/portable 卷门因 Docker socket 提升权限额度审批被拒，未发布
新镜像；当前状态为 **implementation-complete / regression-validated / Docker-release-pending**，整体
比例暂不变。

2026-08-16 又从 `server.go`、固定上游 `WebDAV.vue`/controllers 和当前 handler 重新做动作差集，选择
WebDAV mounted import/restore 为下一 must-fix：`import-preview`、`import`、`restore-webdav` 仍直接
`ShouldBindJSON`，无 raw/expanded cardinality；顶层 `Resolve` 后的目录内项又绕开 `Service.Open`，
不能排除 symlink/special file 或 restore path 重新打开。精确合同见
...
当前状态为 **inventory-complete / implementation-pending**；本轮未修改应用或测试，整体比例保持 96%。
