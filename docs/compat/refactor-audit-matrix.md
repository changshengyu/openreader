...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置、夜间面和内联章节缓存均已专项发布。主正文请求生命周期已随 `0a8a0ef` 实施。**当前新差集是后端换源写入生命周期**：远程 fetch 后仍以 pre-fetch Book 全行 `Save`，未复验当前书或目标书源。 | 正文合同已通过前端 748/748、build、四视口及后端 focused/race/full/vet；状态 `aligned / regression-validated / GitHub-sync-and-Docker-evidence-pending`。新换源写入合同见 [`reader-source-change-write-lifecycle-fixed-baseline-second-audit-p0-contract.md`](reader-source-change-write-lifecycle-fixed-baseline-second-audit-p0-contract.md)，状态 `inventory-complete / tests-and-implementation-pending`；候选 UI、三 mode 和位置恢复不重开。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category/remote-add 和章节正文列所有权均已实施。当前 `changeBookSource` 仍在远程 fetch 后以旧 Book 全行 `Save`，且目录替换前不复验 Book/目标 Source snapshot。 | [`reader-source-change-write-lifecycle-fixed-baseline-second-audit-p0-contract.md`](reader-source-change-write-lifecycle-fixed-baseline-second-audit-p0-contract.md) 要求 current-row/source-snapshot prevalidation、owned-column guarded update、authoritative response/event 和陈旧 409；状态 `inventory-complete / tests-and-implementation-pending`。既有 parser、候选、缓存、位置和可见 UI 合同保持关闭。 |
...
## 当前整体进度快照（2026-09-09，Reader source-change write lifecycle inventory）
...
实施并完成本地全量及四视口验证，GitHub 同步与可信 Actions/Docker 证据因 443 超时待补。当前下一项
must-fix 是换源 fetch 后的 Book/Source 存活复验、全行 Save 和权威响应生命周期；剩余约 1% 还包含
后续逐路由 action 审计、长尾固定基准复审与真实设备证据，不能从 direct binder、`Save` 或
...
- **P0 Reader 主链已覆盖**：工具层/面板状态机、正文排版、移动点击与连续滚动、设置、书签、正文
  ...
  后端 persistent-variable/cache stale commit 已随 `0a8a0ef` 本地验证完成，待 GitHub/Actions 发布证据。
  新换源写入 inventory 只处理 fetch 后并发提交资格，不推翻已签收的候选/面板/位置可见模块。
