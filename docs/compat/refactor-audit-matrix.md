...
| Reader：书内正文搜索 | `SearchBookContent.vue`、`vuex.js#dialog*`、`Reader.vue#showSearchContent`、`BookController.searchBookContent/searchChapter`、`SearchResult.kt` | `OverlayBookContentSearch.vue`、`useBookContentSearch.js`、`useReaderContentSearchIntent.js`、`useReaderSearchNavigation.js`、`readerBookSearch.js`、`backend/services/contentsearch`、`backend/api/books.go` | **2026-08-02 第二轮固定基准审查重新打开为 `must-fix`**。旧 P2 的原始正文、精确/大小写/重叠、游标、取消、UTF-16、多用户和跳转结论继续成立；但可见 shell 实际仍为错误的固定 880px/520px、默认 top、满宽自动聚焦输入、全表 loading、额外空状态和错误 footer。另确认同 ID 的 URL 变化不 reset，前端 intent/Reader/现代与 legacy Go 接口均会 trim 上游本应原样搜索的关键词。 | [`book-content-search-fixed-baseline-second-audit-p2-contract.md`](book-content-search-fixed-baseline-second-audit-p2-contract.md) 为覆盖性合同；状态 `audit-complete / tests-and-implementation-pending`。不得再以旧 569 项测试或 `1aeffb9` 发布记录宣称当前 shell 完全对齐。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **Bookmark 第二轮与此前夜间内容面修复已由本机作为 `f2f0d6e` 发布**：frontend 654/654、Go、build、普通 Reader 桌面/390/360/iPad、真实 EPUB 1440/390/360、新卷 portable v1/v2 assets/跨用户/重启和历史 TXT/EPUB/UMD/CBZ/相对缓存/owner isolation 均通过。 | `f2f0d6e`/`latest` 共同指向 amd64/arm64 OCI index `sha256:cfaba7d453bde2a4b44198aa57ef8ef5ecbaa3b4cce0ab77b4c816311455c736`；等待设备验收。 |
