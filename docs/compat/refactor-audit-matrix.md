...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。阅读进度、外部 WebDAV、远程封面等已发布；六动作中的可见登录/注册、书签批删、旧 Category reorder、TXT 与 Reader 换源均已裁决/发布。**2026-08-11 新确认公开 auth wire boundary 未签收**：login/register 无 body cap、接受首 JSON 后尾随值，bcrypt 过长注册密码误成 `500`。 | 既有专项合同继续有效；新增 [`auth-request-boundary-fixed-baseline-second-audit-p2-contract.md`](auth-request-boundary-fixed-baseline-second-audit-p2-contract.md)，状态 `inventory-complete / implementation-pending`。目标为 16 KiB 声明/流式上限、单 JSON、精确 `413/400/401` 和零副作用；旧账号与 JWT 状态机不得重开。 |
