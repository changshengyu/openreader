...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Explore、远程 BookInfo/TOC 和本地单书 refresh request lifecycle 均已关闭。`8df38f1` 使 local refresh 的 opened read/parse/stage/transaction 传播 caller context，post-stage 复验 owner Book/归档快照，guarded owned-field update 拒绝删除、归档路径或 metadata 变化。 | [`local-book-refresh-request-lifecycle-fixed-baseline-second-audit-p2-contract.md`](local-book-refresh-request-lifecycle-fixed-baseline-second-audit-p2-contract.md) 通过旧实现红测、focused/race/full/vet、真实 BookInfo/Reader 和可信卷门；状态 `aligned / regression-validated / Docker-published / awaiting-device-verification`。`8df38f1` OCI index 为 `sha256:1f6c8c509457043400f19e181b4d52fb8c648d5f84509c7b4fbdd44fdb610232`。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release workflow | `8df38f1` 通过 frontend 742/742、Go full/race/vet、build、Compose、本地格式/归档回归、BookInfo -> refresh-local -> Reader 三视口和 GHCR 回拉容器；受信 Actions run `33068512106` 通过原生候选、fresh portable、historical volume 与 published-platform 门后发布。 | amd64/arm64 发布 `8df38f1`/`latest`，OCI index `sha256:1f6c8c509457043400f19e181b4d52fb8c648d5f84509c7b4fbdd44fdb610232`；manifests 分别为 `sha256:c2a753bbf1dae93bb5c11fe5ec2a478a07950cdcae902d843c4d6bb119e3e587`、`sha256:a83a45245ca889f9c09a18371a9cb066df36a4c3101b7374aeba9785b1ccfc7d`。Git HEAD 为 browser smoke `b338c3f`；用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-08-27，本地书 refresh lifecycle 已发布）
...
入口/分页/取消生命周期、远程 BookInfo/TOC 和本地书 refresh handler 的取消/陈旧提交亦已关闭。剩余
约 1% 当前包含后续逐路由 action 审计、长尾固定基准复审与真实设备证据，不能从 direct binder、
`Save` 或 contextless 调用差集缩小推导完成。
