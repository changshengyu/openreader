...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **P0 持续复核；移动点击翻页第三次用户验证后重新打开**：`18d9183` 的合成 transform 保留为基础，但普通点击仍错误启动选文轮询、正文图层到 touchend 才临时提升，且旧测试允许首段近零位移。下一批先以失败测试固定 touchstart 预热、短点击零重试和 16/32ms 可见起步，再实现。 | [`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；原生 touch/wheel、EPUB、图片和其它模式继续隔离。此前 479、Go/build 和 Docker 证据只证明第二批终点/机制正确，不再作为“体感已完成”的证据。 |
