...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、翻页、格式运行时、设置、夜间面和内联章节缓存均已专项发布。主正文请求生命周期已随 `0a8a0ef` 实施；**后端换源写入生命周期也已随 `3bb465f` 实施**：远程 fetch 后重验当前 Book 与目标 Source，使用 guarded owned-column update 和权威重载，删除、较新换源或目标 Source 失效均不会被迟到结果覆盖。 | 正文与换源合同均通过 frontend 748/748、build、Compose、四视口及后端 focused/adjacent/race/full/vet；状态 `aligned / regression-validated / GitHub-sync-and-Docker-evidence-pending`。换源写入合同见 [`reader-source-change-write-lifecycle-fixed-baseline-second-audit-p0-contract.md`](reader-source-change-write-lifecycle-fixed-baseline-second-audit-p0-contract.md)；候选 UI、三 mode 和位置恢复不重开。 |
...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category/remote-add、章节正文和 `changeBookSource` 的列所有权/远程工作后提交资格均已测试先行实施。换源 transaction 现在复验 current Book、active association 与 Source fetch semantics，并以权威 current row 投影响应和事件。 | [`reader-source-change-write-lifecycle-fixed-baseline-second-audit-p0-contract.md`](reader-source-change-write-lifecycle-fixed-baseline-second-audit-p0-contract.md) 已按 `31b2963`/`5734f74`/`3bb465f` 的合同、红测、实现顺序关闭；状态 `aligned / regression-validated / GitHub-sync-and-Docker-evidence-pending`。下一 action 必须重新从当前路由与固定上游取证。 |
...
## 当前整体进度快照（2026-09-09，Reader source-change write lifecycle implemented）
...
实施并完成本地全量及四视口验证，GitHub 同步与可信 Actions/Docker 证据因 443 超时待补。换源 fetch
后的 Book/Source 存活复验、显式列更新和权威响应生命周期也已按 `31b2963`/`5734f74`/`3bb465f` 的
合同、红测、实现顺序关闭，并通过本地全量及四视口验证。剩余约 1% 还包含后续逐路由 action 审计、
长尾固定基准复审、待补可信发布证据与真实设备证据，不能从 direct binder、`Save` 或 contextless 调用
差集缩小推导完成。
...
- **P0 Reader 主链已覆盖**：工具层/面板状态机、正文排版、移动点击与连续滚动、设置、书签、正文
  ...
  后端 persistent-variable/cache stale commit 已随 `0a8a0ef` 本地验证完成；换源写入并发提交资格已随
  `3bb465f` 本地验证完成。两者待 GitHub/Actions 发布证据，且均不推翻已签收的候选/面板/位置可见模块。
