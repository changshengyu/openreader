...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | **iPad 关闭路径与第七批连续点击已发布；第八批点击视觉节奏已随 `a54bd72` 发布**：竖向 `page/scroll/scroll2` 采用有界非零初速、零末速的连续 Hermite 曲线，首刷新位移不超过页面步长 4%；双手机向上/向下点击、连续双击、Long Task、LayoutShift 和段落几何均通过，未观测到正文重排。 | [`reader-ipad-panel-dismiss-p0-contract.md`](reader-ipad-panel-dismiss-p0-contract.md)、[`reader-mobile-page-click-p0-contract.md`](reader-mobile-page-click-p0-contract.md)；等待用户用 `a54bd72` 真机复验体感。 |
