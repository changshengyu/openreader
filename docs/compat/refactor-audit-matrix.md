...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、格式运行时、设置、夜间面、远程正文和换源写入生命周期均已专项实施。**真机否决 `e1631d0` 的整书串行修复后，`0ecc4d9` 已把远程 gate 收缩到同章；相邻章节恢复并行且保留同章合并与生命周期 CAS**。同日桌面翻页反馈已完成本地实现：纵向文本恢复 document-root，点击区恢复 30/40/30，滚轮/触控板保持原生且不在边界合成切章；用户要求的固定章节运行栏保留。 | [`reader-remote-chapter-concurrency-device-feedback-p0-contract.md`](reader-remote-chapter-concurrency-device-feedback-p0-contract.md) 已用旧实现红测、Gin 并发、race/full/vet 和 frontend/build/Compose 复验，状态 `implemented / regression-validated / Docker-pending / awaiting-device-verification`；桌面翻页见 [`reader-desktop-pagination-device-feedback-p0-contract.md`](reader-desktop-pagination-device-feedback-p0-contract.md)，同样等待 Docker 与真机签收。 |
...
| 远程相邻章节并发 | 上游连续窗口并发加载；正文进入 Chapter variable scope，不要求把同书相邻章串行。 | `e1631d0` 的 user/book gate 会让相邻章产生队头阻塞；`0ecc4d9` 改为 user/book/chapter，同章合并、相邻章并行。 | **2026-09-14 真机重开后已重新实施并回归，Docker-pending**：保留真实生命周期 409、同章 cache 复用和取消；见 [`reader-remote-chapter-concurrency-device-feedback-p0-contract.md`](reader-remote-chapter-concurrency-device-feedback-p0-contract.md)。 |
