...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。公开 auth wire boundary 已发布 `f5c15d7`；管理员 user JSON 写入口与共享密码长度已随 `6c1c6db` 关闭。**2026-08-12 下一项确认用户配置 PUT 仍无 actual-read/single-JSON**：declared/chunked 8 MiB+1、第二 JSON 和垃圾均可写入并回显。 | 管理员专项继续为 `aligned / Docker-published`。新增 [`user-setting-write-boundary-fixed-baseline-second-audit-p2-contract.md`](user-setting-write-boundary-fixed-baseline-second-audit-p2-contract.md)，状态 `inventory-complete / implementation-pending`；目标只增加合法 key 后的 8 MiB 单 JSON 与零副作用，三键/CAS/force/历史行/backup 不重开。 |
