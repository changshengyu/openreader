...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue` | **刷新阅读进度 P1 已实施并完成应用门禁**：完整网络快照现按上游整批替换非 pending 进度，服务器空值清除对应客户端记录；只有真实 `pendingSync` 暂存并 CAS 重试。前端 520/520、Go/build、断开 WebSocket + 2099 年旧缓存的三视口真实浏览器合同通过。BookGroup 与独立 `lastCheckTime` 合同继续有效。 | [`bookshelf-refresh-progress-p1-contract.md`](bookshelf-refresh-progress-p1-contract.md)；Docker 本地镜像/卷门禁与发布记录待完成。 |
