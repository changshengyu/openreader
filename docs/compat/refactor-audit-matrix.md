...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue`、`main.js#getCover`、`BookController#getBookCover` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue`、`BookCover.vue`、`stores/bookshelf.js`、`useSync.js` | **BookManage 第二轮固定基准复审已重开，当前判定为错误重构/待重建**：既有唯一 overlay、缓存/事务/隔离能力可保留，但移动卡片、额外阅读进度、扩展搜索、关闭清空查询、错误几何、无选择禁用、批量分组无确认及“停止 n/total”可见交互均与固定上游冲突。书架/BookInfo 与封面 capability 的既有专项结论不因此回退；BookGroup 将在 BookManage 后独立复审。 | [`book-management-fixed-baseline-second-audit-p2-contract.md`](book-management-fixed-baseline-second-audit-p2-contract.md) 是当前 BookManage 权威合同；BookInfo 仍见 [`bookinfo-action-ownership-cleanup-p1-contract.md`](bookinfo-action-ownership-cleanup-p1-contract.md) 与 [`book-cover-proxy-p2-contract.md`](book-cover-proxy-p2-contract.md)。 |
