...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category patch 与 batch category 列所有权已实施。当前直接差集收敛到 `POST /api/books/remote` existing URL 分支：分组请求仍以 `tx.Save` 提交 URL 查询时的完整 Book 快照。 | [`remote-book-existing-add-write-lifecycle-fixed-baseline-second-audit-p2-contract.md`](remote-book-existing-add-write-lifecycle-fixed-baseline-second-audit-p2-contract.md) 已固定现有 200/201/URL/分组/UI 合同，并要求 request-context 重读、guarded `category_id`、删除不复活和权威 event；状态 `inventory-complete / tests-and-implementation-pending`。已签收新 URL 远程工作、临时 Reader、BookInfo/结果卡与 body/owner/source/parser 边界不重开。 |
...
## 当前整体进度快照（2026-09-04，Remote Book existing-add lifecycle inventory）
...
的 Actions 最终状态与 digest 待复核。剩余约 1% 当前包含已确认的 remote existing-add stale
full-row `Save` must-fix、后续逐路由 action 审计、长尾固定基准复审与真实设备证据，
