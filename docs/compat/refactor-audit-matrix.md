...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、`ReturnData.kt` | `backend/api/*.go`、`middleware/*.go`、前端 `api/*.js` | **按动作逐项复审；优先模块已有专项合同，`api-diff.md` 的旧 scaffold 状态已纠正**。公开 auth、管理员 user、用户配置、BookGroup/Category、Book 与 BookSource 写边界均已关闭并发布。下一项 Bookmark wire/字段/并发删除边界已完成 inventory：四个 JSON mutation 仍无 actual-read/single-document 边界，batch create/delete 无 2,000 原始项上限，note update 的空/未知 patch 和整行 `Save` 仍可能清空或复活行。 | [`bookmark-write-boundary-fixed-baseline-second-audit-p2-contract.md`](bookmark-write-boundary-fixed-baseline-second-audit-p2-contract.md) 当前 `inventory-complete / implementation-pending`；只新增请求和列所有权门，不重开已发布的 Bookmark UI、独立 ID、定位、同步或备份语义。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release scripts | **BookSource 写入/导入/配额边界已在 `d9ddc0f` 完成完整回归和 Docker 门**：frontend 740/740、Go full/focused race/vet、build、宿主/本地容器/GHCR 回拉容器真实 HTTP、fresh portable-v1/v2/cross-user/restart、historical TXT/EPUB/UMD/CBZ/relative-cache/owner-isolation与 source legacy/COW/logical+portable/restore/restart 均通过。 | 本机 amd64/arm64 已发布 `d9ddc0f` 与 `latest`，OCI index `sha256:548bf0984e7fa5039411bd75f9ae8ac8496052010255bfe746bf36fa9336dc8f`；两平台 revision label 均为 `d9ddc0f113d4bc2cc76a6722ecaf19f617163345`。用户生产环境当前运行提交仍未知，发布不等于服务器已升级。 |

## 当前整体进度快照（2026-08-12，`d9ddc0f` 发布签收 pass）

按全量计划的模块/合同口径而不是代码行数估算，整体约 **95%**。该数字表示固定基准合同和测试先行
实现覆盖度；用户配置、BookGroup/Category、Book 与 BookSource 写入/导入边界均完成实现、全量、运行时、
新旧卷和正式 Docker 发布；Bookmark 写入边界已完成只读合同、尚未进入红测，不表示剩余 5% 只是简单收尾。
...
- **P2 已覆盖的高风险项**：账号/overlay/cache 隔离、原子阅读进度、删除收敛、替换规则、书签、
  ...
  owner 与归档引用边界，BookSource JSON/cardinality/原子 `sourceLimit`，以及 server-only
  WebSocket/recipient scope 和备份事务 worker。
- **尚未完成的主线**：Bookmark wire/批量/note-only/并发删除边界进入下一测试先行切片；其余尚未逐动作
  签约的 Go REST/错误/事务语义；仍待第二轮固定基准复审的长尾组件；
...
2026-08-12 的下一项 REST 长尾只读审查选定 Bookmark 写入边界。既有 Bookmark 固定上游 UI、独立 ID、
当前书/当前用户隔离、事务、一次 post-commit event 和备份恢复保持已签收；新证据只指出四个 JSON body
仍无 actual-read/single-document 边界，batch create/delete 无原始 2,000 项上限，note update 的空/未知
patch 会清空备注且整行 `Save` 在并发删除后可能 fallback upsert。专项合同已登记为
`inventory-complete / implementation-pending`，应用和测试尚未修改。
