...
状态：**旧 SQLite/路径/缓存夹具与 EPUB/UMD/CBZ/TXT archive API、全格式 Docker
旧卷回归均已完成；历史相对 cache、跨用户 Docker 与可移植 archive 备份仍是后续范围。**
...
- `backend/cmd/create-old-volume-fixture` 生成可由容器启动的旧 SQLite + 相对路径 TXT、以及
  stale absolute `OriginalFile` 的 EPUB、标准 reader-dev UMD、CBZ archive；每书都有旧
  `CachePath` 和 `/retired-host` source/cache 诱饵。`HISTORICAL_VOLUME=1 scripts/docker-volume-backup-smoke.sh`
  已在本地镜像通过四种 archive 的读取、refresh、逻辑备份/restore 和 restart，并逐一比对
  原 archive SHA-256；普通新卷 smoke 也在同一镜像通过。这完成 VOLUME-DB/PATH 与
  VOLUME-FORMATS-2，不替代尚未单独构造的合法相对 cache、跨用户 Docker 夹具。
...
- 本批已通过 `go test ./...`、`npm test`（386）和 `npm run build`；提交后本地重建的
  `:885c8b7` 已先通过 TXT Docker smoke 并已发布
  ...
  `sha256:ca582c2ab0251a712df336083ef16dc188db66f29d3a4efa9246d26a9d4efcba`）。全格式 fixture
  随后针对该镜像完成本地 Docker 验证；下一次发布会带上这份完整验证记录。
