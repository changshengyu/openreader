...
状态：**旧 SQLite/路径/缓存夹具与 EPUB/UMD/CBZ/TXT archive API 回归已完成；完整全格式
Docker fixture 仍待继续。**
...
| 原始 archive | reader-dev 本地书以原始文件为正文重建来源。 | `library/<LibraryPath>/<OriginalFile>` 与 `localBookSourcePath`。`TestHistoricalMountedVolumeRebuildsEPUBUMDAndCBZArchives` 覆盖 stale absolute source + 无派生 content 的三种 archive。 | API 已证实；Docker 部分已证实 | 对 TXT、EPUB、UMD、CBZ 删除 `content/` 后均能读；refresh 只能新建派生 generation，archive 字节哈希不变。 |
...
- `localBookArchiveRoot`、`existingRegularPathInside` 和 db 的受控目录迁移保持无 archive
  ...
  绝对 cache 路径。
- `backend/cmd/create-old-volume-fixture` 生成可由容器启动的旧 SQLite + TXT archive +
  `/retired-host` source/cache 诱饵；`HISTORICAL_VOLUME=1 scripts/docker-volume-backup-smoke.sh`
  已在本地镜像通过读取、refresh、逻辑备份/restore 和 restart。原有新卷 smoke 也在同一
  镜像通过。这个 Docker evidence 只覆盖 VOLUME-DB/PATH/CACHE 与 TXT，不能替代
  EPUB/UMD/CBZ 的 VOLUME-FORMATS-2。
- `TestHistoricalMountedVolumeRebuildsEPUBUMDAndCBZArchives` 将最小 EPUB、标准 reader-dev
  ...
  `refresh-local` 后验证原 archive SHA-256 不变。Docker fixture 仍只包含 TXT，不能把 API
  回归误写成全格式容器验证。
- 本批已通过 `go test ./...`、`npm test`（386）和 `npm run build`；提交后本地重建的
  `:885c8b7` 已复跑上述两种 Docker smoke，并已发布
  `ghcr.io/changshengyu/openreader:885c8b7` 与 `:latest`。两者的多架构 OCI index digest
  均为 `sha256:3a417024275d5b2be412b6d1a9d02f663bf041f38b6131ef9b1cbb4a5025a3c5`
  （amd64 `sha256:d0e9b0b3e6440930229d88e197ac3af32fac31956bd2ccf5ebcc456ea189be16`；arm64
  `sha256:ca582c2ab0251a712df336083ef16dc188db66f29d3a4efa9246d26a9d4efcba`）。发布仍必须
  诚实列出 VOLUME-FORMATS-2 的 EPUB/UMD/CBZ 未完成状态。
