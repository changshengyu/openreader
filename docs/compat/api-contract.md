...
| Cache | `/api/cache/stats`, `/api/cache`, `/api/books/:id/cache`, batch clear and post-commit pruning | Cache operations must not delete unrelated user data. Existing current-user JSON and cache progress/cancel flows remain, but the pending second-audit filesystem lifecycle requires rooted regular-file read/write/stat/remove, accurate existing-file counts, all-user reference fail-closed pruning and write/prune serialization across every caller; see [`remote-chapter-cache-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md`](remote-chapter-cache-filesystem-lifecycle-fixed-baseline-second-audit-p2-contract.md). Status: `inventory-complete / tests-and-implementation-pending`. |
