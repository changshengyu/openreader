...
| `POST /api/local-store/import-preview`, `POST /api/webdav/import-preview` | JSON `{paths}` or `{items:[{path,title,author,tocRule}]}`. JWT and source-specific store permission required. LocalStore already enforces 1 MiB single JSON and 200 raw/expanded items; WebDAV must adopt the same admission in its pending second audit. | `200 {items}`. Each readable regular item is copied once to a caller-scoped immutable stage; success item contains `{path,book,importToken}`. | A parser failure remains an item-level `{path,error,importToken}` in the `200` envelope. The token remains valid for a later `{items:[{path,importToken,tocRule}]}` preview/import; mounted-file mutation/removal cannot affect that retry. WebDAV source-backed reads must use its caller-rooted file service rather than a resolved host path. |
