...
| Remote BookInfo/TOC lifecycle | `/api/reader/remote-sessions`, `/api/books/:id/refresh` | The pending request-lifecycle audit requires the two remaining contextless BookInfo/TOC handlers to propagate caller cancellation. Explicit refresh must also revalidate the owned Book snapshot after remote work and reject a deleted/changed target before chapter or shelf mutation; see [`remote-book-detail-toc-request-lifecycle-fixed-baseline-second-audit-p2-contract.md`](remote-book-detail-toc-request-lifecycle-fixed-baseline-second-audit-p2-contract.md). |
