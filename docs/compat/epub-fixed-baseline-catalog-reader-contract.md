...
状态：**2026-07-18 已完成固定基准复审、失败测试、实现、全量自动化和三视口真实浏览器验证；Docker/历史 volume 发布门禁进行中。**
...
| EPUB-FIXED-6 | API 数据夹具证明旧 fragment rows 可继续打开并在显式刷新后按 resource path 收敛；历史 Docker volume 另验证旧 EPUB 可读取/刷新，且数据库、archive、cache roots、用户隔离和 portable backup 不破坏。 | API historical rows + Docker historical volume/backup |
...
- Docker/历史 volume/portable backup 结果待本批本地镜像完成后补记。
