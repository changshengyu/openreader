...
2. TXT 首先读取 512000 字节探测编码和默认目录规则。无目录时按约 10 KiB 在换行处
   生成伪章节；规则目录下的过长章节再按约 100 KiB 切分。正文从原始字节范围读取，
   所以 BOM、编码和多字节边界均属于兼容面。
...
|---|---|---|---|---|
| TXT 自定义规则无匹配预览 | 正常返回空章节，用户仍可编辑/确认流程。 | `Importer.Preview` 在零章节时返回 `ErrNoReadableChapters`；API 返回 400，现有测试也把此错误编码为正确行为。 | **must-fix** | 先替换 importer/API/前端状态测试：重解析为 `200`、同一 token、`chapterCount=0`、明确不可确认提示；确认导入的最终零章节拒绝语义需以同一合同单独锁定。 |
| TXT 规则目录长章节 | 规则目录下约 100 KiB 切分。 | `parseTXTText` 不执行有目录长章节切分。 | **must-fix** | 增加 UTF-8 与多字节边界夹具，锁定标题、顺序、正文连续性和近似切分阈值。 |
...
| 编号 | 夹具与断言 | 覆盖的入口 |
|---|---|---|
| E4-TXT-1 | UTF-8 BOM、GBK/GB18030、无目录、规则目录和超过 100 KiB 的单章 TXT；断言章节边界、连续正文和重新解析。 | 直接上传、LocalStore、WebDAV、reader content |
| E4-TXT-2 | 显式规则无匹配：预览 `200` + 空章节 + 原 token；界面展示可恢复空态，确认按钮不写 book。 | importer、`/preview`、OverlayBookImport、OverlayStorageImport |
