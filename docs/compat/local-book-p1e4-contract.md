...
状态：**审查完成；E4-TXT-1、E4-TXT-2、E4-EPUB-1 的“纯图片首封面资源”子项、E4-UMD-1 的标准 reader/refresh 与历史 pseudo-archive 恢复子项，以及 E4-CBZ-1 的首图封面投影已完成。其余真实格式、EPUB fragment/跨资源和完整旧挂载卷项目仍待继续。**
...
| EPUB 目录规则 | `toc` 取 TOC 顺序/标题；`spin` 取 spine顺序/标题；`spin+toc` 与 `toc+spin` 只在主列表标题为空时回退另一列表标题；`spin<toc` 与 `toc<spin` 强制采用另一列表标题；空值默认 `spin+toc`。上游 `BookChapter` 同时有起止 fragment 和相邻 URL 边界。 | `buildEPUBChapters` 的全部六种策略及默认值已由 `TestParseEPUBWithRuleCombinesSpineAndNav` 断言，但 `resolveEPUBPath` 目前去除 fragment，且路径 map 会合并同 XHTML 的多个 TOC 项。 | **must-fix（E4-EPUB-2）** | 按 [`epub-fragment-p1e4-contract.md`](epub-fragment-p1e4-contract.md) 先补 NAV/NCX fragment、同资源 slice、跨 XHTML link 和旧 row 回归；资源 URL 必须仍受当前用户和 book capability 限制。 |
...
| EPUB 阅读资源 | 解压资源页显示，图片/样式/锚点有效；同 resource 的 chapter fragment 依 `startFragmentId`/`endFragmentId` 截取，跨 resource 时按下一章节 URL 停止。 | `epubreader` 用受控临时资源和 iframe，属实现替换；相对资源/唯一 resource link 已工作，但当前完整 XHTML 不能表示 fragment slice。 | **must-fix（E4-EPUB-2）** | 受控 capability/CSP 保持不变；实现 document slice 与精确 fragment→目录状态映射，不退回不受限解压。 |
...
| E4-EPUB-2 | **已完成合同提取，待实现**：NAV/NCX fragment、同 XHTML 多目录项 slice、跨 XHTML 链接、相对图片/样式/字体、旧 metadata 和越权/篡改 capability。详见 [`epub-fragment-p1e4-contract.md`](epub-fragment-p1e4-contract.md)。 | parser、import、资源 capability、iframe real browser、migration/security |
...
- E4-TXT-1、E4-TXT-2、EPUB 首封面、E4-UMD-1 与 E4-CBZ-1 已完成；E4-EPUB-2 已完成上游合同提取，下一步先写 NAV/NCX fragment 与 iframe slice 的失败夹具，再处理 E4-PDFMD-1 与 E4-VOLUME-1；每项仍遵守“合同、失败测试、实现”顺序。
