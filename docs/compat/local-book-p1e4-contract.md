...
| E4-VOLUME-1 | **进行中，第二批验证完成**：旧 SQLite 缺列、progress/bookmark、历史绝对 `OriginalFile`/`CachePath`、私有 archive root 与安全 cache migration 已有失败夹具和实现；EPUB/UMD/CBZ/TXT archive 的 API 读取、refresh 与 archive hash 回归，以及四种 archive 的真实 Docker 旧卷读取、刷新、逻辑备份不破坏性、restart 均已通过。Git `885c8b7` 的现有镜像先发布；全格式 Docker fixture 在该镜像上复验通过，下一镜像发布会记录新的 Git/digest。历史绝对路径只能在所属 archive 根内重定位，不能读取宿主绝对路径。逻辑备份 ZIP 不含本地 archive，验证目标是“不破坏挂载卷”而非凭 ZIP 重建本地书。剩余合法相对 cache 一次迁移、跨用户 Docker 夹具与 portable archive backup。详见 [`local-book-old-volume-p1e4-contract.md`](local-book-old-volume-p1e4-contract.md)。 | 启动、列表、章节读取、刷新、逻辑备份不破坏性、Docker volume smoke |
