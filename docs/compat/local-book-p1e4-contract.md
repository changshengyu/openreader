...
| EPUB 目录规则 | `toc` 取 TOC 顺序/标题；`spin` 取 spine 顺序/标题；`spin+toc` 与 `toc+spin` 只在主列表标题为空时回退另一列表标题；`spin<toc` 与 `toc<spin` 强制采用另一列表标题；空值默认 `spin+toc`。 | `buildEPUBChapters` 的全部六种策略及默认值已由 `TestParseEPUBWithRuleCombinesSpineAndNav` 断言。 | **partial** | 继续补 fragment、跨资源章节的真实归档/API/iframe 夹具；资源 URL 必须仍受当前用户和 book capability 限制。 |
...
| E4-EPUB-1 | 六种规则与纯图片 `titlepage.xhtml` 已实现：预览、导入和 iframe 都保留 capability 保护的封面资源；NAV/NCX、fragment、跨资源、相对图片/样式的完整组合夹具仍待继续。 | parser、import、资源 capability、iframe real browser |
...
- E4-TXT-1、E4-TXT-2、EPUB 首封面、E4-UMD-1 与 E4-CBZ-1 已完成；下一项处理 EPUB fragment/跨资源、E4-PDFMD-1 与 E4-VOLUME-1；每项仍遵守“合同、失败测试、实现”顺序。
