...
状态：**TXT、UMD、CBZ、PDF/Markdown 遗留兼容和大部分旧卷门禁已完成；E4-EPUB-2 的历史 slice 能力已发布，但新目录结论经固定基准二次复审后待纠正。**
...
| EPUB 目录规则 | `toc` 按首次 href 顺序去重，重复引用最后一次非 `null` title 写回 resource（空串回退 XHTML `<title>`）；`spin` 单独用 spine/document title。其余含 TOC 的混合规则先读 TOC，故 spine 也观察到 TOC 写回标题；`spin*` 与 `toc*` 的主要差异是主列表顺序/成员。空规则默认 `spin+toc`。 | `buildEPUBChapters` 以 `(resourcePath, resourceFragment)` 保留 NAV/NCX 同 XHTML 多目录项，且现有测试仍按方法参数假设 `+`/`<` 有不同标题。 | **must-fix（固定执行语义）** | 新导入/刷新按 href 和实际 title side effect 重建；旧 fragment rows/capability 继续可读。 |
...
| EPUB 阅读资源 | 固定 Web API 为每个 href 去重后的目录项返回一个解压 XHTML URL；单 iframe 不拼接 resource。`EpubFile#getContent` 的跨 resource 分支用于其它辅助路径，不能代表 Web Reader。 | 单 capability iframe 与相对资源已等价；但新 parser 错把同 href 多 fragment 生成为多个目录项。 | **must-fix（2026-07-18 固定基准复审）** | 新导入/刷新按 href 去重；历史 fragment rows 继续可读。详见 `epub-fixed-baseline-catalog-reader-contract.md`。 |
