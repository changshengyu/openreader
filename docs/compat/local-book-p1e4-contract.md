...
状态：**审查完成；E4-TXT-1、E4-TXT-2、E4-EPUB-1、E4-EPUB-2、E4-UMD-1 与 E4-CBZ-1 已完成实现、回归与 Docker 发布。E4-PDFMD-1 已完成实现与回归，Docker 发布待执行；完整旧挂载卷项目仍待继续。**
...
| E4-PDFMD-1 | **已完成，Docker 发布待执行**：可见 direct UI 收敛为 TXT/EPUB/UMD/CBZ；文本 PDF、扫描/无文本 PDF、Markdown 和 `.text` 的历史 API/archive 阅读、刷新、cache 回建、失败无持久写入与跨用户隔离均有合同测试。详见 [`pdf-markdown-p1e4-contract.md`](pdf-markdown-p1e4-contract.md)。 | 直接上传 UI/API、历史 archive 阅读/刷新；错误状态不得创建书架/archive |
...
- E4-TXT-1、E4-TXT-2、EPUB 首封面、E4-EPUB-2、E4-UMD-1、E4-CBZ-1 与 E4-PDFMD-1 已完成；下一步处理 E4-VOLUME-1；每项仍遵守“合同、失败测试、实现”顺序。
