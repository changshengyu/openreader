...
以下是待实现的 additive API；既有路径及其成功响应保持不变。
...
4. 在一个数据库 transaction 中恢复 JSON 和 portable 本地书的 shelf/chapter 引用；文件采用
   staging → 验证 → 受控 promote，并在 transaction/rename 失败时补偿删除本次新建目录。持久
   写入成功后才发送 bookshelf/progress/bookmark sync 事件。
...
实现顺序固定为：先提交本合同和 API/迁移矩阵；再加入上述失败测试；随后实现 service、薄
Gin handler、前端确认/标识和 Docker smoke；最后才本地构建并发布 GHCR。任何跳过
PORTABLE-DOCKER-6 的候选都不能声称 portable archive backup 已完成。
