...
- The direct browser multi-select/multipart second audit authorizes no migration. Its target changes only future direct-request admission and frontend orchestration: the existing single-object API, two-/three-file stages, 24-hour token cleanup, SQLite rows, library archives, hidden legacy parser formats, backup members and mounted roots remain unchanged. Selected files will be previewed one at a time and confirmed only by their existing caller-scoped token; cancelled tokens remain derived cache and expire normally. See `direct-local-book-import-multipart-workflow-fixed-baseline-second-audit-p1-contract.md`; implementation and mounted-volume evidence are still pending.
