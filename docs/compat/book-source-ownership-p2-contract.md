...
可重试迁移标记和旧卷事务迁移、P2-S2a 的 owner-scoped service，以及 P2-S2b 的书源
管理/调试 REST 已经测试先行实施；**搜索、探索、书籍、Reader、scheduler、备份和管理员
消费者尚未全部切换到关联作用域**。本合同不把仍存的全局查询或旧测试视为正确性依据。
...
- 搜索、探索、远程书、换源、Reader、scheduler、备份恢复、管理员计数/重置仍是 P2-S2c/
  P2-S3/P2-S4 的剩余消费者；因此本切片可提交 Git，但仍不发布 Docker、不宣称模块完成。
