...
- `BookSource.UserID`：`0` 保留给默认模板，真实用户必须为非零 ID。
- `BookSource.Detached`：正常列表/搜索/探索不返回；仅用于保住仍被该用户书籍引用、
  但已不在当前活动配置中的历史书源。普通“删除使用中的书源”仍维持现有 `409`
  保护，不把显式删除悄悄变成 detached。
- `BookSourceNamespace`（名称可在实现时按现有命名规范调整）：每个用户一行初始化
  标记。它必须区分“从未初始化”和“用户显式清空后仍为空”。

默认模板使用同一 `BookSource` 结构的 `UserID = 0` 行，并用默认 namespace 标记区分
“未配置默认”与“已配置为空”。现有 `data/defaultBookSources.json` 必须导入并继续
作为兼容镜像，不能在升级时丢弃。

活动列表、计数、导出、搜索、探索和换源候选只读取：

```text
user_id = current user AND detached = false
```

已在书架中的书籍读取其 `SourceID` 时，还必须同时验证该 source 的 `UserID` 与书籍
`UserID` 相同；允许读取本用户的 detached source，以免默认重置或备份恢复把现有书籍
变成悬空引用。

### 3.2 身份与唯一性

- 上游兼容身份优先使用规范化后的 `BaseURL/bookSourceUrl`，限定在同一 `UserID`。
- 为兼容历史上允许空 URL 的行，空 URL 只能在同一用户内以稳定 ID/规范化名称兜底；
...
1. 在添加所有权前识别旧的全局表，快照所有源行、用户、远程书籍和
   `SourceFailure` 引用。
2. 给每个现有用户复制一份旧活动书源并创建 initialized marker。
3. 按“用户 ID + 旧 source ID”映射更新该用户的 `books.source_id` 和
   `source_failures.source_id`；不得更改本地书 `source_id = 0`。
4. 当前 `defaultBookSources.json` 存在时，它成为 `UserID = 0` 的默认模板；不存在时，
   以升级前的全局活动列表作为默认模板，保持旧系统中新账号看到相同书源的行为。
5. 提交前验证每本远程书的 source 属于同一个用户，所有用户的活动源数量与升级前
   全局数量一致，另一个用户的 source ID 不可用于当前用户。
6. 迁移可重复启动且不重复复制；任何一步失败时，旧表、书籍和失败缓存保持原样。

迁移不得重写书籍内容、章节、进度、书签、缓存路径或用户凭证。旧浏览器
`bookSourceList@<scope>` 缓存需要通过新的 schema/version key 失效，不能在 ID 重映射后
先展示旧 ID 再等待后台刷新。
...
- 默认中同 URL 的书源更新目标用户现有行并尽量保留 ID。
- 新默认源创建为目标用户的新行。
- 不在新列表且未被书籍使用的旧源可以删除。
- 不在新列表但仍被书籍使用的旧源改为 detached，避免悬空 ID；它不再参加搜索、
  探索、候选、管理列表和计数。
- 后续重新导入同 URL 时复用并重新激活 detached 行。
...
1. **迁移**：两用户共享旧 source ID 的旧卷升级后各有私有副本；各自书籍和失败缓存
   指向本用户副本；默认文件优先、无默认文件回退旧列表；二次启动不复制；注入失败
   全事务回滚。
...
2. **P2-S2 API/运行时隔离**：CRUD、搜索/探索、书籍/reader/scheduler、用户级广播。
