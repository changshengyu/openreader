...
| 书源管理 | `components/BookSource.vue`、首页书源入口 | 基础管理、导入预览、远程预览、三段调试、失效检测、批量启停/删除、批量设置分组已有；本批补齐阅读器/详情/全局换源面板的自定义关键词搜索 | 仍缺恢复默认 |
...
| RSS/书海 | `Rss*`、`Explore.vue` | RSS 已抽出共享 `RSSManager`；书海补齐上游式书源分组、展开源、按 `exploreUrl` 入口触发发现结果，后端支持指定入口分页加载 | 书海结果展示仍可继续向上游搜索结果弹层靠拢 |
