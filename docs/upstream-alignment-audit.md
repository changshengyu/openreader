...
| 书架管理 | `components/BookManage.vue`、`BookGroup.vue` | 已有全局弹层；批量删除、批量添加/移除分组走上游主操作区，缓存/清缓存/导出等扩展真实能力收进“更多批量操作”，移动端不再堆叠全部按钮；分组管理已按上游保护语义只允许删除空分组，非空分组不再被删除并解绑书籍 | 基本对齐，继续真机验收移动端；分组显示/隐藏开关仍需后端字段支持 |
...
| WebDAV/备份 | `components/WebDAV.vue` | 设置页和全局弹层共用 `WebDAVBrowser`；目录浏览、上传、新建目录、下载、重命名、删除、导入书架、恢复备份已有；本批补齐移动端“已选择/批量删除/批量加入书架/上传文件/取消”footer | 基本对齐，继续移动端验收 |
