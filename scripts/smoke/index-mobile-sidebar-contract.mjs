...
async function run() {
  ...
  try {
    ...
    assert(Math.abs(closedMargin + 260) < 0.5, `sidebar should default hidden at -260px, got ${closedMargin}`)

    await page.locator('.mobile-menu-trigger').click()
    await waitForSidebarMargin(page, 0)
    const openedSidebar = await rect(page, '.app-sidebar')
    assert(Math.abs(openedSidebar.width - 260) < 0.5, `sidebar width should be 260px, got ${openedSidebar.width}`)
    ...
    assert(Math.abs(beforeScroll.top - afterScroll.top) < 0.5, `bottom icons should not move vertically on sidebar scroll: ${beforeScroll.top} -> ${afterScroll.top}`)
    assert(Math.abs(beforeScroll.left - afterScroll.left) < 0.5, `bottom icons should not move horizontally on sidebar scroll: ${beforeScroll.left} -> ${afterScroll.left}`)
    ...
    assert(Math.abs(duringDrag) < 0.5, `270px drag should bring sidebar to 0px margin, got ${duringDrag}`)
    const dragIcons = await rect(page, '.sidebar-bottom-icons')
    assert(Math.abs(dragIcons.left - beforeScroll.left) < 0.5, `bottom icons should stay stable during drag, got ${dragIcons.left} vs ${beforeScroll.left}`)
    await dispatchTouch(page, 'touchend', 330, 222)
    await waitForSidebarMargin(page, 0)

    assert(failures.length === 0, failures.join('\n'))
    console.log('index-mobile-sidebar: ok 390x844 width=260 dragLimit=270 fixedBottomIcons=true')
    await context.close()
  } finally {
    await browser.close()
  }
